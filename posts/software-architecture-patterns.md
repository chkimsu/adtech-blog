같은 서비스도 코드를 한 덩어리로 둘 수도, 층으로 나눌 수도, 작은 서비스 여럿으로 쪼갤 수도 있어요. **목적과 상황에 따라 짜맞추는 방식이 달라집니다.**

코드를 어떻게 나누고, 어떤 조각이 어떤 조각과 어떻게 대화하게 할지 — 이 큰 그림을 **아키텍처(architecture)**라고 부릅니다. 그리고 자주 쓰여서 이름이 붙은 정형화된 설계도들을 **아키텍처 패턴**이라고 해요.

> 한 줄 요약: 아키텍처 패턴은 **자주 쓰여 이름이 붙은 나누기 방식의 모음**이다. 정답은 없고, 상황에 맞는 선택만 있다.

이 글에서는 가장 자주 등장하는 6가지를 그림과 숫자로 하나씩 풀어봅니다. **이벤트 기반, 계층형, 모놀리식, 마이크로서비스, MVC, 마스터-슬레이브**입니다. 용어는 낯설어도 아이디어는 단순해요.

---

## 1. 이벤트 기반 (Event-Driven) — 사건을 던지고 잊는다

> 한 줄로: 보내는 쪽은 "결제 완료" 한 줄을 남기고 끝낸다. 누가 읽고 무엇을 하는지는 받는 쪽 각자가 정한다.

이벤트 기반은 조각들이 **직접 서로를 부르지 않고, '사건(이벤트)'으로 대화**하는 방식입니다.

가운데에 **이벤트 브로커**가 있어요. 한쪽(생산자, producer)은 "결제 완료!", "주문 취소!" 같은 사건을 브로커에 던집니다. 누가 받을지는 신경 쓰지 않아요. 그 사건에 관심 있는 쪽(소비자, consumer)들이 알아서 가져다 처리합니다.
...
**좋을 때** — 알림·결제·로그처럼 "한 사건 → 여러 처리"가 많을 때, 트래픽이 갑자기 몰려도 브로커의 큐가 완충해 줍니다. **조심할 점** — 흐름이 눈에 안 보여서, "이 사건이 누구를 거쳐 어디로 갔지?" 추적이 어려워요.
...
## 2. 계층형 (Layered) — 층은 바로 아래층하고만 말한다

> 한 줄로: 요청은 화면 → 로직 → 데이터 접근 → 저장 순서로 한 층씩 내려간다. 화면이 저장소에 바로 가지 않는다.
...
## 3. 모놀리식 (Monolithic) — 모든 기능이 한 덩어리에

> 한 줄로: 게시글, 댓글, 그룹, 미디어가 한 프로그램 안에 있고 한 번에 배포된다. 작을 때는 이만큼 편한 게 없다.
...
## 4. 마이크로서비스 (Microservices) — 작은 서비스 여럿이 따로 돈다

> 한 줄로: 상품, 장바구니, 할인, 주문 서비스가 각자 따로 돌고 각자 DB를 가진다. 할인 서비스가 죽어도 주문은 받는다.

마이크로서비스는 모놀리식을 **작고 독립적인 여러 서비스로 쪼갠** 방식입니다. 상품 서비스, 장바구니 서비스, 할인 서비스, 주문 서비스… 각자 따로 돌고, **각자 자기 DB**를 가져요.

사용자 요청은 먼저 **API 게이트웨이**를 거칩니다. 게이트웨이가 "이 요청은 장바구니 담당" 하고 알맞은 서비스로 보내줘요.
...
## 5. MVC — 화면, 로직, 중재자를 나눈다

> 한 줄로: 사용자 입력을 Controller가 받아 Model에 넘기고, Model이 만든 결과를 View가 화면으로 보여준다.

MVC는 한 화면을 **Model·View·Controller 세 역할로 나누는** 방식입니다. 주로 화면이 있는 앱(웹·모바일)에서 써요.

- **Model** — 데이터와 규칙. 실제 일을 한다.
- **View** — 사용자에게 보이는 화면.
- **Controller** — 사용자 입력을 받아 Model과 View를 잇는 중재자.
...
<text x="305" y="81" text-anchor="middle" style="font-size:10px; fill:var(--text-muted)">중재자</text>
...
<text x="510" y="81" text-anchor="middle" style="font-size:10px; fill:var(--text-muted)">화면</text>
...
<text x="305" y="237" text-anchor="middle" style="font-size:10px; fill:var(--text-muted)">데이터·규칙</text>
...
<figcaption style="margin-top:0.75rem; font-size:0.9rem; color:var(--text-muted)">사용자 액션 → 컨트롤러 → (모델 갱신·DB) → 뷰 생성 → 사용자에게 표시. 역할이 셋으로 나뉘어, 화면만 바꾸거나 로직만 바꾸기 쉽다.</figcaption>
</figure>
...
## 6. 마스터-슬레이브 (Master-Slave) — 쓰기는 한 곳, 읽기는 여럿

> 한 줄로: 쓰기는 원본 DB 한 대에만 한다. 읽기는 원본을 복제한 복사본 여러 대가 나눠 받는다.
...
#
# 원본(primary)은 쓰기를 받는다. 복사본은 읽기만 받는다.
...
정답은 없습니다. **규모·팀·트래픽**에 맞춰 고르는 거예요. 가장 흔한 선택은 모놀리식 ↔ 마이크로서비스입니다. 작게 시작할 땐 모놀리식이 빠르고, 커지면서 부분별로 따로 키워야 할 때 마이크로서비스로 쪼갭니다. "처음부터 잘게 쪼개기"는 대개 과한 선택이에요.
...
시스템을 이렇게 설계했다면, 다음은 **실제로 돌리고 바깥에 노출하는** 일이 남습니다. 그 이야기는 [쿠버네티스 네트워킹 글](post.html?id=kubernetes-networking)에서 — Pod·Service·Ingress가 트래픽을 어떻게 흘려보내는지 같은 방식으로 풀어봅니다.
