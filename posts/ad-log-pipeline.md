...
`BidLog`의 `true_value`와 `shading_ratio`를 같이 남기는 게 실무의 요령입니다. 나중에 "이 입찰가가 왜 이 값이었나"를 되짚어야 할 때가 옵니다. 내부 가치와 깎은 비율이 분리돼 있어야 모델 문제인지 입찰 전략 문제인지 나눠 볼 수 있습니다.
:::
...
1. **로그는 광고 ML의 원재료입니다.** 모든 학습 데이터는 로그의 JOIN으로 만들어집니다. 로그 스키마를 모르면 학습 데이터를 이해할 수 없습니다.
