...
이 구분이 중요한 이유는 난이도가 다르기 때문입니다. 스스로 만든 편향은 되돌릴 값을 우리가 알고 있어서 산수로 풀립니다. 다운샘플링 비율 $r$은 우리가 직접 정한 숫자니까요. 반대로 당하는 편향은 그 값을 추정해야 해서 훨씬 어렵습니다. 그래서 이 글은 쉬운 쪽을 먼저 끝내고 어려운 쪽으로 넘어갑니다. 3절이 Negative Downsampling Bias, 4절이 Sample Selection Bias입니다.
