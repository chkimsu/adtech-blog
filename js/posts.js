...
const posts = [
  ...
  },
  {
    ...
    excerpt: 'POST 는 "수정"이 아니라 "이걸 처리해 줘"다. 그리고 HTTP 는 요청 하나에 응답 하나가 반드시 짝이라, 메서드가 무엇이든 답은 온다 — 작은 서버를 띄워 GET·POST·"조회인데 POST" 셋을 실제로 불러 보면 세 요청 모두 응답 본문이 있다. 메서드 다섯의 뜻, 주소에 동사를 넣으면 안 되는 이유, 내부·파트너·공개 세 등급, 비밀번호를 가리면서 남기는 로그 한 줄. 그 줄은 누가 남기나 — 요청 하나에 부르는 쪽과 받는 쪽이 각각 한 줄씩 남기는데, 도착도 못 한 요청은 서버 로그에 아예 없고 답이 늦어 포기한 요청은 서버 로그에 성공으로 남는다. 그리고 클릭 로그 100만 줄(86MB)을 10,000줄씩 나눠 879KB로, 압축해 131KB로 보내는 법까지.',
    date: '2026-08-15',
