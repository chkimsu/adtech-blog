...
(function () {
  ...
  var STORY_ORDER = ['rtb', 'modeler', 'modeling', 'attribution', 'hb', 'targeting'];
  ...
  var STORIES = {
    ...
    modeling: {
      ...
      slides: [
        ...
        {
          icon: 'scale',
          title: '새 모델을 켠 다음이 진짜 시작이다',
          body: '모델을 바꿨다고 끝이 아닙니다. 예측이 실제보다 자꾸 낮거나 높아지지 않는지, 사람들의 행동이 갑자기 달라지지 않았는지 계속 지켜봅니다. 연휴 하나만 지나도 어제까지 맞던 모델이 오늘 틀립니다.'
        },
        {
          icon: 'recycle',
          title: '이상하면 다시 공부시킨다 — 고리가 닫힌다',
          body: '감시하다 신호가 이상해지면 학습을 앞당겨 다시 돌립니다. 광고의 예측 모델은 한 번 만들고 두는 게 아니라, 매일 이 고리를 한 바퀴씩 도는 일입니다.',
          cta: {
            text: '‘누구 공이냐’ 이야기로 →',
            href: '#attribution',
    ...
    /* ── 6. 두뇌의 하루 (모델러의 눈) ── */
    modeler: {
      id: 'modeler', icon: 'brainpct',
      label: '두뇌의 하루',
      dek: '같은 0.1초를, 예측 모델의 자리에서 다시 본다.',
      slides: [
        {
          icon: 'brainpct',
          title: '이번엔 두뇌 쪽에 앉아 본다',
          body: '‘광고가 뜨기까지’에서는 경매를 밖에서 봤다. 이번엔 자리를 옮겨, 확률을 찍는 쪽에 앉아 같은 0.1초를 다시 본다. 이 일을 하는 사람을 pCTR/pCVR 모델러라고 부른다.'
        },
        {
          icon: 'bidders',
          title: 'DSP가 두뇌에 묻는다 — “이 사람, 누를까?”',
          body: '입찰 요청이 도착하면 DSP는 스스로 답하지 않는다. 두뇌(예측 모델)에게 묻는다. 답이 오기 전까지 입찰가를 정할 수 없어서, 이 질문의 왕복 시간이 곧 전체 시간 예산을 먹는다.',
          term: '두뇌 = pCTR/pCVR 모델',
          example: '허용 시간 약 100ms 중 두뇌 몫은 10ms 안쪽'
        },
        {
          icon: 'funnel',
          title: '먼저 ‘아는 것’을 긁어온다 — 피처 조회',
          body: '이 사람에 대해 아는 것, 이 자리에 대해 아는 것을 창고에서 꺼낸다. 나이대·관심사·최근 본 광고·지면 종류·지금 시각. 이 재료가 없으면 모델은 아무것도 못 한다.',
          term: '재료 = 피처 (Feature) · 창고 = Feature Store',
          example: '피처 수백 개를 1ms 안에'
        },
        {
          icon: 'brainpct',
          title: '확률을 찍는다 — 아직 원값',
          body: '모델이 “이 사람이 이 광고를 누를 확률 2.1%”처럼 숫자를 낸다. 그런데 이 숫자를 바로 쓰면 안 된다. 순위는 맞아도 절댓값이 어긋나 있을 수 있어서다.',
          term: '확률 찍기 = 추론 (Inference)',
          example: '후보 2,000개에 각각 한 번씩'
        },
        {
          icon: 'scale',
          title: '눈금을 맞춘다 — 보정',
          body: '모델이 전체적으로 낮게 보는 버릇이 있으면 입찰가가 통째로 쪼그라든다. 그래서 예측 평균을 실제 평균에 맞춰 눈금을 고친다. 순위를 바꾸는 게 아니라 절댓값을 고치는 일이다.',
          term: '눈금 맞추기 = Calibration',
          compare: {
            shared: '보정을 빼먹으면 낙찰자가 바뀐다',
            left: { head: '보정 전', sub: 'Raw', pay: '2.1%', desc: '낮게 봄 → 너무 싸게 입찰 → 패찰' },
            right: { head: '보정 후', sub: 'Calibrated', pay: '2.4%', desc: '실제와 일치 → 제값에 입찰' }
          }
        },
        {
          icon: 'gavel',
          title: '보정된 확률이 입찰가가 된다',
          body: '확률에 “한 번 눌리면 얼마를 버나”를 곱하면 이 노출의 가치가 나온다. 그 값이 입찰가의 뼈대다. 즉 두뇌가 찍은 숫자가 곧 돈이 된다 — 확률이 10% 틀리면 입찰가도 10% 틀린다.',
          term: '입찰가 뼈대 = pCTR × pCVR × 전환가치',
          example: '42ms에 응답 전송'
        },
        {
          icon: 'medal',
          title: '이긴 것만 보인다 — 여기가 함정이다',
          body: '낙찰되면 노출·클릭·전환이 기록된다. 그런데 패찰한 요청은 아무 기록도 남지 않는다. 그래서 모델은 ‘이겨서 보여준 광고’만 보고 배운다. 안 보여준 광고가 좋았을지는 영원히 알 수 없다.',
          term: '이긴 것만 보는 편향 = Selection Bias',
          example: '낙찰률이 낮을수록 이 편향은 커진다'
        },
        {
          icon: 'stack',
          title: '채점표가 쌓인다 — 봤나·눌렀나·샀나',
          body: '노출은 즉시, 클릭은 몇 초 뒤, 구매는 며칠 뒤에 기록된다. 그래서 오늘 학습을 돌릴 때 어제 클릭한 사람의 ‘구매했나’ 칸은 아직 비어 있다. 이 시차가 pCVR을 어렵게 만드는 이유다.',
          term: '늦게 오는 정답 = 지연 전환 (Delayed Feedback)',
          example: '전환의 절반 이상이 클릭 다음 날 이후 도착'
        },
        {
          icon: 'recycle',
          title: '고리가 닫힌다 — 그리고 다시 돈다',
          body: '쌓인 기록으로 새 모델을 학습하고, 조심스레 올리고, 예측이 현실에서 밀리는지 지켜본다. 밀리기 시작하면 다시 학습한다. 모델러의 하루는 이 고리를 한 바퀴 도는 일이다.',
          term: '지켜보기 = 모니터링 · 밀림 = 드리프트',
          cta: {
            text: '‘모델이 배우는 법’으로 →',
            href: '#modeling',
            note: '학습 쪽을 더 자세히 보고 싶다면 이 이야기로.'
          }
        }
      ]
    },
      ...
      slides: [
        ...
        },
        {
          ...
          cta: {
            text: '전체 지도(자세히)로 →',
            href: 'ecosystem.html',
            note: '5가지 이야기를 다 봤어요. 이제 21개 모듈 전체 지도도 한눈에 볼까요?'
          }
        }
      ]
    }
