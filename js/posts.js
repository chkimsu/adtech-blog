...
const posts = [
  ...
  },
  {
    ...
    excerpt: 'POST 는 "수정"이 아니라 "이걸 처리해 줘"다. 그리고 HTTP 는 요청 하나에 응답 하나가 반드시 짝이라, 메서드가 무엇이든 답은 온다 — 작은 서버를 띄워 GET·POST·"조회인데 POST" 셋을 실제로 불러 보면 세 요청 모두 응답 본문이 있다. 메서드 다섯의 뜻, 주소에 동사를 넣으면 안 되는 이유, 내부·파트너·공개 세 등급, 비밀번호를 가리면서 남기는 로그 한 줄. 그 줄은 누가 남기나 — 로그는 두 종류다. 요청이 오면 코드 없이 남는 접속 로그와, 부르는 쪽이 서버를 한 번 더 불러야 남는 이벤트 로그. 광고가 화면에 떴는지 사용자가 눌렀는지는 서버가 알 길이 없어서, 앱이 그 사실을 요청으로 보내는 것 말고는 방법이 없다. 그래서 두 줄이 자주 다른 말을 한다 — 도착도 못 한 요청은 서버 로그에 아예 없고, 답이 늦어 포기한 요청은 서버 로그에 성공으로 남는다. 그리고 클릭 로그 100만 줄(86MB)을 10,000줄씩 나눠 879KB로, 압축해 131KB로 보내는 법까지.',
    date: '2026-08-15',
  ...
  },
  {
    ...
    excerpt: '입사 나흘째에 첫 연동을 맡으면, 먼저 물어야 할 것은 무슨 언어로 짜나가 아니라 누가 부르나다 — 앱이 초당 2,639건을 보내는 수집 API 와 12ms 안에 답해야 하는 입찰 API 는 비밀키·인증·버전·재시도가 통째로 갈린다. 첫 주에 가장 비싼 약속은 같은 요청을 두 번 세지 않겠다는 멱등성이다. 요청마다 고유 값을 안 붙이고 재시도를 걸면 전환 1,000건이 1,180건으로 부풀고, 전환 한 건당 광고비가 ₩5,000에서 ₩4,237로 싸 보여 예산이 잘못 움직인다. 재시도 시각을 무작위로 흩으면 500건 중 성공이 180건에서 472건으로 늘어난다. 상태코드·목록 넘기기·필드 호환·인증 다섯과 첫 주에 밟는 지뢰 여덟을 실제 요청과 응답으로 본다.',
    date: '2026-08-14',
    categories: ['Software Engineering'],
