...
불과 몇 년 전만 해도 광고는 이렇게 팔렸습니다. 신문사 영업사원이 전화를 겁니다. 광고주와 며칠에 걸쳐 가격을 흥정합니다. 계약서에 도장을 찍습니다. 한 지면에 광고 하나, 한 달 단위 거래였습니다.

지금은 다릅니다. 여러분이 인스타그램 피드를 내리는 그 순간, 이 흥정이 다시 열립니다. 0.1초 안에 끝납니다. 사람 대신 기계가 합니다. 노출 하나마다 매번 새로 합니다. 사람이 전화로 하루 몇 건 흥정하던 일을, 지금은 전 세계에서 쉴 새 없이 반복합니다. 그래서 이 판이 복잡해졌습니다.

복잡함의 원인은 기술이 아니라 **이해관계**입니다. 한 번의 광고 노출(Impression)에는 보통 **세 명의 이해관계자**가 있습니다.
...
회사가 늘어나면 좋은 점도 있고 나쁜 점도 있습니다. 좋은 점은 경쟁입니다. 누구나 이 표준을 따르면 경매에 낄 수 있습니다. 나쁜 점은 회사 경계마다 수수료가 붙고, 데이터가 조금씩 끊긴다는 겁니다. 예를 들어 DSP는 경매에서 진 다른 회사가 얼마를 썼는지 알 수 없습니다. 이걸 Censored Data라 부릅니다(§2.4에서 다시 다룹니다).

이 세계를 여는 표준 규격이 **OpenRTB**이고, 이 세계 전체를 **열린 RTB**라고 부릅니다. 앞으로 이 블로그 글 제목 옆에 '무대: 열린 RTB'라고 적힌 배지가 보일 겁니다. 바로 이 여러 회사 릴레이 얘기라는 뜻입니다. 다른 글을 열기 전에 이 배지부터 확인하는 습관을 들이면 헷갈림이 줄어듭니다.

### 반대로 한 회사가 이 사슬을 통째로 갖고 있다면 [무대: 닫힌 생태계]

**네이버·카카오·메타·구글처럼, 한 회사가 DSP부터 매체까지 전부 가진 세계도 있습니다.** 이런 회사를 **닫힌 생태계(Walled Garden)**라 부릅니다.

네이버 안에서는 네이버가 DSP이자 Exchange이자 매체입니다. 광고주가 GFA에 소재를 올리면 같은 회사 안에서 경매가 열립니다. 같은 회사가 운영하는 지면에 그대로 뜹니다. 회사 경계를 넘는 순간 자체가 없습니다.

그래서 담장 안은 다릅니다. 수수료를 나눌 회사가 적고, 로그가 한 곳에 다 모입니다. 사용자가 로그인해 있으니 노출·클릭·구매를 같은 사람 것으로 정확히 이을 수 있습니다. 대신 그 데이터는 담장 밖에서는 안 보입니다.

앞으로 이 블로그 글 제목 옆에 '무대: 닫힌 생태계'라고 적힌 배지가 보일 겁니다. 바로 이 한 회사 세계 얘기라는 뜻입니다. 두 배지가 섞여 있거나 '무대: 공통'이라고 적힌 글도 있습니다. 두 세계 모두에 해당한다는 뜻입니다. 역시 다른 글을 열기 전에 배지부터 확인하면 좋습니다.
...
pCTR이 3.2%, pCVR이 5%, 전환가치가 $20이라고 해봅시다.

`True Value = 0.032 × 0.05 × 20 = $0.032`

이 값이 노출 1회당 기댓값입니다. 여기에 Bid Shading으로 깎은 값이 최종 입찰가가 됩니다.

이 곱셈 하나를 직접 코드로 돌려 보면, 왜 이 블로그의 거의 모든 글이 이 공식으로 돌아오는지 감이 옵니다. 이번엔 원화로, 매체 쪽 CPM 시각까지 같이 넣어 계산해 봅니다.

```python
# 광고 한 번의 노출이 "얼마짜리"인지 계산해 본다.
# 광고판의 거의 모든 돈 계산은 이 곱셈 하나로 통한다: 노출 -> 클릭 -> 전환.

CPM = 3_000                # 이 지면의 노출 단가: 1,000회 노출에 3,000원(매체가 부르는 값)
cost_per_impression = CPM / 1000     # 노출 1회의 원가

pCTR = 0.01                # 이 유저가 이 광고를 클릭할 확률(pCTR 모델의 출력)
pCVR = 0.05                # 클릭 후 실제로 구매할 확률(pCVR 모델의 출력)
conversion_value = 20_000  # 전환 1건이 광고주에게 주는 가치(예: 마진)

# 1) CPM -> CPC 환산: 이 지면을 클릭 기준으로 사면 얼마인 셈인가
equivalent_cpc = cost_per_impression / pCTR

# 2) CPC -> CPA 환산: 전환 기준으로 사면 얼마인 셈인가
equivalent_cpa = equivalent_cpc / pCVR

# 3) 거꾸로 계산: "전환 1건에 20,000원까지는 낼 만하다"고 정했다면
#    이 노출 한 번의 기대가치(= 부를 수 있는 최대 입찰가)는 얼마인가
max_bid = conversion_value * pCTR * pCVR

print(f"노출당 원가                  {cost_per_impression:,.1f}원")
print(f"환산 CPC                     {equivalent_cpc:,.0f}원")
print(f"환산 CPA                     {equivalent_cpa:,.0f}원")
print(f"이 노출의 기대가치(최대 입찰가)  {max_bid:,.1f}원")
print(f"이문(기대가치 - 원가)          {max_bid - cost_per_impression:+,.1f}원")

# 출력:
# 노출당 원가                  3.0원
# 환산 CPC                     300원
# 환산 CPA                     6,000원
# 이 노출의 기대가치(최대 입찰가)  10.0원
# 이문(기대가치 - 원가)          +7.0원
```

숫자 다섯 개가 하는 말은 결국 하나입니다. 이 지면은 원가 3원인데, 내가 기대하는 가치는 10원입니다. 그러니 사도 남는 장사입니다. eCPM(§2.3)도, 랭킹의 True Value도, 뒤에 나올 자동입찰도 전부 이 다섯 줄 계산의 변주일 뿐입니다.

같은 지면, 같은 가격(CPM 3,000원)인데 pCTR만 바뀌면 어떻게 될까요? 위 코드의 pCTR 값을 다섯 번 바꿔 돌려 봤습니다.

| pCTR | 환산 CPC | 환산 CPA(pCVR 5% 가정) | 최대 입찰가(전환가치 2만 원 가정) | 원가(3원) 대비 이문 |
|---|---|---|---|---|
| 0.5% | 600원 | 12,000원 | 5.0원 | +2.0원 |
| 1% | 300원 | 6,000원 | 10.0원 | +7.0원 |
| 2% | 150원 | 3,000원 | 20.0원 | +17.0원 |
| 3% | 100원 | 2,000원 | 30.0원 | +27.0원 |
| 5% | 60원 | 1,200원 | 50.0원 | +47.0원 |

pCTR이 높을수록 같은 지면이 더 싼 거래가 됩니다. 최대 입찰가도 함께 오릅니다. pCTR 모델이 정확할수록 돈을 더 버는 이유가 여기 있습니다.

:::deep 더 깊이 — 광고는 왜 1개가 아니라 1,000개 단위(CPM)로 파는가
CPM의 M은 로마 숫자 1,000을 뜻하는 mille(밀레)입니다. Cost Per Mille, 1,000회당 비용이라는 뜻입니다. 신문·라디오 시절부터 광고는 늘 이 1,000단위로 값을 매겼습니다. 왜 1개 단가로 팔지 않았을까요.

이유는 단순합니다. 노출 1개의 값이 너무 작습니다. 위 계산에서 노출 1회 원가는 3원이었습니다. 실제 시장에는 이보다 훨씬 작은, 소수점 몇 자리짜리 거래도 흔합니다. "노출 1개에 0.003원입니다"라고 협상하는 건 비현실적입니다. 숫자가 너무 작아서 감이 안 옵니다. 1,000개 단위로 묶으면 "3원"이라는, 사람이 다루기 편한 크기의 숫자가 됩니다.

두 번째 이유는 계산 안정성입니다. 광고 시스템은 하루에도 수십억 건의 노출을 처리합니다. 노출 하나하나를 소수점 여러 자리로 계산해 합산하면 반올림 오차가 쌓입니다. 1,000 단위로 묶어 계산하면 이 오차가 훨씬 덜 쌓입니다.

세 번째 이유는 비교의 편의성입니다. 어떤 지면은 하루 노출이 100번이고, 어떤 지면은 1억 번입니다. 규모가 이렇게 다른 지면을 "노출 1개당 가격"으로 비교하면 자릿수가 안 맞습니다. 1,000회라는 공통 단위로 맞추면 어디든 같은 잣대로 비교할 수 있습니다. eCPM(§2.3)이 하는 일이 바로 이것입니다. CPC도 CPA도, 결국 전부 CPM이라는 공통 단위로 환산해 비교합니다.
:::
...
2021년 iOS 14.5부터 Apple이 **ATT(App Tracking Transparency)** 팝업을 의무화했습니다. 유저가 "허용" 누르지 않으면 IDFA(광고 식별자)가 0으로 찍혀 "이 사람이 그 사람인가"를 추적할 수 없습니다. 대신 Apple은 자체 포스트백 프로토콜인 **SKAdNetwork**(최근은 AdAttributionKit)를 제공합니다. 다만 개별 유저 단위가 아니라, **노이즈가 섞인 집계 데이터**로만 전환을 전달합니다. MMP와 퍼포먼스 광고의 측정 패러다임이 전부 다시 짜이는 중입니다.
...
그림으로 먼저 전체를 보고 싶다면 [생태계 지도](ecosystem.html)가 있습니다. 흐름을 재생 버튼으로 따라갈 수 있습니다. pCTR·pCVR을 다루는 실무자 경로가 궁금하면 [ML 엔지니어 트랙](ml-track.html)도 있습니다.

한 가지만 기억해 두세요. 이 블로그의 글 제목 옆에는 작은 색 배지가 붙어 있습니다. '무대: 열린 RTB' 아니면 '무대: 닫힌 생태계'라고 적혀 있습니다. 방금 §1에서 배운 바로 그 구분입니다. 어느 세계 얘기인지 먼저 확인하고 읽으면 훨씬 덜 헷갈립니다.
