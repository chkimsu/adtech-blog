...
# (a) 아침 몰아쓰기: 이른 구간부터 살 수 있는 만큼 사고 예산이 떨어지면 종료
left, rush = BUYABLE, []
for cap in CAP:
...
for name, clicks in [("(a) 아침 몰아쓰기", rush), ("(b) 균등", even), ("(c) 가중", smart)]:
...
# 출력:
# (a) 아침 몰아쓰기 소진 100.0% / 전환 34.0건 / CPA 29411원
# (b) 균등 소진 93.3% / 전환 35.6건 / CPA 26217원
# (c) 가중 소진 100.0% / 전환 49.4건 / CPA 20260원
```

아침 몰아쓰기와 균등이 34건 대 35.6건으로 비슷한 것이 함정입니다. 몰아쓰기는 예산을 다 쓰지만 전환율 3.2%인 저녁을 못 사고, 균등은 살 물량이 없는 새벽 몫 6.7%를 남깁니다. 전환율로 가중하면 같은 100만 원으로 CPA가 29,411원에서 20,260원으로 내려갑니다.
...
PID 는 원래 온도 조절기 같은 제어 장치에 쓰이는 방법입니다. 목표 온도를 걸어 두면 지금 온도와의 차이만큼 세기를 올리거나 내립니다. 예산 페이서도 같은 계산을 하고, 재는 값이 하나뿐입니다. 시간당 목표보다 많이 썼으면 $\lambda$를 내리고, 적게 썼으면 올립니다. 아래 그림이 그 네 단계입니다.
...
이 출력은 **잘 도는 예가 아닙니다**. 소진율 85.6%로 144달러가 남았습니다. 오차를 달러 그대로 곱한 탓에 t=6h에서 $\lambda$가 1.0에서 0.1로 곤두박질칩니다. 오차를 예산 대비 비율로 바꾸고 $K$ 값을 작게 잡으면 이 진동이 잦아듭니다. 실제 튜닝은 이 글의 범위를 넘으니, 여기서는 $K$ 를 잘못 잡으면 이렇게 된다는 예로 남깁니다.
...
여러 캠페인에 한정된 예산을 나누는 문제를 손으로 만져 보는 데모입니다. **한 곳에 몰아주면 그 캠페인의 한계 효용이 떨어져 전체가 손해**라는 것이 곡선으로 보입니다. 앞 절의 페이싱이 시간에 대해 하는 일을 여기서는 캠페인 사이에서 합니다.
...
지면과 경매를 한 회사가 쥐고 있으면 총량이 이미 보입니다. 계획을 추정이 아니라 계산으로 세웁니다. 하루에 그 지면에 요청이 몇 건 오는지, 그중 이 광고주의 조건에 맞는 것이 몇 건인지를 자기 로그에서 셉니다. 그래서 위 표의 과대예측, 과소예측 문제가 훨씬 작습니다. 남는 문제는 캠페인끼리 같은 요청을 두고 겨루는 다음 절의 내부 경쟁입니다.
