...
pCTR의 정의와 eCPM으로 이어지는 길은 [pCTR 예측](post.html?id=pctr-prediction)에서 다룹니다. 이 글은 그 확률을 계산하는 모델의 안쪽만 다룹니다.
...
# FP32(4바이트)로 저장했을 때 메모리. 서버에 올라가는지가 여기서 정해진다.
GB = 1024 ** 3
...
- `user_interest=러닝` 단독으로는 CTR 예측에 거의 도움이 안 됩니다
- `user_interest=러닝 AND ad_category=운동화` 조합은 CTR을 크게 높입니다
- `user_interest=러닝 AND ad_category=운동화 AND hour=21` 3차 조합은 더 정확합니다

이 Feature Interaction을 어떻게 포착하느냐가 CTR 모델 진화의 핵심 방향입니다.
...
# 피처가 늘어나면 두 방법의 격차가 벌어진다. 서빙에 올릴 수 있는지가 이 차이로 정해진다.
...
곱셈을 전개하면 항이 두 종류로 나뉩니다. 자기 자신과 곱한 대각선 항 $z_i^2$, 그리고 서로 다른 둘을 곱한 항 $z_i z_j$입니다. 대각선이 아닌 항은 $(i, j)$와 $(j, i)$로 두 번씩 나오니 계수가 2입니다. 그래서 양변에서 대각선을 빼고 2로 나누면 원하던 값만 남습니다.
...
# attention 가중치가 실제로 어떻게 달라지는지 확인한다. 표준 라이브러리만 사용한다.
import random, math
...
AXIS = {"운동": 0, "전자기기": 2, "여행": 3}     # 카테고리마다 쓰는 자리를 다르게
...
> 실무 원칙: 모델의 AUC를 1% 올리는 일보다, 적절한 모델을 적절한 단계에 배치하는 시스템 설계가 비즈니스 임팩트가 더 큽니다.
...
같은 DIN 논문을 읽고 같은 코드를 짜도, 어느 회사에서는 AUC가 오르고 어느 회사에서는 안 오릅니다. 차이를 만드는 것은 모델이 아니라 **로그**입니다.
...
열린 RTB의 DSP는 같은 모델을 쓸 수 없습니다. 이유는 하나입니다. **유저를 이어 붙일 식별자가 없습니다.**
...
- 전환은 며칠 뒤에 오는 문제 → [Online Learning 과 지연 피드백](post.html?id=online-learning-delayed-feedback)
- pCTR이 입찰가로 바뀌는 과정 → [Bid Shading](post.html?id=bid-shading-censored)
