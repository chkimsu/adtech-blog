...
문제는 이 재료를 **두 번 꺼내 써야** 한다는 것입니다. 한 번은 모델을 가르칠 때(학습), 또 한 번은 실제로 광고를 고를 때(서빙)입니다. 그리고 이 둘이 조금이라도 다르면 모델은 조용히 틀립니다.
...
담장 안에서는 이 맞바꿈이 한쪽으로 유리합니다. 로그인 ID가 안정적이라 **유저 피처를 미리 계산해 캐시에 얹어 둘 수 있기 때문입니다.** 요청이 오면 꺼내 쓰기만 하니 조회 시간이 거의 안 듭니다.
