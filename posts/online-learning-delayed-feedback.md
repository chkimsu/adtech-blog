...
이 글은 세 가지를 차례로 해부합니다. 먼저 **왜 모델이 낡아지는가**(Concept Drift). 다음으로 **어떻게 최신 상태를 유지하는가**(Online Learning). 마지막으로 **전환 지연이 학습을 어떻게 방해하는가**(Delayed Feedback). Concept Drift 는 세상이 변해 모델이 낡는 것이고, Delayed Feedback 은 라벨이 늦게 도착하는 것입니다.

> [pCVR 모델링 포스트](post.html?id=pcvr-modeling)에서 지연 전환이 학습 라벨을 어떻게 오염시키는지 다뤘습니다. 이 글은 그 문제를 더 넓은 맥락 — Online Learning 전체 파이프라인 — 에서 다룹니다.
...
말이 어렵지만 뜻은 단순합니다. 어제까지 맞던 답이 오늘은 틀린 답이 되는 것입니다. 모델은 자기가 낡았다는 걸 스스로 모릅니다. 그래서 성능이 떨어지는 게 아니라, **떨어지는 줄 모른 채로 계속 자신 있게 틀립니다.**
...
정리하면 처방이 셋으로 나뉩니다. Recurring Drift는 **피처로** 흡수합니다. Gradual Drift는 **정기 재학습**으로 따라갑니다. Sudden Drift만이 **즉시 재학습**을 필요로 합니다. 그런데 즉시 재학습은 그렇게 간단하지 않습니다. 다음 절부터 "얼마나 자주 다시 배울 것인가"라는 질문 하나가 어떻게 이 글 전체의 주제로 커지는지 보겠습니다.
...
Online Learning의 단점 칸에 적힌 **Catastrophic Forgetting**은 추상적으로 들립니다. 직접 만들어 보면 손에 잡힙니다. 1절의 예시를 그대로 씁니다. 평일 CTR 1.8%, 주말 CTR 3.2%. 4주 동안 1시간 단위로 관측이 들어오고, 학습기는 그걸 보며 예측을 갱신합니다.
...
요일 피처를 넣은 학습기만 주말 첫 시간부터 3.03%로 출발합니다. 급변 직후 오차가 0.165%p로, 최고 학습률 모델의 4분의 1입니다. **되풀이되는 패턴은 학습률로 풀 문제가 아니라 피처로 풀 문제입니다.** 1절에서 Recurring Drift는 재학습이 필요 없다고 한 이유가 이것입니다. 구조로 푸는 접근은 [Multi-Task Learning](post.html?id=multi-task-learning)으로 이어집니다.
...
- 그 사이 시장이 한 번이라도 급변하면(1절의 Sudden Drift) 정확한 라벨로 낡은 세상을 배우게 됩니다.
...
> 가중치의 뜻은 아래에 적고, 지수분포로 근사한 유도는 이 절 끝의 「더 깊이」에 있습니다. 지연 전환이 라벨을 오염시키는 문제 자체는 [pCVR 모델링 포스트](post.html?id=pcvr-modeling)에 있습니다.
...
앞 코드에서는 지연 분포를 구간별 표로 들고 다녔습니다. 실무에서는 이걸 **지수분포** 하나로 근사하는 경우가 많습니다. 왜 그런지, 그리고 무엇을 잃는지 계산해 봅시다.

지연 시간 $D$가 파라미터 $\lambda$의 지수분포라고 가정하면 CDF는 이렇게 한 줄입니다.

$$F_d(t) = 1 - e^{-\lambda t}$$

파라미터가 하나뿐이라는 게 핵심입니다. 평균 지연 $\mathbb{E}[D] = 1/\lambda$ 하나만 알면 모든 $t$에서 $F_d(t)$가 정해집니다. 앞 코드의 표는 구간 6개 × 비율 = 값 6개를 관리해야 했습니다. 지수분포는 숫자 하나입니다. 캠페인마다, 광고주마다 따로 추정해야 하는 상황에서 이 차이는 큽니다.

FSIW 가중치도 닫힌 식이 됩니다. Positive 가중치를 대입하면 이렇습니다.

$$w^{(+)}(t) = \frac{1}{1 - e^{-\lambda t}}$$

여기서 바로 읽히는 사실이 두 개 있습니다. 첫째, $t \to 0$이면 $w^{(+)} \to \infty$입니다. 방금 누른 클릭에서 도착한 전환은 무한히 큰 가중치를 받습니다. 그래서 실무에서는 반드시 상한을 씌웁니다(clipping). 앞 절 코드의 0~1시간 구간이 2.40%로 출렁였던 이유가 이것입니다. 둘째, $\lambda t$가 3을 넘으면 $F_d > 0.95$이므로 가중치는 1.05 근처까지 내려옵니다. 평균 지연의 세 배만 기다리면 보정이 거의 필요 없어집니다.

$\lambda$ 추정도 간단합니다. 이미 도착한 전환들의 지연 시간 평균을 쓰면 되는데, 여기에 함정이 있습니다. 관측된 전환은 "이미 도착한 것"만 모인 표본이라 지연이 짧은 쪽으로 치우쳐 있습니다. 이것을 그대로 평균 내면 $\lambda$를 과대추정합니다(즉 실제보다 빠르다고 믿습니다). 그래서 우도를 쓸 때 **아직 안 온 전환은 절단(censoring)으로 처리**합니다. 클릭 후 $t_i$가 지났는데 아직 전환이 없는 건은 "$D > t_i$였다"는 정보로만 씁니다.

$$\log L = \sum_{i \in \text{도착}} \log \left( \lambda e^{-\lambda d_i} \right) + \sum_{j \in \text{미도착}} \log \left( e^{-\lambda t_j} \right)$$

이 절단 처리는 패찰 가격 문제와 수학적으로 같은 모양입니다. [Bid Shading](post.html?id=bid-shading-censored) 글에서 다룹니다. 한쪽은 "얼마에 졌는지 모른다", 다른 쪽은 "언제 전환할지 모른다"입니다. 둘 다 "관측이 한쪽에서 잘려 있다"는 같은 문제입니다.

잃는 것도 분명합니다. 지수분포는 **무기억성**을 가정합니다. 6시간을 기다린 클릭이 앞으로 더 기다려야 할 시간의 분포가, 방금 누른 클릭과 같다는 뜻입니다. 실제 전환은 그렇지 않습니다. 30분 안에 35%가 몰리는 급한 봉우리가 있고, 3~7일에 늘어지는 꼬리가 따로 있습니다. 이런 이봉 구조를 지수분포 하나로는 못 담습니다. 그래서 정교하게 갈 때는 지수분포 두세 개를 섞거나(mixture), 와이불(Weibull)처럼 위험률이 시간에 따라 변하는 분포를 씁니다.
:::
...
셋의 갱신 주기가 다른 것도 그래서입니다. 층마다 감당할 수 있는 속도가 다릅니다. 빠른 층이 느린 층의 실수를 임시로 덮고, 느린 층이 빠른 층으로는 못 고치는 근본을 나중에 바로잡습니다.
...
구분하는 방법은 4절의 $F_d$를 그대로 쓰는 것입니다. 어제 12시 클릭이라면 지금까지 도착할 확률이 얼마인지 계산해서, 관측값을 그 확률로 나눠 봅니다. 나눈 값이 평소 수준이면 지연 때문입니다. 나눈 값도 낮으면 진짜 하락입니다. 지연을 감안하지 않은 대시보드는 매일 아침 가짜 경보를 울립니다.
...
그래서 담장 안에서는 짧은 창을 공격적으로 씁니다. 1~6시간 창으로 자주 학습하고, 안 들어온 전환은 $F_d$ 역수 가중으로 되돌립니다. 4절에서 계산한 보정이 실제로 잘 먹는 환경입니다. 지연 분포를 캠페인별로 매일 다시 추정해도 비용이 크지 않습니다. 피처의 온·오프라인 일관성만 지켜지면 이 루프는 꽤 조용히 돕니다. 그 부분은 [Feature Store](post.html?id=feature-store-serving)에서 다룹니다.

대신 다른 숙제가 생깁니다. 담장 안은 지면이 몇 개로 정해져 있어 **Recurring Drift가 유독 강합니다.** 같은 검색 지면, 같은 시간대, 같은 유저 군이 매주 반복됩니다. 2절에서 본 것처럼 이건 학습률로 풀 문제가 아닙니다. 요일·시간·지면 피처로 풀어야 합니다.
...
첫 번째가 가장 아픕니다. **유실은 지연이 아닙니다.** 4절의 역수 가중은 "언젠가 오긴 온다"를 전제로 합니다. 안 오는 전환은 아무리 나눠도 복원되지 않습니다. 이걸 지연으로 착각하면 가중치를 과하게 부풀려 pCVR을 반대로 과대추정합니다. 이건 표본이 한쪽으로 잘려 있을 때 생기는 편향입니다. [Negative Sampling](post.html?id=negative-sampling-bias)과 같은 계열입니다.
...
5. **되풀이되는 패턴은 학습률이 아니라 피처로 푼다** — 2절 코드에서 요일 피처를 가진 학습기만 주말 첫 시간부터 3.03%로 출발했습니다. 나머지 넷은 주말을 세 번 겪고도 매번 1.9% 부근에서 다시 배웠습니다. 학습률을 올려 얻는 건 적응 속도뿐이고 기억은 아닙니다.
...
- 지연 전환의 문제 정의(중복, 지연, 클릭 편향) → [pCVR 모델링](post.html?id=pcvr-modeling)
