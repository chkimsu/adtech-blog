...
const posts = [
  ...
  },
  {
    ...
    excerpt: '운동화 브랜드의 마케터 서연 씨가 대시보드에서 소재 A 1.08% 와 소재 B 1.21% 를 보고 「B 로 바꿔도 되나요」를 묻습니다. 노출 40,000 씩에서 나온 두 값이라, 차이 0.13%p 를 그 차이의 흔들림 0.0752%p 로 나누면 1.73 이고 문턱 1.96 을 못 넘습니다. 95% 신뢰구간은 「참값이 이 안에 있을 확률 95%」가 아니라, 같은 실험을 100번 하면 구간이 참값을 품는 횟수가 95번쯤이라는 뜻입니다. 시드를 고정한 시뮬레이션에서 1,000번 중 949번이 품었습니다. 노출을 4배인 160,000 씩으로 모으면 같은 차이에서 값이 3.46 으로 올라 확인됩니다. 주문 금액처럼 평균을 견줄 때 왜 t 가 되는지, 소재가 셋이면 왜 분산을 나누는지까지 같은 숫자 한 벌로 봅니다.',
    date: '2026-10-03',
    categories: [
    ...
    contentUrl: 'posts/stats-confidence-interval-ttest.md'
  },
  {
    id: 'distributed-training',
    world: 'both',
    worldNote: '데이터를 나누고 기울기를 합치는 방법은 두 무대가 같습니다. 다른 것은 임베딩 표의 크기입니다. 담장 안은 로그인 ID 2억 줄이 표의 86% 라 파라미터 서버로 빼야 하고, 열린 RTB 는 쿠키가 며칠이면 바뀌어 임베딩이 몇 GB 에 그칩니다.',
    worldPractical: '담장 안에서는 유저 표 23.8GB 가 학습 때 71.4GB 가 되어 GPU 80GB 한 장에 안 들어가므로, 임베딩만 파라미터 서버 두 대에 41.6GB 씩 나누고 신경망 38MB 만 GPU 넷이 복제합니다. 열린 RTB 에서는 유저 줄에 자리를 주지 않고 광고 소재와 지면과 문맥에 예산을 써서 모델 전체가 GPU 한 장에 들어가고, 파라미터 서버 없이 데이터 병렬만으로 끝납니다. 그래서 담장 안의 설계를 그대로 옮기면 쓰지 않는 서버를 띄우고, 반대로 옮기면 학습이 메모리 부족으로 시작조차 못 합니다.',
    title: '분산 학습: 임베딩이 큰 광고 모델은 왜 다르게 나누나',
    excerpt: '피처를 하나 더하자 이어 학습이 막히고 30일 전체를 처음부터 다시 배워야 합니다. 7억 4,550만 행을 초당 32,700행으로 넘기면 GPU 한 장에 6시간 20분이고, 04:00 에 시작해도 배포 요청 시각 06:09 를 네 시간 넘깁니다. 데이터를 넷으로 나눠 같은 모델 넷이 함께 배우면 1시간 50분이 되는데 네 배가 아니라 3.5배입니다. 기울기를 합치고 늦은 장을 기다리는 시간이 장이 하나 늘 때마다 5.3% 씩 붙기 때문입니다. 광고 모델이 다른 점은 임베딩 표 27.7GB 가 학습 때 83.2GB 가 되어 GPU 80GB 한 장에 안 들어간다는 것이고, 그래서 임베딩만 파라미터 서버 두 대에 쪼개고 GPU 는 한 배치가 건드리는 0.002% 의 줄만 받아 옵니다.',
    date: '2026-10-02',
    categories: [
      'ML Infrastructure'
    ],
    ...
    contentUrl: 'posts/distributed-training.md',
    series: 'mlops-track'
  },
  {
    id: 'batch-vs-realtime-inference',
    world: 'both',
    worldNote: '무엇을 미리 계산해 두고 무엇을 요청 안에서 계산하나는 두 무대가 같은 질문입니다. 다른 것은 미리 만든 값을 찾을 키가 있느냐입니다. 담장 안은 회원 번호가 어제와 오늘이 같고, 열린 RTB 는 쿠키가 자주 바뀝니다.',
    worldPractical: '담장 안에서는 요청에 회원 번호가 실려 와서 새벽에 만든 유저 벡터 표를 그대로 찾습니다. 3.58GB 표가 회원 1,400만 명 거의 전부에게 쓸모가 있고, 요청 때는 0.20ms 짜리 조회 한 번으로 끝납니다. 남는 문제는 값이 낡는 것이라 활성 유저 168만 명만 한 시간마다 다시 만듭니다. 열린 RTB 에서는 쿠키가 바뀌어 어제 만든 벡터를 찾을 키가 없어 요청마다 유저 타워를 돌리고, 그 2.0ms 가 매 입찰 예산에 실립니다. 처음 보는 쿠키는 입력이 비어 있어 지면과 시간 같은 요청 안의 정보가 그 자리를 채웁니다.',
    title: '배치 추론과 실시간 추론: 무엇을 미리 계산해 두나',
    excerpt: '지훈 씨가 일곱째 주에 후보 추리기 코드를 열어 보니 요청마다 유저 벡터를 만들고 있었습니다. 한 요청에 2.0ms 지만 하루 2억 2,800만 번이면 126.7시간입니다. 같은 사람이 하루 16.3번 요청하니, 새벽에 한 번만 만들면 3.3시간으로 38배 줄어듭니다. 회원 1,400만 명의 벡터는 64차원 4바이트로 3.58GB 이고 워커 8개로 25분입니다. 다만 새벽 값은 오늘 낮을 모르니 활성 유저 12% 168만 명만 한 시간마다 다시 만듭니다. 요청 2,639 QPS 를 받는 값은 CPU 12대 한 달 4,147,200원, GPU 3대 8,208,000원입니다.',
    date: '2026-10-01',
    categories: [
      'ML Infrastructure'
    ],
    ...
    contentUrl: 'posts/batch-vs-realtime-inference.md',
    series: 'mlops-track'
  },
  {
    id: 'stream-window-aggregation',
    world: 'both',
    worldNote: '창을 자르는 방법과 워터마크, 늦게 온 클릭을 어떻게 하나는 두 무대가 같습니다. 다른 것은 클릭이 몇 회사를 거쳐 오느냐입니다. 담장 안은 자기 수집 API 로 바로 와 2분이 99.9% 를 덮고, 열린 RTB 는 매체와 중개 회사를 거쳐 와 도착 지연이 시간 단위가 되기도 합니다.',
    worldPractical: '담장 안에서는 클릭이 자기 수집 API 로 바로 오고 회원 ID 까지 붙어 와서, 광고별 창만이 아니라 사람별 창(최근 5분에 누른 광고 수)도 같은 잡에서 만들 수 있습니다. 열린 RTB 에서는 클릭이 매체 서버와 중개 회사를 지나 오고 회사마다 자기 배치 주기가 있어, 워터마크 2분으로는 덮이는 비율이 크게 떨어집니다. 그래서 실시간 피처의 재료를 남을 거쳐 오는 클릭 대신 우리 서버 안에서 생기는 것(받은 입찰 요청 수, 우리가 낸 응답, 이긴 횟수)으로 바꾸고, 전환은 며칠 뒤에 와서 실시간 전환 피처를 사실상 만들지 않습니다.',
    title: '스트림 집계: 최근 1시간 클릭 수는 누가 어떻게 세나',
    excerpt: '모델 팀이 「광고별 최근 1시간 클릭 수」를 실시간 피처로 써 달라고 합니다. 한 줄이라 쉬워 보이지만 15시 1분의 최근 1시간과 15시 7분의 최근 1시간은 다른 구간이고, 그 6분에 클릭 120건이 오갑니다. 클릭은 topic ad.click 에 놓이고 Flink 잡 ad_click_1h 이 세어 Redis 키 ad:9931:clk_1h 에 TTL 2분으로 놓습니다. 창을 자르는 방법이 둘이고(텀블링, 슬라이딩) 창을 닫는 시각을 정하는 값이 워터마크입니다. 워터마크 2분이면 14:00~15:00 창은 15:02 에 닫히고 하루 클릭 228만 건 중 2,280건이 그보다 늦게 와 자기 창을 놓칩니다. 그래서 스트림은 1,203 을 세고 하루 뒤 배치는 1,204 를 셉니다.',
    date: '2026-09-30',
    categories: [
      'ML Infrastructure'
    ],
    ...
    contentUrl: 'posts/stream-window-aggregation.md',
    series: 'mlops-track'
  },
  {
    id: 'model-deployment-rollback',
    world: 'both',
    worldNote: '1% 에서 시작해 게이트를 지나며 넓히고 가중치로 되돌리는 순서는 두 무대가 같습니다. 다른 것은 트래픽을 나누는 자리와 게이트를 읽기까지 걸리는 시간입니다. 담장 안은 요청을 우리가 나누고 30분이면 읽고, 열린 RTB 는 거래소에서 오는 입찰 요청만 나눌 수 있고 몇 시간을 기다립니다.',
    worldPractical: '담장 안에서는 앱과 광고 서버가 한 회사 안에 있어 게이트웨이 가중치로 요청을 무작위로 나누고, 두 무리에 같은 지면의 같은 시간 요청이 섞입니다. 클릭이 2분 안에 99.9% 도착하므로 30분 창의 COPC 가 창이 닫힐 때 거의 확정입니다. 열린 RTB 에서는 입찰 요청의 1% 에만 새 모델의 입찰가를 쓰는데, 새 모델이 낮게 부르면 경매에서 져 노출 자체가 줄어 게이트에 낙찰률이 하나 더 붙습니다. 클릭과 전환이 매체와 광고주 쪽에서 늦게 넘어와 단계마다 몇 시간씩 머물고, 1% 를 하루 두는 팀도 있습니다.',
    title: '모델 배포와 롤백: 1% 로 시작해 15분 안에 되돌리는 순서',
    excerpt: '배포 화면의 첫 칸이 「새 버전에 보낼 트래픽 비율」이고 기본값이 1 입니다. 선배는 되돌리는 순서를 먼저 알고 누르라고 합니다. 새 모델은 트래픽 1% 에서 시작해 게이트 넷을 통과할 때마다 10%, 50%, 100% 로 넓힙니다. 게이트는 8ms 초과율 6% 이하, 오류율 0.1% 이하, COPC 0.95~1.05, 클릭률 차이 -2% 이상입니다. 1% 단계는 5분에 클릭이 79건뿐이라 COPC 를 읽지 못하고 서버가 죽지 않나만 봅니다. 지훈 씨의 첫 배포는 10% 단계에서 COPC 1.09 로 걸려 알람부터 14분 만에 돌아왔고, 그중 기계가 쓴 시간은 1분입니다.',
    date: '2026-09-29',
    categories: [
      'ML Infrastructure'
    ],
    ...
    contentUrl: 'posts/model-deployment-rollback.md',
    series: 'mlops-track'
  },
  {
    id: 'inference-server',
    world: 'both',
    worldNote: '서버가 뜨는 41초와 후보를 한 텐서로 넣는 계산은 두 무대가 같습니다. 다른 것은 미리 해 둘 수 있는 것의 범위입니다. 담장 안은 회원 ID 로 유저 임베딩을 미리 구워 두고, 열린 RTB 는 쿠키라 유저 쪽을 요청 때 계산합니다.',
    worldPractical: '담장 안에서는 요청량과 회원을 알아 웜업 200건을 어제 로그에서 실제 분포대로 뽑고, 회원 ID 로 임베딩 줄을 미리 만들어 서버는 요청마다 조회만 합니다. 열린 RTB 에서는 요청량이 거래소 사정에 따라 튀고 쿠키가 매번 달라, 웜업 표본을 거래소별 로그에서 섞어 뽑고 유저 쪽 숫자 묶음을 요청이 온 그때 계산해 한 텐서 계산에 그 몫이 더 붙습니다.',
    title: '추론 서버: 학습한 모델 파일이 요청에 답하기까지',
    excerpt: '학습이 끝나면 파일이 둘 남습니다. 신경망 38MB 와 임베딩 표 27.7GB 입니다. 작은 쪽은 ONNX 로 바꿔 서버 12대가 뜰 때 내려받고, 큰 쪽은 임베딩 서비스 한 곳에 둡니다. 새 서버는 이미지 시작 12초, 내려받기 6초, 적재 3초, 웜업 20초를 지나 41초에 준비 신호를 켭니다. 요청 하나에 실린 후보 800건은 한 텐서로 한 번에 넣어 3.0ms 이고, 하나씩 부르면 고정 비용이 800번 쌓여 96ms 입니다. 두 버전을 같이 실으면 전환이 설정 한 줄이고, 그 값은 임베딩 27.7GB 입니다. 웜업을 빼면 배포 한 번에 폴백이 5,637건 더 나갑니다.',
    date: '2026-09-28',
    categories: [
      'ML Infrastructure'
    ],
    ...
    contentUrl: 'posts/inference-server.md',
    series: 'mlops-track'
  },
  {
    id: 'container-ci-pipeline',
    world: 'both',
    worldNote: '이미지와 검문 셋은 두 무대가 같습니다. 다른 것은 둘째 검문인 계약 테스트의 약속을 누가 정하느냐입니다. 담장 안은 우리 팀이 정해 한 벌이고, 열린 RTB 는 거래소마다 정해 거래소 수만큼 늘어납니다.',
    worldPractical: '담장 안에서는 요청과 응답의 모양과 예산 8ms 를 우리 팀이 정하니 계약 테스트가 한 벌이고, 이미지 하나가 우리 게이트웨이 뒤의 서버 12대로 갑니다. 열린 RTB 에서는 입찰 서버가 거래소마다 다른 OpenRTB 판과 응답 예산에 맞춰야 해서 계약 테스트가 연결된 거래소 수만큼 있고, 거래소 한 곳이 형식을 바꾸면 그 벌만 걸립니다.',
    title: 'Docker 와 CI: 고친 코드가 서버에 오르기까지의 검문 세 곳',
    excerpt: '노트북에서 잘 돌던 코드가 서버에서는 다른 예측값을 냅니다. 0.0412 와 0.0397, 3.6% 차이입니다. 코드는 같은데 그 아래 깔린 라이브러리 판이 2.3 과 2.4 로 달랐습니다. 이미지는 기본 리눅스부터 코드까지 층을 쌓아 굳힌 한 장이고, 서버 12대가 같은 장을 씁니다. 커밋 하나는 CI 에서 6분 20초 동안 이미지가 되고 검문 셋을 지납니다. 유닛 테스트 214개 48초, 계약 테스트는 스키마 12개와 부하 1,000건에서 8ms 초과율 6% 이하, 이미지 검사는 심각 취약점 0 과 1.5GB 이하입니다. 하나라도 걸리면 이미지는 저장소에 오르지 않습니다.',
    date: '2026-09-27',
    categories: [
      'ML Infrastructure',
    ...
    contentUrl: 'posts/container-ci-pipeline.md',
    series: 'mlops-track'
  },
  {
    id: 'model-versioning-reproducibility',
    world: 'both',
    worldNote: '재료 넷을 적어 두는 표는 두 무대가 같습니다. 다른 것은 재료 중 데이터가 얼마나 자주 바뀌나입니다. 담장 안은 04:00 에 닫힌 파티션이 백필이 아니면 다시 안 써지고, 열린 RTB 는 전환이 며칠에 걸쳐 와서 어제 파티션이 오늘도 다시 써집니다.',
    worldPractical: '담장 안에서는 노출과 클릭이 자기 로그라 스냅샷 번호 하나로 30개 파티션의 그때 내용을 다시 읽을 수 있고, 지원이 없어도 30일치 7억 4,550만 행을 복사해 두는 것이 가능합니다. 열린 RTB 에서는 광고주 서버에서 오는 전환이 파티션을 계속 다시 쓰므로, 학습 실행이 읽은 행을 그 자리에서 복사해 두지 않으면 재현이 처음부터 불가능합니다. 그래서 파티션 목록만이 아니라 읽은 행을 담은 복사본의 위치까지 실행 기록에 적습니다.',
    title: '모델 버전과 재현: 지난주 모델을 오늘 똑같이 다시 만들 수 있나',
    excerpt: '팀장이 「9월 3일 모델이 제일 좋았는데 다시 만들어 볼 수 있어요?」라고 묻습니다. 같은 커밋으로 학습을 돌렸는데 COPC 0.98 이 나오지 않습니다. 모델 v412 는 코드 a3f9c21, 데이터 파티션 30개, 설정 네 줄, 학습 이미지 1.14 가 학습을 한 번 지나 나온 결과이고, 넷 중 하나라도 다르면 다른 모델입니다. 오늘 다시 만들면 8월 20일 파티션이 백필로 다시 써졌고 라이브러리가 2.3 에서 2.4 로 올라갔고 시드가 없어서 다른 값이 나옵니다. 레지스트리에 재료 넷과 지표와 상태를 적어 두면 그 질문에 30초 안에 답하고, 적어 둔 대로 맞추면 같은 값이 나옵니다.',
    date: '2026-09-26',
    categories: [
      'ML Infrastructure'
    ],
    ...
    contentUrl: 'posts/model-versioning-reproducibility.md',
    series: 'mlops-track'
  },
  {
    id: 'training-pipeline-dag',
    world: 'both',
    worldNote: '일곱 칸의 순서와 재실행, 백필, 멱등은 두 무대가 같습니다. 다른 것은 「어제 로그가 다 왔다」가 성립하는 시각입니다. 담장 안은 클릭이 2분 안에 99.9% 와서 04:00 에 어제가 닫히고, 열린 RTB 는 전환이 광고주 서버에서 며칠 뒤에 와 어제 파티션이 며칠에 걸쳐 다시 써집니다.',
    worldPractical: '담장 안에서는 노출과 클릭이 자기 로그라 04:00 의 첫 칸이 보는 파티션 24개가 곧 확정된 어제이고, 백필은 조인이나 피처 코드를 고쳤을 때만 하는 예외입니다. 열린 RTB 에서는 전환 포스트백이 며칠에 걸쳐 오므로 며칠 전 파티션을 매일 다시 만드는 것이 일상이고, 그래서 덮어쓰기(멱등)가 선택이 아니라 전제가 됩니다. 라벨이 전환인 pCVR 모델은 담장 안에서도 그저께 파티션으로 학습하거나 아직 안 온 전환을 보정합니다.',
    title: '학습 파이프라인: 새벽 4시에 모델이 다시 만들어지는 길',
    excerpt: '월요일 05:12 에 「pctr_daily 실패 — build_features」 한 줄이 옵니다. 새벽에 도는 것은 매일 04:00 에 시작하는 일곱 칸입니다. 로그가 다 왔는지 보고, 라벨과 피처를 만들고, 55분 학습하고, 게이트를 지나 06:09 에 새 모델을 내놓습니다. 셋째 칸이 멈추면 처음부터가 아니라 그 칸부터 다시 돌려 1시간 40분이면 끝납니다. 조인 조건을 고쳐 지난 30일을 다시 만들면 한 날 63분씩 여섯 날을 동시에 돌려 5시간 15분입니다. 그 둘이 안전하려면 같은 칸을 두 번 돌려도 결과가 같아야 하고, 덧붙이기로 쓰면 재실행 때 2,485만 행이 4,970만 행이 됩니다.',
    date: '2026-09-25',
    categories: [
      'ML Infrastructure'
    ],
    ...
    contentUrl: 'posts/training-pipeline-dag.md',
    series: 'mlops-track'
  },
  {
  ...
  },
  {
    ...
    excerpt: '지훈 씨가 여덟째 주에 옆 데이터 플랫폼 팀 저장소를 열어 봅니다. 파일이 수백 개라 어디부터 읽을지 몰라서, 가장 최근에 온 사람 한 명의 커밋을 달별로 늘어놓았습니다. 첫 달은 수급 DAG 한 장, 둘째 달은 SQL 모델, 셋째 달은 실시간 잡 하나, 여섯 달이 지나서는 배포 자동화입니다. 폴더가 곧 일감의 종류이고 순서가 곧 배우는 순서입니다. 그 사이사이에 「리뷰 반영」 커밋이 끼어 있고, 저장소에 pandas 와 PyTorch 는 없습니다.',
    date: '2026-09-20',
  ...
  },
  {
    ...
    excerpt: '지훈 씨가 만든 API 가 받은 클릭 한 건이 네 곳에 가 닿습니다 — 대시보드는 2초, 예산 소진 확인은 5초, 광고주 리포트는 5분, 모델 학습은 다음 날 새벽입니다. 그 사이에 자리가 여섯인데, 넘김 다섯 중 셋은 앞에서 밀고 둘은 뒤에서 가지러 갑니다. 앱이 만든 85바이트에는 과금액이 없고(앱이 보낸 값은 고칠 수 있기 때문입니다), 웹서버가 98바이트를 앞에 붙이며, 수집 에이전트는 내용을 한 글자도 안 건드린 채 봉투에만 셋을 적습니다. Kafka 가 며칠 들고 있는 건 마지막 넘김이 "가지러 가는" 방식이기 때문이고, 3시간 멈춘 소비자는 2.4분에 따라잡지만 10일 멈추면 앞 72시간치는 이미 없습니다.',
    date: '2026-08-16',
    categories: ['Software Engineering', 'ML Infrastructure'],
  ...
  },
  {
    ...
    title: 'ML 모델링 면접: 광고 모델 질문 스물여덟 개와 답하는 순서',
  ...
  },
  {
    ...
    world: 'na',
    title: '해시와 정렬: 20만 개에서 찾기가 7초에서 0.4밀리초가 되는 이유',
  ...
  },
  {
    ...
    world: 'na',
    title: '트리와 힙: 넣는 순서만 달라도 찾는 횟수가 두 배 되는 이유',
  ...
  },
  {
    id: 'api-basics',
    world: 'na',
    title: 'API 기초: 요청과 응답, 메서드 다섯, 로그가 남는 두 자리',
    excerpt: '신입 지훈 씨가 첫 주에 막힌 자리부터 시작합니다. POST 는 "수정"이 아니라 "이걸 처리해 주세요"이고, HTTP 는 요청 하나에 응답 하나가 반드시 짝이라 메서드가 무엇이든 답은 옵니다. 실제 요청·응답 원문 셋, 주소 짓는 규칙, 내부·파트너·공개 세 등급, 비밀번호를 가리면서 남기는 로그 한 줄. 그리고 클릭 한 번에 서버에 남는 두 줄 — nginx 가 저절로 남기는 접속 로그와 앱이 보내야만 남는 이벤트 로그를 실물로 나란히 놓고 봅니다.',
    date: '2026-08-15',
  ...
  },
  {
    ...
    world: 'na',
    title: 'API 연동 실무: 상태코드 · 목록 나눠 받기 · 버전 호환 · 인증',
    excerpt: '지훈 씨가 둘째 주에 남의 회사 서버와 처음 붙습니다. 코드는 30분이면 쓰는데 막히는 것은 그다음입니다 — 답이 안 와서 다시 보내면 그 전환이 한 건인지 두 건인지를 문서가 말해 주지 않습니다. 요청마다 번호를 안 붙이면 전환 1,000건이 1,180건으로 부풀어 전환당 광고비가 ₩5,000에서 ₩4,237로 싸 보이고, 그 숫자로 예산이 움직입니다. 상태코드, 5만 행을 커서로 나눠 받기, 필드를 지우기 전에 세어 보는 법, 다시 보내는 시각을 흩는 이유, 인증 셋을 실제 요청과 응답으로 봅니다.',
    date: '2026-08-14',
    categories: ['Software Engineering'],
  ...
  },
  {
    ...
    categories: ['ML Infrastructure', 'Software Engineering'],
    tags: ['ML Infra', 'Kafka', 'Event-Driven', 'System Design'],
    contentUrl: 'posts/data-distribution-layer.md',
    series: 'engineering-foundations'
  },
  {
    id: 'offline-metrics-gauc',
    world: 'both',
    worldNote: '순위 점수(AUC)·요청 안 순위 점수(GAUC)·정규화 엔트로피(NE)·예측 대비 실제(COPC)의 정의와 계산은 두 무대가 똑같다. 다른 건 점수가 틀렸을 때 어디가 아픈지다. 열린 RTB 는 예측 대비 실제의 오차가 곧 입찰가 오차라 경매에서 지는 것으로 바로 이어지고, 닫힌 생태계는 경매를 직접 돌리는 대신 클릭당 과금 물량과 노출당 과금 물량을 한 줄에 세우는 자리에서 같은 오차가 드러난다.',
    worldPractical: '닫힌 생태계에서는 요청·랭킹·노출·클릭 로그가 한 회사에 다 남아, 요청 안 순위 점수(GAUC)를 요청 id 로 바로 묶어 계산할 수 있다. 무작위 노출 트래픽을 1% 떼어 치우치지 않은 평가 표본을 따로 쌓는 것도 우리가 정하면 되는 일이다. 대신 지면과 과금 상품이 섞여 있어, 예측 대비 실제(COPC)를 지면별·광고별로 쪼개 보는 일이 실무의 절반을 차지한다. 열린 RTB 의 수요 쪽 플랫폼(DSP)은 이긴 노출만 로그에 남아 평가 표본이 애초에 승자 쪽으로 기울어 있다. 요청 id 도 거래소가 준 것이라 같은 요청 안 후보를 우리 쪽에서 다 보지 못해 그룹 정의부터 흔들린다. 그래서 예측 대비 실제와 정규화 엔트로피(NE)를 먼저 승인 기준에 걸고, 순위 점수는 무작위 입찰로 따로 모은 표본에서만 믿는 편이 안전하다.',
    title: 'GAUC 와 오프라인 지표: AUC 가 올라도 매출이 그대로인 이유',
    excerpt: '배포 전 점수가 0.7412 에서 0.7434 로 올랐는데 A/B 매출은 +0.1%, 범위는 -0.4% 에서 +0.6% 로 제자리였던 배포를 점수 다섯 개로 다시 읽습니다. 노출 서른 건을 한 통에 섞어 재면 0.7778 이지만, 짝을 같은 요청 안으로 제한하면 0.6000 입니다. 짝 144개 중 124개가 요청을 가로지른 짝이라, 경매가 쓰지도 않는 것을 재고 있었기 때문입니다. 확률 벌점만 보면 검색(0.05418)이 뉴스피드(0.15296)보다 세 배 좋아 보이지만, 기저엔트로피로 나누면 0.9788 대 0.8989 로 순서가 뒤집힙니다. 예측에 3을 곱하면 순위 점수는 소수점까지 그대로인데 정규화 엔트로피는 1.2093 이 되니, 승인을 취소하고 중단해야 하는 배포입니다.',
    date: '2026-08-14',
  ...
  },
  {
    ...
    world: 'both',
    worldNote: '확률의 역수로 세기(IPS)·가중치 합으로 나누기(SNIPS)·이중 강건(DR)의 식은 두 무대에서 똑같다. 다른 건 성향점수를 누가 쥐고 있느냐다. 담장 안은 경매를 직접 돌리니 탐색 비율과 로그 스키마를 우리가 정한다. 열린 RTB의 DSP는 우리 랭킹 확률만 알고, 경매를 통과할 확률은 거래소 쪽이라 따로 추정해 곱해야 한다.',
    worldPractical: '담장 안에서는 고른 광고의 확률 필드를 노출 로그 스키마에 넣는 것이 우리 결정이고, 탐색 비율도 지면별로 다르게 잡을 수 있다. 대신 광고주와 매체가 둘 다 고객이라, 탐색이 만드는 클릭률 하락을 양쪽에 설명하고 승인을 받아야 한다. 열린 RTB에서는 성향점수가 두 항의 곱이 된다 — 우리 정책이 그 광고를 고를 확률과, 그 입찰이 경매를 통과할 확률이다. 뒤 항은 관측되지 않아 모델로 추정하므로 오차가 실린다. 그래서 확률 하나에만 기대는 계산보다, 예측 모델이 받쳐 주는 이중 강건이 유리하다. 전환이 포스트백이나 측정 사업자를 거쳐 늦게 들어오고 일부는 아예 안 오는 것도 열린 RTB 쪽에서 더 크게 작용한다.',
    title: 'Off-Policy Evaluation: 안 띄운 광고를 지난주 로그로 채점하는 법',
    excerpt: '지난주 로그에는 옛 정책이 고른 광고의 결과만 있습니다. 두 정책이 같은 광고를 고른 건만 세면 요청 10,000건 중 2,595건이 남고, 나아진 폭이 진짜 22.5%가 아니라 56.3%로 부풉니다. 겹친 건을 확률의 역수만큼 크게 세면 치우침은 사라지지만, 가중치 80짜리 한 건이 추정치를 1.669%에서 2.469%로 밀어 올립니다. 실제로 값을 받치는 건수는 10,000건이 아니라 339건이고, 클릭 9.5건이 그 숫자를 지탱하는 셈입니다. 가중치 상한 20을 걸면 흔들림은 절반이 되지만 400주 평균이 2.124%로 내려앉아, 가중치 합으로 나누기를 같이 걸어야 그 몫이 대부분 돌아옵니다.',
    date: '2026-08-14',
  ...
  },
  {
    id: 'doubly-robust',
    world: 'both',
    worldNote: '이중 강건의 식은 두 무대에서 같다. 다른 것은 성향점수가 얼마나 미더우냐다. 담장 안은 경매를 직접 돌리니 고른 광고의 확률을 우리가 정확히 남길 수 있다. 열린 RTB의 DSP는 우리 랭킹 확률에 경매 통과 확률을 곱해야 하는데 뒤 항이 모델 추정치라, 예측 모델이 받쳐 주는 이 방법의 값어치가 커진다.',
    worldPractical: '담장 안에서는 성향점수를 직접 남기므로 확률의 역수로 세기만으로도 대개 충분하다. 로깅이 부실했던 지난 기간을 다시 채점할 때만 이 방법을 얹는다. 열린 RTB에서는 성향점수가 구조적으로 부정확하다 — 경매 통과 확률을 관측할 수 없어 모델로 추정해 곱하기 때문이다. 확률 하나에만 기대면 값이 낮게 나와 멀쩡한 캠페인을 중단하게 되므로, pCTR 예측을 두 번째 받침으로 쓰는 편이 안전하다. 어느 무대든 예측 모델은 평가에 쓸 로그로 학습하면 안 되고, 앞 기간 학습·뒤 기간 평가로 기간을 갈라야 한다.',
    title: 'Doubly Robust: 추정 둘을 겹치면 왜 더 튼튼한가',
    excerpt: '지난주 로그로 새 모델을 채점할 때 쓰는 재료는 옛 정책이 그 광고를 고를 확률 하나입니다. 그 확률이 틀리면 추정치가 통째로 기웁니다. 클릭 확률을 예측하는 pCTR 모델로 답을 먼저 깔고 확률의 역수 가중을 예측이 빗나간 몫에만 걸면, 둘 중 하나만 맞아도 값이 진짜에 붙습니다. 성향점수가 틀린 경우 확률의 역수로 세기는 2.442%로 낮게 나오지만 이 방법은 2.797%로 진짜 값 2.807% 옆에 붙습니다. 대신 흔들림은 0.809%에서 0.806%로 거의 그대로이고, 예측 모델을 평가에 쓸 로그로 학습하면 값이 조용히 예측 쪽으로 기웁니다.',
    date: '2026-08-15',
    categories: ['Measurement & Modeling', 'Bandits & Personalization'],
    tags: ['Causal Inference', 'A/B Testing', 'Contextual Bandit', 'Ad Ranking'],
    contentUrl: 'posts/doubly-robust.md',
    series: 'judgment-track'
  },
  {
    ...
    worldPractical: '담장 안에서는 배분 규칙이 우리 코드라 그 광고가 뜰 확률(성향점수)을 추정하지 않고 기록하면 된다. 대신 우리가 안 고른 광고는 그 지면에서 정말로 0번 뜨기 때문에, 탐색 예산을 명시적으로 잡지 않으면 굳음이 그대로 남는다. 그 예산을 0으로 두면 40세대 내내 진짜 1위가 안 뜨는 결과가 그대로 나온다. 열린 RTB의 DSP는 성향점수가 두 겹이다 — 우리가 입찰했을 확률과 그 입찰이 이겼을 확률인데, 뒤쪽은 경매에서 지면 관측되지 않는다. 그래서 같은 역확률 가중(IPS)을 걸어도 담장 안보다 분산이 크고, 탐색 예산을 잡아도 경매에서 지면 그 예산을 쓰지도 못한다. 실무 순서는 양쪽 다 같다 — 탐색으로 성향점수를 0에서 떼어 놓는 것이 먼저고 역확률 가중은 그다음이다.',
    title: '피드백 루프 편향: 어제 띄운 광고만 학습 데이터가 되면 무엇이 굳나',
    excerpt: '광고 여덟 개를 하루 한 번씩 40일 돌려 봅니다. 첫날 500건에서 뽑힌 클릭 수 하나가 그 뒤 39일의 예산을 정해 버립니다. 진짜 클릭률 2.60%인 소재가 첫날 8클릭을 받아 추정값 1.793%에 얼어붙으면, 예산을 받는 소재 넷은 사흘째에 고정되고 그 소재의 누적 노출은 500건에서 멈춥니다. 대신 진짜 1.90%인 소재가 102,500건을 가져가 매출 ₩980,500을 만들고, 리포트만 보면 진짜로 좋았던 쪽이 먼저 중단됩니다. 한 번 굳은 뒤 역확률 가중으로 되돌리려 하면 그 소재가 뜰 확률이 0이라 계산에서 아예 빠지고, 진짜 값 2.0875%가 1.880%로 도로 내려앉는 이유까지 숫자로 봅니다.',
    date: '2026-08-14',
    categories: ['Measurement & Modeling', 'Bandits & Personalization'],
  ...
  {
    id: 'feedback-loop-remedies',
    world: 'both',
    worldNote: '굳음을 푸는 처방 자체는 두 무대가 같다 — 추정값이 낮은 광고에도 노출을 떼어 주는 것뿐이다. 다른 것은 그 노출이 실제로 나가느냐다. 담장 안은 우리가 배분을 직접 결정하니 떼어 준 몫이 그대로 화면에 뜨고, 열린 RTB는 떼어 줬어도 경매에서 지면 그 예산이 쓰이지 않는다.',
    worldPractical: '담장 안에서는 ε 탐색·강제 슬롯의 몫을 배분 코드에 그대로 박아 넣을 수 있어 성향점수를 설계로 확정할 수 있다. 그래서 이 글의 5%·8% 같은 값이 곧 실제 집행 예산이 되고, 측정 정확도가 필요하면 강제 슬롯 쪽 예산을 늘리는 식으로 조절한다. 열린 RTB의 DSP는 탐색으로 지정한 요청도 낙찰돼야 노출이 되므로, 같은 5%를 걸어도 실제로 쌓이는 표본은 낙찰률만큼 줄어든다. 그래서 열린 RTB에서는 탐색 비율을 낙찰률로 나눠 잡거나 입찰가를 함께 올려야 의도한 표본이 모이고, 그만큼 클릭 손해도 커진다. 어느 쪽이든 판단 기준은 같다 — 이미 옳게 굳은 판에서는 탐색이 손해이고, 잘못 굳은 판에서만 값을 한다.',
    title: '피드백 루프 끊기: 굳은 것을 푸는 네 방법과 그 대가',
    excerpt: '밀려난 소재를 다시 띄우려면 하루 노출에서 얼마를 떼어 줘야 하고, 그 몫은 그날 포기하는 매출입니다. ε 탐색·낙관적 초기값·강제 슬롯·역확률 가중 넷을 같은 판에 걸어 보면 앞의 셋은 진짜 1등 소재를 사흘에서 여드레 만에 되살리지만, 역확률 가중은 노출을 한 푼도 안 쓰는 대신 굳음을 풀지도 못합니다. 기본 고리는 40일 동안 클릭 17,350건에 그쳐 최선인 18,831건보다 1,481건, ₩740,500을 흘립니다. 시드 100판으로 보면 진짜 상위 셋을 맞힌 60판에서 ε 탐색은 16건 손해이고 틀린 40판에서는 403건 이득이라, 탐색은 잘못 굳었을 때만 값을 합니다. 강제 슬롯은 클릭으로는 68판에서 지지만 측정의 흔들림을 6할 수준으로 줄여 예산 결정을 받쳐 줍니다.',
    date: '2026-08-15',
    categories: ['Measurement & Modeling', 'Bandits & Personalization'],
    tags: ['pCTR', 'Exploration', 'Online Learning'],
    contentUrl: 'posts/feedback-loop-remedies.md',
    series: 'judgment-track'
  },
  {
    ...
    title: 'Uplift 와 증분 전환: 광고가 진짜 늘린 전환만 세는 법',
  ...
  },
  {
    ...
    title: '프라이버시 신호 손실: 쿠키와 광고 ID가 사라지면 무엇부터 망가지나',
  ...
  },
  {
    ...
    world: 'na',
    title: '로드밸런서 · Ingress · API Gateway: 요청이 서비스까지 가는 길',
    excerpt: '지훈 씨가 매체에 서버 IP 를 알려 주려다 선배에게 붙잡힙니다. "그 주소를 주면 다음 배포 때 매체 요청이 통째로 날아갑니다." 대신 받은 주소와 지훈 씨 서버 사이에 부품이 셋 서 있는데, 넷 다 "요청을 어디로 보낼지 정하는" 일을 합니다. 왜 넷일까요. 서버 한 대에서 시작해 서비스 넷·매체 열 곳까지 늘어나는 동안, 사고가 하나 날 때마다 부품이 하나씩 생기는 순서를 실제 설정 파일로 따라갑니다.',
    date: '2026-08-07',
    categories: ['Software Engineering'],
  ...
  },
  {
    ...
    title: 'CTR 피처 엔지니어링: 해싱 · 시퀀스 · 집계 피처를 만드는 법',
  ...
  },
  {
    ...
    title: '어트리뷰션: 라스트클릭부터 멀티터치까지, 그 전환은 누구 공인가',
  ...
  },
  {
    ...
    title: '헤더비딩: 순서대로 묻지 말고 동시에 물어 매체 수익을 올리는 법',
  ...
  },
  {
    ...
    title: '2등 가격 경매(Second-Price): 이겨도 왜 2등 값만 내나',
  ...
  },
  {
    ...
    title: 'DSP·SSP·Ad Exchange: 각각 무엇을 하는 곳인가',
  ...
  },
  {
    ...
    world: 'na',
    title: '소프트웨어 아키텍처 패턴 여섯: 모놀리식부터 마이크로서비스까지 무엇을 고르나',
  ...
  },
  {
    ...
    world: 'na',
    title: '쿠버네티스 네트워킹: Pod · Service · Ingress 가 각각 무엇을 하나',
  ...
  },
  {
    ...
    title: '인과추론 입문: 상관과 인과가 왜 다른가, 반사실과 교란변수',
  ...
  },
  {
    ...
    title: '이중차분법(DiD): 차이를 두 번 빼서 진짜 효과만 남기는 법',
  ...
  },
  {
    ...
    title: '도구변수(IV): 직접 못 흔드는 원인의 효과를 재는 법',
  ...
  },
  {
    ...
    title: 'A/B 테스트 vs 멀티암드 밴딧: 트래픽을 고정할까 옮길까',
  ...
  },
  {
    ...
    title: '광고 시스템 입문: 생태계 · 경매 · 랭킹 · 측정 전체 지도',
  ...
  },
  {
    ...
    title: '오디언스 세그멘테이션: 누구에게 보여줄지 어떻게 가르나',
  ...
  },
  {
    ...
    title: 'Lookalike Modeling: 전환 유저 100명에서 유사 유저를 넓히는 법',
  ...
  },
  {
    ...
    world: 'na',
    title: 'Git 실무 가이드: fetch · merge · rebase · stash 를 언제 쓰나',
  ...
  },
  {
    ...
    title: '광고 로그 시스템 설계: 수집 · 스키마 · 저장소 · 중복 제거',
  ...
  },
  {
    ...
    title: 'Ad Tech 개발 레이어: 광고 시스템의 어느 층에서 무슨 일을 하나',
  ...
  },
  {
    ...
    title: 'Negative Sampling: 학습 데이터가 처음부터 편향되는 이유',
  ...
  },
  {
    ...
    title: 'Two-Tower Retrieval: 수백만 광고에서 10ms 안에 후보를 추리는 법',
  ...
  },
  {
    ...
    title: 'Multi-Task Learning: pCTR과 pCVR을 같이 배우면 왜 더 좋은가',
  ...
  },
  {
    ...
    title: '탐색과 활용(Exploration & Exploitation): 새 소재에 노출 몇 번을 걸까',
  ...
  },
  {
    ...
    title: 'Deep CTR 모델: LR에서 DIN까지 무엇이 달라졌나',
  ...
  },
  {
    ...
    title: 'Calibration(확률 보정): AUC가 높아도 돈을 잃는 이유',
  ...
  },
  {
    ...
    title: 'Ad Network vs Ad Exchange: 묶어 팔기에서 노출 단위 경매로',
  ...
  },
  {
    ...
    title: 'Position Bias: 위치가 만드는 착각을 걷어내는 법',
  ...
  },
  {
    ...
    title: '서빙 지연 · 처리량과 Grafana: 평균이 멀쩡한데 하루 1,053만 건이 넘긴다',
    excerpt: '모델 팀은 평균 3.49ms 라고 보고하고 API 팀은 하루 1,699만 건이 예산을 넘겼다고 말합니다. 둘 다 맞습니다. 같은 요청도 추론 함수 안에서 재면 예산 초과가 3.72%, API 서버 전체 응답으로 재면 19.42% 입니다. 요청량이 3,000에서 3,300으로 10% 느는 동안 초과는 8.93%에서 35.76%가 되고, 타임아웃 재시도는 2,750 QPS 에서 멈추다 2,800 QPS 에서 서비스를 죽입니다. 서버 2대를 굴려 바꾸는 몇 분 동안에는 초과가 20.87% 입니다. Grafana 쪽도 같습니다 — 기본 버킷을 그대로 쓰면 p99 가 11.19ms 대신 16.43ms 로 뜨고, 서버별 p99 를 평균 내면 19.85ms 가 12.97ms 로 눌립니다. 대시보드 패널 여섯과 알람 쿼리까지 붙였습니다.',
    date: '2026-08-23',
    categories: ['ML Infrastructure', 'Software Engineering'],
  ...
  },
  {
    ...
    title: 'Online Learning 과 지연 피드백: 광고 모델은 왜 매일 낡아지는가',
  ...
  },
  {
    ...
    title: 'Auto-Bidding 과 예산 페이싱: 하루 예산을 언제 얼마씩 쓰나',
  ...
  },
  {
    ...
    title: 'Feature Store: 학습과 서빙이 다른 피처를 쓰면 모델이 조용히 틀린다',
  ...
  },
  {
    ...
    title: 'Walled Garden: 한 회사가 DSP부터 매체까지 다 가지면 무엇이 달라지나',
  ...
  },
  {
    ...
    title: '광고 기술 생태계 지도: 요청 하나가 지나는 21개 모듈',
  ...
  },
  {
    ...
    title: 'Bid Shading: 1st Price 경매에서 얼마를 깎아 부를까',
  ...
  },
  {
    ...
    title: 'Thompson Sampling: 분포에서 뽑아 고른다, 기본형부터 Linear TS까지',
  ...
  },
  {
    ...
    title: 'UCB vs Thompson Sampling: 같은 표에서 왜 다른 소재를 뽑나',
  ...
  },
  {
    ...
    title: 'Disjoint LinUCB: 광고마다 계수를 따로 배우면 무엇이 달라지나',
  ...
  },
  {
    ...
    title: 'Ad Serving Flow: 후보 12만 건이 800건으로 좁혀지는 순서',
  ...
  },
  {
    ...
    title: 'UCB 계열: UCB1이 맥락을 못 보는 자리와 LinUCB가 메우는 자리',
...
const series = {
  ...
  'judgment-track': {
    ...
    posts: ['offline-metrics-gauc', 'offpolicy-evaluation', 'doubly-robust', 'feedback-loop-bias', 'feedback-loop-remedies', 'uplift-incrementality', 'privacy-signal-loss'],
  },
  'kakao-adtech': {
    title: '카카오 광고 사례 트랙',
...
const mlTrack = {
  ...
  stages: [
    ...
    },
    {
      ...
      posts: ['offline-metrics-gauc', 'offpolicy-evaluation', 'doubly-robust', 'feedback-loop-bias', 'feedback-loop-remedies',
