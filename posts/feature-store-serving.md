...
> **골라 읽는 법** — 긴 글입니다(약 33분). 처음부터 다 읽지 않아도 됩니다.
>
> - 전체 조감도만 → 1~2절
> - 세 갈래 파이프라인만 → 3~4절
> - 아키텍처와 신선도 저울만 → 5~6절
...
문제는 이 재료를 **두 번 꺼내 써야** 한다는 것입니다. 한 번은 모델을 가르칠 때(학습), 또 한 번은 실제로 광고를 고를 때(서빙)입니다. 그리고 이 둘이 조금이라도 다르면 모델은 조용히 틀립니다. 시험 문제를 냈던 교과서와 실제 시험지가 다른 상황입니다.
...
예를 들어 봅시다. "이 유저의 지난 30일 구매 횟수"는 하루에 한 번 계산해도 충분합니다. 오늘 아침 값과 지금 값이 거의 같기 때문입니다. 반면 "이 유저가 지금 세션에서 본 광고 수"는 초 단위로 바뀝니다. 어제 계산한 값은 쓸모가 없습니다.

여기서 저울이 하나 생깁니다. **신선하게 만들려면 비싸집니다.** 하루 한 번 배치로 돌리면 큰 계산도 저렴하게 할 수 있습니다. 초 단위로 갱신하려면 스트리밍 인프라를 세우고 계속 돌려야 합니다. 그래서 실무는 **피처를 신선도별로 나눠 각각 다른 파이프라인에 태웁니다.**
...
피처 하나만 놓고 봅니다. "이 유저의 지난 7일 클릭 수"입니다. 학습 때는 배치가 어젯밤 자정 기준으로 계산합니다. 서빙 때는 Redis 카운터가 지금 이 순간 값을 줍니다. **정의는 똑같은데 기준 시점만 다릅니다.**

```python
# 학습과 서빙이 같은 피처를 다르게 계산하면 무슨 일이 생기나.
#
# 상황: "이 유저의 지난 7일 클릭 수"라는 피처 하나.
#   학습 때  — 하루 한 번 배치로 계산. 어젯밤 자정 기준 값이 하루 종일 쓰인다.
#   서빙 때  — Redis 카운터로 실시간 집계. 지금 이 순간 값이다.
# 정의는 같은데 기준 시점이 다르다. 이게 training-serving skew다.

import math

# 유저 10명. (자정 기준 7일 클릭수, 자정 이후 오늘 추가된 클릭수)
users = [
    ("u01",  2, 0),   # 조용한 유저 — 어긋남 없음
    ("u02",  5, 1),
    ("u03",  1, 4),   # 오늘 갑자기 활발해진 유저
    ("u04", 12, 0),
    ("u05",  3, 7),   # 오늘 폭발 — 어긋남 최대
    ("u06",  8, 2),
    ("u07",  0, 3),   # 어제까지 클릭 0 → 배치값은 0
    ("u08", 15, 1),
    ("u09",  6, 0),
    ("u10",  4, 5),
]

# 아주 단순한 로지스틱 모델. 클릭수가 많을수록 pCTR이 오른다고 학습됐다고 하자.
#   logit = -4.0 + 0.22 * clicks_7d
# 계수 0.22는 "클릭수 1회 늘 때 오즈가 약 1.25배" 정도의 세기다.
W0, W_CLICKS = -4.0, 0.22

def pctr(clicks):
    """로지스틱 함수로 확률을 만든다. 1/(1+e^-z)."""
    z = W0 + W_CLICKS * clicks
    return 1.0 / (1.0 + math.exp(-z))

# 입찰가 = pCTR × 클릭당가치(1,000원) × 1000 / 1000  → 노출당 기대수익(원)
CLICK_VALUE = 1000

print("유저   배치값  실제값  학습가정pCTR  서빙실제pCTR   입찰가(배치)  입찰가(실제)   차이")
gap_sum = bid_gap_sum = 0.0
worst = None
for uid, batch_clicks, today_clicks in users:
    real_clicks = batch_clicks + today_clicks   # 서빙 시점의 진짜 값
    p_batch = pctr(batch_clicks)                # 배치값으로 예측하면
    p_real  = pctr(real_clicks)                 # 진짜 값으로 예측하면
    bid_batch = p_batch * CLICK_VALUE
    bid_real  = p_real * CLICK_VALUE
    gap = (p_real - p_batch) * 100              # %p 차이
    gap_sum += abs(gap)
    bid_gap_sum += abs(bid_real - bid_batch)
    if worst is None or abs(gap) > abs(worst[1]):
        worst = (uid, gap, bid_batch, bid_real)
    print(f"{uid}    {batch_clicks:5d}  {real_clicks:5d}    {p_batch*100:8.3f}%    {p_real*100:8.3f}%"
          f"    {bid_batch:9.2f}원  {bid_real:9.2f}원  {gap:+7.3f}%p")

n = len(users)
print()
print(f"평균 절대 오차       {gap_sum/n:.3f}%p")
print(f"평균 입찰가 오차     {bid_gap_sum/n:.2f}원 (노출 1건당)")
print(f"가장 심한 유저       {worst[0]} — pCTR {worst[1]:+.3f}%p, 입찰가 {worst[2]:.2f}원 → {worst[3]:.2f}원")
print()
# 하루 1,000만 노출이면 이 오차가 얼마가 되나
DAILY = 10_000_000
print(f"하루 {DAILY:,}회 노출 기준 입찰가 왜곡 총량 {bid_gap_sum/n*DAILY/100_000_000:.1f}억원")
print("→ 피처 정의가 같아도 '기준 시점'이 다르면 이만큼 어긋난다.")

# 출력:
# 유저   배치값  실제값  학습가정pCTR  서빙실제pCTR   입찰가(배치)  입찰가(실제)   차이
# u01        2      2       2.765%       2.765%        27.65원      27.65원   +0.000%p
# u02        5      6       5.215%       6.416%        52.15원      64.16원   +1.201%p
# u03        1      5       2.231%       5.215%        22.31원      52.15원   +2.984%p
# u04       12     12      20.424%      20.424%       204.24원     204.24원   +0.000%p
# u05        3     10       3.422%      14.185%        34.22원     141.85원  +10.763%p
# u06        8     10       9.622%      14.185%        96.22원     141.85원   +4.564%p
# u07        0      3       1.799%       3.422%        17.99원      34.22원   +1.624%p
# u08       15     16      33.181%      38.225%       331.81원     382.25원   +5.044%p
# u09        6      6       6.416%       6.416%        64.16원      64.16원   +0.000%p
# u10        4      9       4.229%      11.712%        42.29원     117.12원   +7.483%p
#
# 평균 절대 오차       3.366%p
# 평균 입찰가 오차     33.66원 (노출 1건당)
# 가장 심한 유저       u05 — pCTR +10.763%p, 입찰가 34.22원 → 141.85원
#
# 하루 10,000,000회 노출 기준 입찰가 왜곡 총량 3.4억원
# → 피처 정의가 같아도 '기준 시점'이 다르면 이만큼 어긋난다.
```

가상 데이터 10명이지만 읽는 법은 실제와 같습니다.

u01·u04·u09를 보세요. 오늘 클릭이 없어서 어긋남이 **0**입니다. 조용한 유저에게는 배치값이 그대로 맞습니다. 문제는 u05입니다. 자정 기준 3회였는데 오늘 7회를 더 눌렀습니다. 모델은 3회짜리 유저로 보고 pCTR을 3.4%로 찍는데, 진짜 값을 넣으면 14.2%입니다. **입찰가가 34원에서 142원으로, 4배 차이입니다.**

여기서 중요한 건 어긋남의 방향이 한쪽이라는 점입니다. 배치값은 항상 과거이므로 **활발해진 유저를 항상 과소평가합니다.** 지금 가장 반응이 좋은 유저에게 가장 낮게 입찰하는 셈입니다. 평균만 보면 3.4%p라 작아 보이지만, 그 오차가 하필 가장 값비싼 유저에 몰려 있습니다.
...
## 6. Feature Freshness vs Latency 트레이드오프 [무대: 닫힌 생태계]

담장 안에서는 이 저울이 한쪽으로 유리하게 기울어 있습니다. 로그인 ID가 안정적이라 **유저 피처를 미리 계산해 캐시에 얹어 둘 수 있기 때문입니다.** 요청이 오면 꺼내 쓰기만 하니 조회 시간이 거의 안 듭니다.

열린 RTB의 DSP는 그게 안 됩니다. 쿠키가 매번 달라 미리 구운 피처를 찾을 **키 자체가 흔들립니다.** 그래서 요청 시점에 만들어야 하는 피처가 훨씬 많고, 100밀리초 예산이 더 빡빡합니다. 같은 신선도를 원하면 담장 안보다 비싸게 사야 합니다.

아래 트레이드오프는 양쪽 다 적용되지만, 출발선이 다릅니다.
