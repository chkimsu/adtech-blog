...
const posts = [
  ...
  },
  {
    ...
    worldPractical: '담장 안에서는 로그인 ID로 씨앗을 결정론적으로 붙일 수 있어, 같은 확장 비율에서도 정확도가 훨씬 늦게 무너진다. 친구·팔로우 같은 관계 데이터까지 있으면 그래프 확장이 실제로 쓸 만해진다(이 글의 §5가 담장 안 전용인 이유다). 열린 RTB에서는 쿠키·MAID 같은 확률적 신호로 씨앗을 매칭해야 하고 크로스디바이스 정확도가 60~70%에 그친다. 같은 3% 확장이라도 씨앗 자체에 이미 잡음이 섞여 들어가는 셈이다. 그래서 열린 RTB일수록 확장 비율을 보수적으로 잡고, 증분 리프트를 홀드아웃으로 직접 재는 습관이 더 중요해진다.',
    world: ['open-rtb', 'walled-garden'],
  ...
  {
    id: 'ad-log-system',
    worldPractical: '담장 안은 노출·클릭·전환이 모두 자사 서버에 남는다. 스키마를 한 팀이 통제하고 조인 키도 내가 만든 것이라 확실해서, 필드 변경이 사내 합의 한 번으로 끝난다. 열린 RTB는 로그가 SSP·자사 서버·광고주 포스트백으로 흩어져 있어 스키마 합의 자체가 회사 간 협상이 되고, 남이 보내주는 필드가 예고 없이 바뀐다. 그래서 내가 만드는 필드와 남이 주는 필드를 갈라 두고, 남이 주는 쪽은 전부 nullable로 잡는다. Candidate Log를 남길 수 있느냐도 갈린다 — 담장 안은 후보 전체를 자기가 골랐으니 다 로깅할 수 있지만, 열린 RTB의 DSP는 애초에 남이 보내준 요청 안에서만 후보를 안다.',
    world: 'both',
    ...
    excerpt: '로그는 디버깅 기록이 아니라 ML 모델의 학습 데이터입니다. 그래서 로그 설계가 모델 성능의 상한선을 정합니다. 이 글은 그 시스템을 어떻게 설계·운영하는지 — 수집 계층, 스키마 진화, 저장소 선택, 품질 감시 네 가지를 다룹니다. 핵심은 Candidate Log입니다. 노출된 광고만 남기면 모델은 “이미 경쟁력 있는 광고들 사이의 미세한 차이”만 배우고, 랭킹에서 탈락한 광고의 패턴은 영원히 못 배웁니다. 저장 비용이 수십~수백 배 늘어나는 대신 오프라인 replay 평가와 탈락 원인 분석이 가능해집니다. 스키마를 고쳤을 때 어제 로그가 어디서 터지는지도 코드로 직접 읽어 봅니다.',
    date: '2026-04-11',
    categories: ['ML Infrastructure'],
