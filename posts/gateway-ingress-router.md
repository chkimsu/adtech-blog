...
`10.0.3.14` 는 우리 `bidder` 서버의 IP다. 매체는 이 주소로만 요청을 보낸다. 중간에 아무것도 없으니 경로가 짧고, 12ms 예산을 쓰기에도 유리하다. 서비스가 하나뿐인 초기에는 이 구성이 가장 단순하고 가장 빠르다.

매체와 우리는 같은 데이터센터 안에서 전용 회선으로 붙어 있다. 그래서 사설 IP를 그대로 부를 수 있고, TLS 없이도 되고, 12ms라는 빡빡한 예산이 성립한다. 바깥 인터넷을 건너는 열린 RTB 연동이라면 이 숫자가 훨씬 커진다.
...
여기서 "연속 2회"가 핵심이다. 1회 실패로 빼면 네트워크가 한 번 튄 것만으로 멀쩡한 서버가 빠진다. 반대로 5회를 기다리면 이미 죽은 서버에 약 27초 동안 요청이 계속 들어간다. 5초 주기로 다섯 번을 세고 마지막 타임아웃 2초를 더한 값이다. 그 27초치 요청은 전부 실패다. 2회는 그 사이에서 고른 값이고, 최악의 경우 5+5+2 = 12초 만에 빠진다.
...
대부분이지 전부가 아니다. 한 가지가 더 필요하다. L4 LB는 연결 단위로 대상을 고른다. 대상에서 뺀다는 건 "새 연결을 더 보내지 않는다"는 뜻이지, 이미 맺어진 연결을 옮긴다는 뜻이 아니다. 매체는 12ms 예산 때문에 연결을 계속 붙여 두고 쓴다. 그래서 앱이 종료할 때 남은 연결을 스스로 닫아 줘야(graceful shutdown) 비로소 손실이 0이 된다. 이걸 빼먹으면 헬스체크를 아무리 잘 잡아도 배포 때마다 몇백 건이 샌다.
...
여기까지가 LB가 해결한 것이다. 이제 못 하는 것을 봐야 한다.
...
포트를 나누면 되지 않느냐 — 8080은 입찰, 8081은 트래킹으로. 그러면 매체 설정을 다시 고쳐 달라고 부탁해야 한다. 2절에서 막 벗어난 자리로 되돌아가는 것이다.
