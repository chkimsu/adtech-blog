...
일반 ML 서빙은 요청 하나에 예측 하나를 냅니다. 광고 서빙은 요청 하나에 후보 수백 개를 한꺼번에 점수 내야 하고, 그런 요청이 초당 수만 건 들어옵니다.
...
# 아래 수치는 전부 가상값이다. 자리 크기만 맞췄고 사내에서 잰 값이 아니다.
...
읽는 요령은 **고정 비용과 후보 비례 비용을 나눠 보는 것**입니다.
...
그림을 위에서 아래로 읽으면 세 덩어리입니다. 첫째, **로드 밸런서**는 요청을 서버들에 나눠 주는 장치입니다. 죽은 서버를 빼고, 계속 실패하는 서버에는 요청을 보내지 않습니다. 둘째, **모델 서버 군집**은 점수를 매기는 곳입니다. 경량 모델은 CPU에, 무거운 모델은 GPU에, 임베딩 테이블은 메모리 큰 별도 서비스에 나눠 얹습니다. 셋째, **모델 관리 계층**은 어느 버전을 누구에게 보여 줄지 정하는 계층입니다.
...
광고 모델 서빙 장애는 곧 매출 손실입니다. 1분 다운타임 = 수백만 건의 입찰 기회 손실.
...
처방은 둘입니다. 첫째, **fan-out을 줄입니다.** 개별 조회 100번을 배치 조회 1번으로 묶으면 느릴 확률을 한 번만 겪습니다. 둘째, **꼬리를 잘라냅니다.** 조회에 짧은 타임아웃을 걸고 늦으면 기본값을 씁니다.
:::
...
# 같은 캐시를 유저 임베딩에 쓰면 무대에 따라 다르다 (가상 수치)
...
다른 것은 유저 쪽입니다. 로그인 ID가 안정적인 담장 안은 유저 임베딩도 캐시가 먹어 평균 0.453ms인데, 쿠키가 1회성인 열린 RTB는 1.062ms입니다. 같은 하드웨어, 같은 코드인데 2.3배 차이입니다. 그래서 열린 RTB의 DSP는 유저 피처를 적게 쓰고 광고·문맥 피처에 더 기댑니다.
