광고주가 보유한 전환 유저는 1,000명입니다. 이 1,000명과 "비슷한" 유저 10만~100만 명을 찾아 타겟팅하면 어떨까요? 이것이 **Lookalike 모델링(Lookalike Modeling)**의 핵심 질문입니다. 리타겟팅 다음으로 ROI 가 가장 높은 타겟팅 전략입니다. "알려진 좋은 유저(Seed)"에서 출발해 "아직 발견되지 않은 좋은 유저"로 넓히는 일입니다. 개념은 단순합니다. 하지만 구현은 까다롭습니다. 임베딩 공간의 기하학, 분류 모델의 편향, 그래프 전파의 수렴 조건까지 얽혀 있습니다.

씨앗(Seed)는 [오디언스 세그멘테이션](post.html?id=audience-segmentation)에서 만든 세그먼트입니다. [Ad Tech 개발 레이어](post.html?id=adtech-dev-layers)에서는 타겟팅 레이어의 핵심 모듈로 나옵니다. 유사도 계산의 기반인 유저 임베딩은 [Two-Tower 모델(Two-Tower Model)](post.html?id=two-tower-retrieval)이 만듭니다. 이 글은 세 가지 핵심 접근법을 다룹니다. Embedding 유사도, 성향 모델(Propensity Model), 그래프 확장(Graph Expansion)입니다. 성향 모델은 살 것 같은 정도를 점수로 매기는 모델입니다. 씨앗 구성부터 프로덕션 파이프라인까지, 엔지니어 관점에서 전체 스펙트럼을 다룹니다.

---

## 1. 핵심 비교 (Executive Summary)

Lookalike 모델링에는 크게 세 가지 접근법이 있습니다. 각각의 핵심 아이디어, 입력 데이터, 그리고 프로덕션 특성을 먼저 비교합니다.

### 3대 접근법 비교

| 접근법 | 핵심 아이디어 | 입력 데이터 | 확장성 | 정밀도(Precision)@1% | 구현 복잡도 |
|--------|-------------|-----------|--------|-------------|-----------|
| **Embedding 유사도** | 씨앗 중심(Centroid)과 ANN 거리 | 유저 임베딩 (128~256-dim) | 매우 높음 (ANN으로 수억 유저 처리) | 높음 | 중간 |
| **성향 모델** | 씨앗 vs 씨앗 아닌 사람(Non-Seed) 이진 분류 | 유저 피처 전체 (수백~수천 차원) | 높음 (배치 스코어링) | 매우 높음 | 중간 |
| **그래프 확장** | 관계 그래프 라벨 퍼뜨리기(Label Propagation) | 소셜/행동 그래프 | 중간 (그래프 크기 제약) | 높음 | 높음 |

### 멀티 플랫폼 Lookalike 비교

플랫폼마다 Lookalike의 기반 기술과 제어 수준이 크게 다릅니다.

| 플랫폼 | 기술 기반 | 데이터 소스 | 확장(Expansion) 옵션 | 광고주 제어 | 한계 |
|--------|---------|-----------|---------------|-----------|------|
| **Meta (Facebook)** | Social + 관심사 그래프(Interest Graph) + ML | 로그인 기반 cross-platform | 1~10% 슬라이더 | 높음 (소스/비율 선택) | 닫힌 생태계(Walled Garden) 내부만 |
| **Google** | Search/Browse history + ML | Google Account cross-service | 자동 (Performance Max) | 낮음 (자동화 위주) | 유사 오디언스(Similar Audiences) 폐지 |
...
> **실무에서는 단일 접근법보다 조합이 강력합니다.** Embedding 유사도로 후보 풀을 빠르게 만들고, 성향 모델로 순위를 다시 매깁니다. 그러면 속도와 정밀도를 같이 얻습니다. 대형 플랫폼이 공개한 시스템 논문들도 한 방법만 쓰지 않습니다.
...
| **Lookalike 1~3%** | 씨앗과 유사한 신규 유저 | 좁~중간 | 높음 | 낮음 | 씨앗 품질에 의존 |
| **Lookalike 5~10%** | 씨앗과 느슨하게 유사한 유저 | 중간~넓음 | 중간 | 중간 | 시그널 희석 |
| **Interest Targeting** | 특정 관심사 유저 | 넓음 | 낮~중간 | 중간~높음 | 관심사 ≠ 구매 의도 |
| **넓은 타겟팅(Broad Targeting)** | 전체 모집단 | 매우 넓음 | 낮음 | 높음 | 비효율적 지출 |
...
## 2. 씨앗 Audience: Lookalike의 출발점

Lookalike의 성능은 모델 아키텍처보다 **씨앗의 품질**에 더 크게 좌우됩니다. "좋은 유저"를 정확하게 정의하지 못하면, 아무리 정교한 확장 알고리즘을 사용해도 "좋은 유저와 비슷한 유저"를 찾을 수 없습니다. 이 절에서는 씨앗 선정 전략과 품질 평가 방법을 다룹니다.

### 2-1. 씨앗 선정 전략

씨앗을 어떻게 정의하느냐에 따라 Lookalike의 방향이 완전히 달라집니다.

| 씨앗 유형 | 정의 | 장점 | 위험 |
|----------|------|------|------|
| **전체 전환자** | 구매/가입 완료 유저 전체 | 데이터 풍부, 통계적 안정성 | 저가치 전환(쿠폰 사냥꾼 등) 포함 가능 |
| **High-LTV 전환자** | 상위 20% LTV 유저 | 고품질 시그널, 고가치 유저 발굴 | 씨앗 크기 작음 → 오버피팅 위험 |
| **특정 이벤트** | 장바구니 추가, 앱 설치 등 | 캠페인 목적에 특화 | 최종 전환과 괴리 가능 |
| **기존 세그먼트** | RFM Champions, VIP 등급 등 | 이미 검증된 비즈니스 그룹 | 이미 알려진 유저만 (신규 패턴 부재) |

**최적 씨앗 크기**: 최소 1,000명이 필요하며, **Sweet Spot은 1만~10만 명**입니다.

- **너무 작은 씨앗 (< 500명)**: 노이즈에 오버피팅합니다. 소수 유저의 우연한 공통점(예: 특정 시간대 접속)을 패턴으로 학습할 위험이 높습니다.
- **너무 큰 Seed (> 100만 명)**: 시그널이 희석됩니다. "전환 유저"라는 레이블이 너무 다양한 행동 패턴을 포함하면, 중심점(Centroid)는 아무 유저도 대표하지 못하는 "평균의 함정"에 빠집니다.
- **1만~10만 명**: 충분한 통계적 안정성을 확보하면서도, 전환 유저의 핵심 패턴이 살아 있는 구간입니다.

SQL Rule-based 세그먼트, RFM 스코어링, ML 클러스터링이 바로 이 씨앗을 만드는 방법입니다. 셋 다 [오디언스 세그멘테이션](post.html?id=audience-segmentation)에서 다뤘습니다. Lookalike는 세그멘테이션의 결과물을 입력으로 받는 **하류(Downstream) 프로세스**입니다.

> 씨앗 정의에서 가장 흔한 실수는 "전환 유저 전체"를 무조건 씨앗으로 사용하는 것입니다. 프로모션 기간 중 가입한 유저, 1회성 체험 구매 유저, 환불 유저를 포함하면 씨앗의 동질성(Homogeneity)이 급격히 떨어집니다. **전환의 "질"을 기준으로 씨앗을 필터링**하는 것이 첫 번째 최적화입니다.

### 2-2. 씨앗 품질 평가

씨앗을 정의한 후, 확장 전에 반드시 품질을 진단해야 합니다. 품질이 낮은 씨앗을 확장하면 "노이즈의 확대 재생산"이 일어납니다.

**3가지 핵심 진단 지표**:

1. **Homogeneity (동질성)**: 임베딩 공간에서 씨앗 내부 유저 간 평균 cosine similarity. 동질적인 씨앗일수록 중심점이 의미 있는 중심을 나타냅니다.

$$\text{Homogeneity}(S) = \frac{1}{|S|^2} \sum_{i,j \in S} \text{sim}(u_i, u_j)$$

2. **Signal Strength**: 씨앗의 전환율(CVR)과 전체 모집단 전환율의 비율. 이 비율이 높을수록 씨앗이 명확한 "좋은 유저" 시그널을 가지고 있습니다.

$$\text{Signal Strength} = \frac{\text{CVR}_{\text{seed}}}{\text{CVR}_{\text{population}}}$$

3. **피처(Feature) Distribution**: 핵심 피처들이 전체 모집단 대비 더 좁은 분포를 가져야 합니다. 예를 들어, 씨앗 유저의 앱 사용 시간이 전체 대비 분산이 낮다면 → 일관된 행동 패턴이 존재한다는 의미입니다.

| 진단 지표 | Good 씨앗 | Bad 씨앗 | 주요 원인 |
|-----------|-----------|---------|----------|
| Intra-similarity | > 0.3 | < 0.15 | 이질적 유저 혼합 (LTV 편차 큼) |
| CVR Uplift | > 3x | < 1.5x | 시그널 약함 (정의 기준 느슨) |
| Size | 10K ~ 100K | < 500 or > 1M | 너무 작거나 너무 큼 |
| 피처 Variance | 낮음 (집중된 분포) | 높음 (분산 분포) | 정의 기준 불명확 / 프로모션 유저 포함 |
...
**확장(씨앗에서 닮은 사람으로 넓히는 것) 비율 제어**: 유사도 임계값(threshold)을 조정하여 Lookalike 크기를 제어합니다. Threshold를 높이면 → 씨앗과 매우 유사한 소수만 포함 (High Precision, Low Reach). Threshold를 낮추면 → 더 넓은 범위 포함 (Low Precision, High Reach). 이것이 Facebook의 "1~10% 슬라이더"가 안에서 하는 일입니다.

### 3-3. Multi-Centroid (이질적 씨앗 처리)

**문제**: 단순 평균 중심점(Mean Centroid)는 씨앗이 이질적(Heterogeneous)일 때 실패합니다. 예를 들어 쇼핑몰의 전환 유저 씨앗에 "명품 구매자"와 "초특가 사냥꾼"이 같이 들어 있다고 하겠습니다. 이 두 그룹의 임베딩은 벡터 공간에서 아주 다른 자리에 있습니다. 두 그룹의 평균인 중심점은 **어느 그룹에도 속하지 않는 빈 공간**에 놓이게 됩니다.

이것을 "평균의 함정(Averaging Fallacy)"이라 부릅니다. 평균이 의미 있으려면 분포가 단봉(Unimodal)이어야 하는데, 이질적 씨앗은 다봉(Multimodal) 분포를 가집니다.

**해결**: 씨앗 임베딩에 K-Means 클러스터링을 걸어 K개의 서브클러스터를 찾습니다. 그리고 서브클러스터마다 중심점을 잡아 따로 ANN 검색을 돌립니다.
...
각 중심점에서 독립적으로 ANN 검색을 수행한 후, 결과를 합집합하고 중복을 제거합니다. 최종 순위는 모든 중심점 중 최대 유사도를 기준으로 정합니다.

$$\text{score}(u) = \max_{j=1}^{K} \text{sim}(u, c_j)$$

**K 선택**: 보통 K=3~5로 시작합니다. K가 너무 크면 서브클러스터마다 씨앗이 너무 작아져 의미 없는 중심점이 생깁니다. K=1이면 단순 평균 중심점과 같습니다. Silhouette Score로 최적 K를 자동 탐색할 수도 있습니다. 다만 실무에서는 K=3으로 시작해 Lookalike 성과를 보면서 조정하는 쪽이 일반적입니다.
...
2. K-Means로 씨앗을 K개 서브클러스터로 분할하고, 각 중심점을 쿼리로 ANN 검색합니다.
3. 유저별로 모든 중심점에 대한 유사도의 최대값을 저장합니다 (Multi-Centroid의 핵심).
4. 씨앗 유저를 제외하고 유사도 내림차순으로 정렬하여 최종 Lookalike 오디언스를 생성합니다.

출력의 맨 위는 0.9435, 맨 아래는 0.1246입니다. 같은 오디언스 안에서도 뒤로 갈수록 유사도가 이렇게 무너집니다. 6절의 확장 트레이드오프가 바로 이 숫자입니다.

프로덕션에서는 `IndexFlatIP` 대신 `IndexIVFPQ` 등 근사 인덱스를 사용하여 수억 유저 규모에서도 효율적으로 검색합니다. 자세한 인덱스 선택은 [Two-Tower 모델](post.html?id=two-tower-retrieval)의 ANN 절에 있습니다.

### 3-4. Embedding Lookalike의 장단점 정리

**장점**:
- **구현 속도**: Two-Tower 임베딩이 이미 존재하면, 중심점 계산 + ANN 검색만 추가하면 됩니다. 추가 모델 학습 없이 수시간 내에 MVP를 만들 수 있습니다.
- **확장성**: FAISS/ScaNN의 ANN 인덱스 덕분에 수억 유저 규모에서도 밀리초 단위 검색이 가능합니다.
- **해석 가능성**: "유사도 스코어"라는 직관적 메트릭으로 Lookalike의 품질을 설명할 수 있습니다.
- **실시간 가능**: 중심점만 메모리에 올려두면 입찰 요청(Bid Request) 시점에 실시간 유사도 계산이 가능합니다.
...
- **중심점 함정**: 씨앗이 이질적이면 평균 중심점이 무의미해집니다. Multi-Centroid로 완화할 수 있지만 완전한 해결은 아닙니다.
- **비선형 관계 미포착**: cosine similarity는 선형 거리 메트릭이므로, "씨앗과 비선형적으로 유사한" 패턴을 놓칠 수 있습니다.

많은 프로덕션 시스템은 이 한계를 Two-Stage 구조로 우회합니다. Embedding 기반이 **1차 필터(Retrieval)**, 성향(Propensity) 기반이 **2차 정렬(Ranking)**입니다.
...
ANN이 후보를 10M에서 100K로 빠르게 줄입니다. 그다음 성향이 100K에서 50K로 정밀하게 순위를 매깁니다. 속도와 정밀도를 함께 얻는 구조입니다.

---

## 4. 성향 기반 Lookalike

Embedding 기반이 "거리"를 활용한다면, 성향 기반은 "확률"을 활용합니다. "이 유저가 씨앗에 포함될 확률"을 예측하는 이진 분류기(Binary Classifier)를 학습합니다. 그리고 확률이 높은 유저를 Lookalike로 뽑습니다. 대부분의 프로덕션 환경에서 **Embedding보다 높은 정밀도**을 달성하는 것으로 알려져 있습니다.
...
### 4-1. 씨앗 vs 씨앗 아닌 사람 이진 분류

Lookalike를 이진 분류 문제로 재정의합니다.

- **Positive (label = 1)**: 씨앗 유저
...
Embedding 기반과의 핵심 차이는 **입력 데이터의 풍부함**입니다. Embedding 기반은 미리 학습된 고정 차원의 벡터만 씁니다. 성향 기반은 원시 피처를 전부 씁니다. demographics, 행동 집계, 디바이스 정보, 시간대 패턴 같은 것들입니다. 모델이 "무엇이 씨앗과 씨앗 아닌 사람을 나누는지"를 직접 학습합니다. 그래서 임베딩에 포착되지 않은 패턴도 쓸 수 있습니다.
...
- **임베딩**: Two-Tower 유저 임베딩을 피처로 추가 (Embedding + 성향 하이브리드)

[피처 저장소(Feature Store)](post.html?id=feature-store-serving)에서 관리되는 피처들이 여기서 직접 활용됩니다.

### 4-2. 피처 Interaction과 모델 선택

| 모델 | 장점 | 한계 | 적합 상황 |
|------|------|------|----------|
| **Logistic Regression** | 빠름, 해석 가능, 피처 Importance 명확 | 피처 interaction 수동 구성 필요 | 베이스라인, 해석 필요시, 빠른 이터레이션 |
| **XGBoost / LightGBM** | 피처 interaction 자동 학습, 높은 성능, 빠른 학습 | 고차원 임베딩 활용 제한적 | 대부분의 프로덕션 환경 |
| **DNN (DeepFM 등)** | 임베딩 + 피처 동시 학습, 풍부한 high-order interaction | 학습 시간 길고 해석 어려움 | 임베딩 풍부 + 대규모 데이터 |

DeepFM, DCN의 아키텍처가 여기서도 그대로 적용됩니다. 둘은 [Deep CTR Models](post.html?id=deep-ctr-models)에서 다뤘습니다. CTR 예측과 Lookalike의 모델 구조가 유사한 이유는 — 둘 다 "유저 피처 → 이진 확률"을 예측하는 문제이기 때문입니다. 차이는 레이블의 정의(클릭 vs 씨앗 소속)뿐입니다.

다음은 XGBoost 기반 성향 Lookalike의 구현 예시입니다.
...
**코드 설명**:
1. `scale_pos_weight`로 클래스 불균형을 보정합니다 (씨앗이 Negative의 1/10이므로 10배 가중).
2. Validation AUC가 0.8 이상이면 모델이 씨앗과 씨앗 아닌 사람을 잘 구분하고 있다는 의미입니다.
3. AUC가 0.95 이상이면 오히려 의심해야 합니다 — 데이터 누출(Leakage)이거나, 모델이 "이미 전환한 유저"를 외우고 있을 수 있습니다.

### 4-3. Negative Sampling 전략

성향 Lookalike에서 **Negative 세트 구성은 모델 성능을 결정하는 핵심 요소**입니다. "씨앗이 아닌 유저"를 어떻게 샘플링하느냐에 따라, 모델이 학습하는 "차별점"이 완전히 달라집니다.

**3가지 전략 비교**:

| 전략 | 방법 | 장점 | 위험 |
|------|------|------|------|
| **Random Sampling** | 전체 모집단에서 무작위 샘플 | 간단, 편향 없음 | 씨앗과 매우 유사한 유저가 Negative에 포함될 수 있음 |
| **Stratified Sampling** | 인구통계 분포를 유지하며 샘플 | Demographic 편향 방지 | 구현 약간 복잡 |
| **Hard Negative Mining** | 씨앗 근처지만 전환하지 않은 유저 샘플 | 미세한 차이 학습, 높은 정밀도 | 모델이 과도하게 보수적으로 학습할 위험 |

선택 편향(Selection Bias) 문제가 여기서도 똑같이 적용됩니다. [Negative Sampling & Bias](post.html?id=negative-sampling-bias)에서 다룬 그 문제입니다.

> Negative 샘플이 씨앗과 너무 유사하면 모델이 미세한 차이만 학습하고, 너무 이질적이면 피상적 규칙만 학습합니다. 최적의 Negative는 **"씨앗에 가까우면서도 전환하지 않은" 유저**입니다. 실무에서는 Random Sampling으로 시작합니다. 그다음 피처 Importance를 봅니다. Demographic 피처가 상위를 차지하면 Stratified로 바꿉니다. 정밀도가 부족하면 Hard Negative를 추가합니다. 이 단계적 접근이 가장 효과적입니다.

**실무 Tip: Negative Ratio의 영향**

| Positive : Negative 비율 | AUC 경향 | 정밀도@1% 경향 | 학습 시간 |
|------------------------|---------|-------------------|---------|
| 1:3 | 약간 낮음 | 높음 | 빠름 |
| 1:10 | 최적 | 최적 | 중간 |
...
대부분의 경우 **1:10 비율**이 최적 균형점입니다. 비율이 너무 높으면 Negative 안에 씨앗과 유사한 유저가 많이 섞입니다. 잠재적 Lookalike인데 모델은 이들을 "Negative"로 배웁니다. 역설이 생기는 지점입니다.
...
Graph 기반 Lookalike는 유저 간의 **관계(Edge)**를 통해 씨앗의 레이블을 전파합니다. 임베딩이 "개별 유저의 특성"을 포착한다면, 그래프는 "유저 간의 연결 구조"를 포착합니다. 친구 관계, 공통 관심사, 공동 구매 이력 등으로 구성된 그래프에서 씨앗 유저와 가까운 노드를 Lookalike로 선정합니다.
...
**라벨 퍼뜨리기(아는 사람의 표를 이웃에게 퍼뜨리는 방법)**: 씨앗 노드에 레이블 1을 부여하고, 그래프를 따라 반복적으로 전파합니다:

$$f^{(t+1)} = \alpha S f^{(t)} + (1-\alpha) y$$

여기서:
- $S$: 정규화된 인접 행렬 (Normalized Adjacency Matrix)
- $y$: 초기 레이블 벡터 (씨앗 = 1, 나머지 = 0)
- $\alpha$: 전파 강도 (0.5~0.8이 일반적)
- $f^{(t)}$: t번째 반복에서의 각 노드의 Seed-similarity 스코어

수렴 후 $f^*_i$는 노드 $i$가 씨앗과 얼마나 "가까운지"를 나타냅니다. 높은 값을 가진 노드가 Lookalike 후보입니다.
...
User B가 씨앗 1과 씨앗 2 모두와 연결되어 있으므로 f=0.65로 높은 스코어를 받습니다. User D는 Hop 2이지만 A와 B 두 경로를 통해 씨앗과 연결되므로 f=0.41을 받습니다. **다중 경로(Multi-path)**로 연결된 유저는 단일 경로 유저보다 높은 스코어를 받습니다. 이것이 라벨 퍼뜨리기의 핵심 특성입니다.

**Meta(Facebook) Lookalike가 강력한 이유**: Meta는 그래프 셋을 갖고 있습니다. 소셜 그래프(친구 연결), 관심사 그래프(페이지 좋아요, 그룹 멤버십), 인게이지먼트 그래프(포스트 반응, 댓글)입니다. 이 다층적 그래프 구조가 전파 경로를 풍부하게 만들어, 단일 채널 데이터로는 발견할 수 없는 유사 유저를 찾아냅니다. [닫힌 생태계](post.html?id=walled-garden)에서 다룬 것처럼, 이 그래프 데이터는 플랫폼 밖으로 나가지 않습니다. 그래서 Meta의 Lookalike 품질은 외부에서 복제할 수 없습니다.

### 5-2. GNN Embedding + Lookalike

전통적 라벨 퍼뜨리기에는 한계가 있습니다. **Graph Neural Network(GNN)**를 쓰면 그래프 구조와 노드 피처를 함께 쓰는 임베딩을 학습할 수 있습니다.
...
| **정밀도** | 다중 경로 연결 시 높은 정밀도 | 전파 깊이(Hop)가 깊어지면 노이즈 증가 |
...
> Graph 기반 Lookalike의 현실적 장벽은 **"그래프 데이터 확보"**입니다. Meta처럼 소셜 그래프를 직접 가진 플랫폼은 극소수입니다. 나머지는 행동 기반 공동 출현(Co-occurrence) 그래프를 직접 만들어야 합니다. 이 구축 비용이 Embedding/성향보다 높습니다. 그래서 Graph 기반은 **플랫폼 사업자가 아닌 한 ROI가 낮은 접근법**일 수 있습니다.

---

## 6. 확장 비율(Expansion Ratio): Reach vs 정밀도 트레이드오프
...
> 확장을 1%에서 10%로 늘리면 도달(Reach)은 **10배 증가**하지만, 증분 리프트는 **1/6로 감소**합니다. 이것이 Lookalike의 근본적 트레이드오프입니다. "더 넓게 확장하면 더 좋은 것 아닌가?"라는 직관은 틀렸습니다. 확장할수록 씨앗와의 유사성이 급격히 떨어집니다. 결국 넓은 타겟팅과 구분이 없어집니다.

:::deep 더 깊이 — 반경이 1.8%만 늘어도 품질이 무너지는 이유
1% 확장은 중심점에서 반경 $r_1$ 내의 유저, 10% 확장은 반경 $r_{10}$ 내의 유저를 포함합니다. 128차원 공간에서 볼륨은 $r^{128}$에 비례합니다. 그래서 10배 많은 유저를 담으려면 반경은 $10^{1/128} \approx 1.018$배만 늘어나면 됩니다. 하지만 이 작은 반경 증가에도 유사도 분포의 꼬리(tail) 영역 유저가 대량으로 들어옵니다. 그래서 평균 품질이 크게 하락합니다.
:::
...
1% → 성과 확인 → 3% → 성과 확인 → 5%로 점진적으로 확장하면서, 각 단계에서 CPA와 CVR을 모니터링합니다. CPA가 목표치를 초과하는 지점이 해당 캠페인의 **최적 확장 경계**입니다.
...
하나의 확장을 고르는 대신 **여러 확장 레벨을 동시에 운영**하는 방법입니다. 각 레벨에 다른 입찰·예산 전략을 적용합니다.

| 단계(Tier) | 확장 | 입찰 전략 | 일 예산 비중 | 목적 |
|------|-----------|---------|------------|------|
| **단계 1** | 1% | 공격적 (높은 CPC/CPM) | 40% | 핵심 타겟 확보 |
| **단계 2** | 3% (1% 제외) | 보통 | 35% | 확장 + 효율 균형 |
| **단계 3** | 5% (3% 제외) | 보수적 (낮은 CPC/CPM) | 25% | 도달 확장 탐색 |

핵심은 **단계 간 유저 중복을 제거**하는 것입니다. 단계 2에서 단계 1 유저를 제외하고, 단계 3에서 단계 1+2 유저를 제외합니다. 이렇게 하면 동일 유저에게 두 번 입찰하는 카니발리제이션을 방지하고, 각 단계의 순수 Incremental 효과를 측정할 수 있습니다.

이 전략은 Facebook 광고에서 특히 많이 씁니다. 각 단계를 별도 Ad Set으로 만들고, "Custom Audience 제외" 기능으로 중복을 관리합니다.
...
|------|----------------|--------|----------------------|-------------|
| **기술 기반** | Social Graph + 관심사 그래프 + ML 앙상블 | Search/Browse history + ML | 3rd-party data + ML | 1st-party 통합 데이터 + ML |
...
| **확장 옵션** | 1~10% 슬라이더, 국가별 설정 | 자동 (Performance Max 내 자동 확장) | 커스텀 비율 설정, 유사도 임계값 조정 | 플랫폼 제공 사전 정의 옵션 |
...
| **한계** | 닫힌 생태계 내부 활성화만 가능 | 유사 오디언스 폐지, 자동화 위주 | Cookie 차단과 ATT로 데이터 품질 하락 | 각 플랫폼 생태계 내부로 제한 |

닫힌 생태계 플랫폼은 풍부한 1st-party 로그인 데이터를 갖고 있습니다. Meta, Google, 네이버, 카카오입니다. 그래서 본질적으로 더 높은 품질의 Lookalike를 제공합니다. 자세한 배경은 [닫힌 생태계](post.html?id=walled-garden)에 있습니다. Open Web DSP는 3rd-party cookie 의존도가 높습니다. 데이터 품질에서 구조적으로 열위이고, Safari/Firefox의 cookie 차단과 iOS ATT로 격차가 더 벌어졌습니다.
...
> Google은 2023년 **유사 오디언스를 폐지**하고 Performance Max의 자동 타겟팅으로 대체했습니다. 이는 Lookalike의 "수동 제어"가 "알고리즘 자동화"로 전환되는 업계 트렌드를 반영합니다. 광고주가 직접 확장 비율을 조절하는 대신, 플랫폼 알고리즘이 캠페인 목표(전환, ROAS)에 맞춰 자동으로 오디언스를 확장합니다. 이 전환은 광고주의 제어권을 축소하지만, 동시에 "잘못된 확장 설정"으로 인한 성능 저하를 방지하는 효과도 있습니다.
...
1. **유령 광고(Ghost Ad) (Gold Standard)**: Lookalike 오디언스를 Treatment와 대조군(Control)로 랜덤 분할합니다. Treatment는 실제 광고 노출, 대조군은 "광고가 노출될 수 있었던 시점"을 기록만 하고 실제 노출은 하지 않습니다. 두 그룹의 전환율 차이가 순수한 Incremental 효과입니다.

2. **PSA (Public Service Announcement) 대조군**: 대조군 그룹에 공익 광고를 노출합니다. 실제 광고 노출 그룹과의 전환율 차이를 측정합니다. 유령 광고보다 구현이 쉽지만, PSA 광고 자체가 약간의 영향을 줄 수 있습니다.

3. **Geo-holdout**: 특정 지역은 Lookalike 타겟팅을 끄고, 나머지 지역은 켠 상태에서 비교합니다. 개인 수준 실험이 어려울 때의 대안입니다.

**좋은 증분 리프트 기준**:
- Lookalike 1%: 증분 리프트 > 100%가 일반적
- Lookalike 5%: 증분 리프트 > 30%이면 양호
- Lookalike 10%: 증분 리프트 > 10%이면 양호
- Lift ≤ 0%: Lookalike가 효과 없음 → 씨앗 재정의 또는 접근법 변경 필요

**Lift 측정 시 주의할 함정들**:

| 함정 | 현상 | 원인 | 해결 |
|------|------|------|------|
| Selection Bias | Lookalike CVR이 높지만 Lift는 낮음 | 원래 전환할 유저를 타겟팅 | 유령 광고 / PSA 대조군으로 순수 Lift 측정 |
...
| Contamination | 대조군 그룹도 높은 CVR | 대조군 유저가 다른 채널에서 노출 | Cross-channel holdout, Geo-level 실험 |
...
|----------|--------------|----------|------|
| Lookalike vs Retargeting | < 15% | > 30% | 씨앗에서 기존 전환자 제외, Retargeting 유저 Blocklist 적용 |
| Lookalike vs 넓은 타겟팅 | < 40% | > 60% | 확장 비율 축소, 씨앗 특화 |
...
대부분의 Lookalike 시스템은 배치 파이프라인으로 운영됩니다. 일 1회~주 1회 주기로 씨앗을 갱신하고, 임베딩/모델을 업데이트하며, Lookalike 오디언스 목록을 재생성합니다.
...
**씨앗 추출 SQL 예시** — 최근 30일 구매 상위 20% LTV 유저:
...
이 SQL의 핵심 설계 의도:
- `HAVING COUNT(*) >= 2`: 1회성 구매자 제외 → 씨앗 품질 향상
- `NTILE(5) ... ltv_quintile = 1`: 상위 20% LTV 유저만 선택 → High-value 씨앗
...
일부 시스템에서는 입찰 요청 시점에 실시간으로 Lookalike 스코어를 계산합니다. 배치 방식보다 최신 데이터를 반영할 수 있지만, 인프라 비용이 높습니다.

**실시간 경로**:
1. 입찰 요청 수신 → 유저 ID 추출
2. [피처 저장소](post.html?id=feature-store-serving)에서 실시간 피처 조회
3. [Two-Tower](post.html?id=two-tower-retrieval) User Tower로 유저 임베딩 실시간 계산
4. 사전 로드된 씨앗 중심점과 cosine similarity 계산
...
| **구현 복잡도** | 중간 (배치 파이프라인) | 높음 (피처 저장소 + 모델 서빙) |

**실무 권장**: 대부분의 캠페인은 배치로 충분합니다. Lookalike 오디언스의 "유저가 누구인가"는 시간 단위로 크게 변하지 않기 때문입니다. Real-Time은 "직전 행동에 따라 동적으로 타겟팅을 결정"해야 하는 특수한 경우(예: 방금 앱을 설치한 유저를 즉시 업셀 타겟에 포함)에 한정하여 도입하는 것이 비용 효율적입니다.

### 9-3. 운영: 갱신 주기, 모니터링, 씨앗 Refresh

Lookalike는 "한 번 만들고 끝"이 아닙니다. 씨앗은 시간이 지나면 stale해지고, 유저 행동 패턴은 변화하며, 시즌/이벤트에 따라 전환 유저의 특성이 달라집니다. 지속적인 운영 없이는 Lookalike 성능이 점진적으로 하락합니다.

**운영 항목 및 권장 주기**:

| 운영 항목 | 권장 주기 | 경고 임계값 | 조치 |
|----------|---------|-----------|------|
| **씨앗 갱신** | 일 1회 | 씨앗 size ±30% 급변 | 전환 데이터 이상 확인, 프로모션 영향 필터링 |
...
실무에서는 Airflow(작업 순서를 짜고 돌리는 도구)/Prefect 같은 도구로 배치 파이프라인을 스케줄링합니다. 각 단계의 메트릭은 DataDog/Prometheus에 기록합니다. 임계값을 넘으면 Slack/PagerDuty로 알려 빠르게 대응합니다.

**일반적인 장애 시나리오와 대응**:

| 장애 | 증상 | 근본 원인 | 대응 |
|------|------|---------|------|
| 씨앗 급감 | Audience 크기 50%↓ | 전환 추적 코드 장애, ETL 실패 | 이전 빌드 롤백, 전환 파이프라인 점검 |
| Similarity 급락 | Avg sim 0.1↓ | 임베딩 모델 배포 오류, 피처 드리프트 | 임베딩 버전 확인, 이전 인덱스 롤백 |
| AUC 급락 | 성향 AUC 0.6 | 피처 스키마 변경, 데이터 누락 | 피처 저장소 점검, 피처 값 분포 확인 |
| CVR Lift 소멸 | Lookalike CVR ≈ Broad CVR | 씨앗 품질 저하, 시장 변화 | 씨앗 재정의, 전환 기준 강화 |

**A/B 테스트 프레임워크**:

Lookalike 모델이나 씨앗 정의를 변경할 때는 반드시 A/B 테스트로 검증합니다.

- **Treatment**: 새로운 Lookalike 모델/씨앗
- **대조군**: 현재 Lookalike 모델/씨앗
...
Lookalike 모델링은 "알려진 좋은 유저"에서 "아직 모르는 좋은 유저"로의 확장 문제입니다. 이 글에서 다룬 핵심 내용을 5가지로 정리합니다.

1. **Embedding 기반은 빠르고, 성향 기반은 정밀하다** — 첫 버전은 Two-Tower 임베딩 + 중심점 ANN으로 빠르게 론칭하고, 성능 개선이 필요하면 성향 모델(XGBoost/DNN)로 전환합니다. 두 접근법을 결합하면 속도와 정밀도를 동시에 달성할 수 있습니다.

2. **씨앗 품질이 Lookalike 성능의 대부분을 결정한다** — 모델 아키텍처 튜닝보다 씨앗 정의를 먼저 최적화해야 합니다. Intra-similarity > 0.3, CVR Uplift > 3x, Size 10K~100K가 Good 씨앗의 기준입니다. "전환 유저 전체"가 아니라 "가치 있는 전환 유저"를 씨앗으로 정의하는 것이 첫 번째 최적화입니다.

3. **Multi-Centroid는 이질적 씨앗의 가장 실용적 해법이다** — 단순 평균 중심점은 다봉 분포에서 실패합니다. K-Means로 씨앗을 K=3~5 서브클러스터로 분할하고 각각 ANN 검색하는 Multi-Centroid 방식이 구현 대비 효과가 가장 높습니다.

4. **확장 비율은 "높을수록 좋다"가 아니라 "목표에 맞는 최적점"이다** — 퍼포먼스 캠페인은 1~3%, 브랜드 캠페인은 5~10%. 점진적 확장(1% → 3% → 5%)으로 최적점을 탐색하고, CPA가 목표를 초과하는 지점에서 멈춰야 합니다.

5. **Lookalike는 "만들고 끝"이 아니다** — 씨앗 Refresh(일 1회), Overlap 모니터링(vs 리타겟팅 < 15%), Incremental Lift 측정(Ghost Ad / PSA Control)이 지속적으로 필요합니다. 갱신 없이 30일이 지나면 CVR이 30% 이상 하락할 수 있습니다.

> Lookalike 모델링의 본질은 "알려진 좋은 유저"에서 "아직 모르는 좋은 유저"로의 확장입니다. 확장의 품질은 세 가지가 함께 결정합니다. 첫째 [세그멘테이션](post.html?id=audience-segmentation)의 정밀도. 둘째 [임베딩](post.html?id=two-tower-retrieval)의 표현력. 셋째 확장 비율의 절제입니다. 셋 중 하나라도 부족하면 Lookalike는 비싼 넓은 타겟팅이 됩니다. "비슷한 유저"가 아니라 "그냥 아무 유저"를 타겟팅하게 됩니다.
