...
노출은 두 조건 중 먼저 오는 쪽에서 보낸다. 큐에 10건이 차거나, 마지막 전송에서 5초가 지나거나다. 노출은 한 건이 빠져도 다음 건이 곧 온다. 클릭은 한 건이 정산과 학습 라벨에 그대로 들어간다. 같은 SDK 안에서도 한 건을 잃었을 때 잃는 것이 이벤트마다 다르다.
...
먼저 이 글이 형제 글과 어긋나 보이는 자리를 짚어야 한다. [Kafka는 왜 있나](post.html?id=kafka-log-pipeline) 1절 ②는 "파일에 쓰고 나중에 옮긴다"를 유실 사례로 지적했다. 오토스케일이 30대를 20대로 줄이는 순간 132,000줄이 사라진다는 계산까지 붙였다. 그런데 그 글이 문제 삼은 것은 파일이 아니라 5분이다. 계산의 전제가 5분마다 파일을 공용 저장소로 옮기는 배치이고, 사라지는 자리도 "옮기기 전"이라고 그 글이 못 박았다. 그리고 같은 절에 이런 문장이 있다. "파일을 계속 따라 읽는 에이전트를 붙이면 5분이 몇 초로 줄어든다. 맞다."
...
PER_SEC = ROWS_DAY / 86_400               # 수집 서버가 초당 받는 이벤트
...
```text
10.2.31.7 2026-08-06T16:08:26+09:00 POST /v1/e 204 0.004 "MyApp/3.2.1 (iPhone; iOS 19.2)" {"t":"imp","e":[{"rid":"r-8f21","ad":9931,"s":"main_top","ts":1786000101118,"seq":11},{"rid":"r-9d55","ad":8820,"s":"feed_2","ts":1786000101173,"seq":12},{"rid":"r-4a17","ad":7715,"s":"feed_7","ts":1786000101241,"seq":13},{"rid":"r-2e93","ad":6604,"s":"feed_12","ts":1786000101305,"seq":14}]}
```

`e` 배열에 노출 4건이 들어 있다. **첫 건의 `rid` 가 `r-8f21` 이다.** 40분 뒤 눌리는 그 노출이다. 나머지 셋은 같은 화면의 다른 지면이라 `rid` 가 다르다. `rid` 는 광고 요청 하나에 하나씩 붙는 번호이고 지면마다 요청이 따로 나간다. 그래서 배치 바깥이 아니라 건마다 들어 있다.

네 건의 `ts` 는 16:08:21 안에 있는데 서버가 받은 시각은 16:08:26 이다. 5초가 뜬 것은 2절의 타이머 때문이다.
...
`script` 는 코드가 든 파일이고 `call` 은 그중 부를 함수 이름이다. 쪼개는 코드는 우리가 쓴다.

`split_events` 는 `body` 를 열어 `e` 배열을 꺼낸다. 원소 하나마다 레코드 하나를 만들어 되돌려 준다. `rid` 는 배열 안에 건마다 따로 들어 있으니 그대로 따라 나간다. 클릭 줄은 `e` 가 없으니 한 건 그대로 지나간다.
...
{"remote":"10.2.31.7","ts":"2026-08-06T16:08:26+09:00","method":"POST",
 ...
 "agent":"MyApp/3.2.1 (iPhone; iOS 19.2)",
 "body":"{\"t\":\"imp\",\"rid\":\"r-8f21\",\"ad\":9931,\"s\":\"main_top\",\"ts\":1786000101118,\"seq\":11}"}
```

모양이 클릭 줄과 똑같다. `body` 는 여전히 문자열 하나다. 다른 것은 안에 든 것이 배열이 아니라 한 건이라는 점뿐이다.

`rid`+`seq` 가 `r-8f21`+`11` 이다. 이 글의 클릭은 `r-8f21`+`47` 이다. `rid` 는 같고 `seq` 가 다르다. 1절이 만든 멱등키가 쪼갠 뒤에도 건마다 유일하다는 뜻이고, 동시에 두 줄이 같은 `rid` 로 이어 붙는다는 뜻이다. 배열을 펼치면서 `seq` 를 새로 매겼다면 둘 다 깨진다. 재전송된 배치가 새 키를 얻고 2절의 중복 제거가 무너진다.

**클릭과 노출은 다른 topic 으로 간다.** 그래서 `t` 값을 보고 태그를 갈아 주는 필터를 하나 더 건다. 그다음 `[OUTPUT]` 을 태그마다 하나씩 둔다. 위 블록은 클릭 쪽만 남긴 것이라 `Match *` 로 뒀고, 태그를 가른 뒤에는 그 자리가 `Match ad.click` 으로 좁아진다.

여기서 나가는 노출 줄의 이름은 `ad.impression.confirm` 이다. `ad.impression` 이 아니다. 이유는 [Kafka는 왜 있나](post.html?id=kafka-log-pipeline) 도입에 있다. 그 글에서 `ad.impression` 한 줄의 뼈대를 만드는 것은 `bidder` 다. `pctr` 0.0213 과 `bid` 182.4 는 응찰을 계산한 그 프로세스 안에만 있는 값이라 다른 데서 만들 수 없다. 지금 이 홉을 지나는 줄에는 그런 값이 없다. 여기 있는 것은 매체가 "그 광고가 화면에 떴다"고 보낸 **확인**이다. 다른 곳에서 만든 다른 줄이다.

이름을 가르는 이유는 취향이 아니라 계산이다. 형제 글은 `ad.impression` 을 producer 하나짜리 topic 으로 놓고 하루 2억 2,800만 줄과 보존 용량을 잰다. 같은 이름에 확인 줄을 또 넣으면 그 topic 이 하루 4억 5,600만 줄이 된다. 형제 글이 그 숫자로 잡아 둔 보존 기간과 디스크가 통째로 어긋난다. 넣는 곳이 둘이면 topic 도 둘이어야 한다.

대신 두 topic 은 조건 셋을 같이 맞춰야 한다. key 를 둘 다 `req_id` 로 써야 한다. partition 수도 둘 다 12여야 한다. 보내는 쪽이 같은 partitioner 를 쓰는 것이 셋째다. 형제 글 7절이 짚은 함정이 여기에 그대로 걸린다. `ad.impression` 은 `bidder` 가 넣고 이 확인 줄은 우리 에이전트가 넣는다. 서비스가 둘이면 클라이언트도 둘일 수 있다. 자바의 murmur2 와 librdkafka 의 CRC32 는 같은 `r-8f21` 을 다른 번호로 보낸다. 셋이 다 맞을 때만 응찰 줄과 확인 줄이 같은 partition 에 놓인다. 그래야 둘을 `req_id` 로 이어 붙이는 일이 partition 하나 안에서 끝난다.
...
**어디까지 읽었나를 적어 두는 파일이 있다.** 설정의 `DB /var/lib/fluent-bit/pos.db` 다. 이름은 파일이지만 SQLite 라 `cat` 으로는 안 열린다. 안의 한 행을 꺼내 적으면 이렇다.

```text
/var/log/collect/e-2026080616.log   inode=8391204   offset=48,271,104
```

읽는 파일 하나에 한 행이다. `offset` 은 그 파일에서 몇 바이트째까지 읽었는지다. 에이전트가 재시작하면 이 값부터 이어 읽는다. 이 값이 없으면 처음부터 다시 읽거나 끝으로 건너뛴다. 앞은 중복이고 뒤는 유실이다.
...
다만 숫자가 생기는 자리는 표가 적은 자리와 조금 다르다. 이 줄에서 `9931` 을 정수로 만드는 것은 `int()` 가 아니라 `body` 를 여는 순간이다. 앱이 `"ad":9931` 로 따옴표 없이 실어 보냈고 JSON 파서가 그대로 숫자로 읽는다. 코드의 `int()` 는 이 줄에서는 아무 일도 안 한다. 값을 내는 것은 3절이 예고한 그 경우다 — 앱이 `"ad":"9931"` 로 잘못 실어 보낸 줄이다. 3절은 그런 줄이 와도 수집기가 아무 말을 안 한다고 했고, 여기가 그것을 정수로 되돌리는 자리다.

끝까지 문자열로 남는 것은 서버가 앞에 붙인 90 B 쪽이다. `status` 와 `rt` 둘이다. 그 둘은 타입을 붙이는 대신 지운다.
...
offset    8412
timestamp 1786002502310   type=CreateTime
```
...
`timestamp` 1786002502310 은 6절의 `process_ts` 와 같은 값이다. 16:48:22.310 이다. `type=CreateTime` 이라 그렇다. producer 가 `send()` 를 부른 시각을 브로커가 그대로 적는다. 설정을 `LogAppendTime` 으로 바꾸면 브로커가 받은 시각이 박힌다. 그러면 36 ms 뒤인 16:48:22.346 이 여기 들어간다.
...
셋째는 캐시가 줄여 준다. consumer 는 한 번 받아 온 스키마를 들고 있다. 그래도 못 보던 번호가 오면 다시 물어봐야 한다. 그 순간 레지스트리가 죽어 있으면 그 줄은 못 읽는다.
...
위 표는 group 마다 같은 줄을 읽는 시각을 하나씩만 짚는다. 그 사이 여섯 층 어딘가가 얼마나 차오르는지는 정지 화면 한 장으로는 안 보인다. 에이전트나 Kafka를 직접 멈춰 보면 그 앞 칸이 차오르는 순간이 그대로 보인다. 데모는 수집 서버와 파일을 한 층으로 묶어 여섯 층으로 그린다. 홉 사이인 네트워크 168 ms와 producer 36 ms는 층이 없어 안 그린다. 그래서 아래 데모의 누적은 2,012 ms가 아니라 1,808 ms다.
...
앞에서 다르게 센 것이 넷 더 있다. 도입의 다섯은 모양이 바뀐 곳만 셌다. 4절 파이썬 표의 일곱은 Kafka 에 닿기까지 시간이 쌓인 곳이다. 4절 대비 표의 셋은 액세스 로그 줄이 찍힌 뒤에 거치는 곳이다. 위 데모의 여섯은 화면에 그릴 수 있는 층만 셌다. 같은 경로인데 기준이 다섯이라 수가 다 다르다.
