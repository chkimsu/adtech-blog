...
핵심 문제는 pCVR 모델의 **Sample Selection Bias(SSB)**입니다. pCVR 모델은 "클릭한 샘플"만으로 학습합니다. 그런데 서빙 시에는 전체 노출 공간에서 예측해야 합니다. 이 train/serve 분포 불일치를 구조로 푸는 방법이 **멀티태스크 러닝(Multi-Task Learning)(MTL)**입니다.

> 한 줄 요약: 클릭과 전환을 **한 모델에서 같이 배우는** 이유는 두 가지입니다. 하나는 pCVR이 배우는 공간을 서빙하는 공간과 맞추는 것(ESMM). 다른 하나는 데이터가 적은 태스크가 많은 태스크의 표현을 빌려 쓰는 것(Shared-Bottom, MMoE, PLE)입니다. MMoE 는 태스크마다 다른 전문가 조합을 쓰고, PLE 는 공용 전문가와 전용 전문가를 층으로 나눕니다. 빌려 쓰면 이득이지만, 두 태스크가 상충하면 그 이득이 0으로 사라집니다.

> **문제 정의**는 [pCVR 모델링](post.html?id=pcvr-modeling)에 있습니다. SSB, 지연 전환, 중복 전환이 거기 있습니다. 이 글은 그 문제를 **구조로 푸는 방법**만 다룹니다. 각 타워(Tower) 내부의 구조는 [Deep CTR 모델](post.html?id=deep-ctr-models)로 넘깁니다. 태스크가 여러 개면 [보정](post.html?id=calibration)도 태스크별로 따로 해야 합니다. 후보 생성 단계는 [Two-Tower 리트리벌](post.html?id=two-tower-retrieval)이 맡습니다.

> **골라 읽는 법** — 절이 15개인 긴 글입니다. 처음부터 다 읽지 않아도 됩니다.
>
> - 왜 pCVR을 따로 학습하면 안 되는지만 → 1~3절
> - 구조 계보(바닥을 같이 쓰는 구조(Shared-Bottom)·MMoE·PLE)만 → 5·7·8절
...
먼저 전체 지형을 봅니다. 태스크 하나짜리(Single-Task)부터 PLE까지, 각 접근법의 구조적 특성을 한눈에 비교합니다.

| 구조 | 아키텍처 | SSB 해결 | 태스크(Task) 간 간섭 | 파라미터 효율 | 실무 난이도 |
|------|---------|---------|-------------|-------------|-----------|
| **태스크 하나짜리 (pCTR)** | 독립 모델 | 해당 없음 | 없음 | 낮음 (별도 모델) | 매우 낮음 |
| **태스크 하나짜리 (pCVR)** | 독립 모델 (클릭 데이터만) | 미해결 | 없음 | 낮음 (별도 모델) | 낮음 |
| **ESMM** | 두 타워 + 곱셈 | 완전 해결 | 없음 (곱셈 결합) | 중간 (Embedding 공유) | 낮음 |
| **바닥을 같이 쓰는 구조** | 공유 하위 레이어 + 태스크(task)별 타워 | 미해결 | 높음 (Negative Transfer) | 높음 (최대 공유) | 매우 낮음 |
| **MMoE** | 다중 전문가(Expert) + 태스크별 게이트(Gate) | 미해결 | 낮음 (게이트가 조절) | 중간 | 중간 |
| **PLE** | 공용(Shared) + 태스크 전용 전문가(Task-specific Expert), 다층 | 미해결 | 매우 낮음 | 중~낮음 (전문가 분리) | 높음 |

표를 통째로 외우지 말고 두 덩어리로 나눠 읽으세요. **ESMM은 학습 공간을 바꾸는 구조**입니다. 클릭된 샘플 대신 전체 노출에서 배우게 만듭니다. 반면 **바닥을 같이 쓰는 구조·MMoE·PLE는 표현을 나눠 쓰는 구조**입니다. 어느 파라미터를 어느 태스크가 얼마나 쓸지를 정합니다. 그래서 이 둘은 경쟁 관계가 아닙니다. 실제 프로덕션 모델은 두 구조를 겹쳐 씁니다.

> 핵심 관찰: ESMM은 SSB를 해결하는 유일한 구조입니다. 바닥을 같이 쓰는 구조, MMoE, PLE는 SSB를 직접 해결하지 않습니다. 대신 태스크 간 지식 공유와 간섭 조절에 초점을 맞춥니다. 실무에서는 ESMM의 곱셈 구조를 바탕으로 깔고 갑니다. 그 위에 타워 내부를 MMoE나 PLE로 채우는 하이브리드 구성이 일반적입니다.
...
pCVR 모델을 단독으로 학습하면, 학습 데이터는 "클릭이 발생한 샘플"에 한정됩니다. 그런데 서빙 시에는 "전체 노출"에 대해 전환 확률을 예측해야 합니다. 이것이 **Sample Selection Bias(배운 자리와 쓰는 자리가 다른 편향)**입니다.
...
좌변의 pCTCVR은 "노출 하나가 클릭을 거쳐 전환까지 갈 확률"입니다. 이 값은 **전체 노출(Impression) 공간**에서 정의됩니다. 클릭과 전환이 모두 일어난 사건이라, 전체 노출 데이터에서 라벨을 그대로 만들 수 있습니다. 클릭이 없었던 노출은 라벨 0입니다. 이것이 ESMM의 핵심 통찰입니다.

---

## 3. ESMM (노출 전체에서 배우는 모델(Entire Space Multi-Task Model), Alibaba 2018)

### 아키텍처

ESMM은 두 개의 타워로 구성됩니다. CTR 타워(CTR Tower)와 CVR 타워(CVR Tower)입니다. 둘은 공유 임베딩 층(Embedding Layer) 위에 나란히 올라가 각각 예측합니다. 최종 출력은 두 타워의 곱셈으로 만들어집니다.
...
여기서 $y_i$는 클릭 라벨입니다. $z_i$는 클릭과 전환이 모두 일어났는지를 나타내는 라벨입니다. $N$은 전체 노출 수입니다.

핵심은 **CVR에 대한 직접적인 Loss가 없다**는 점입니다. CVR 타워는 곱셈 노드를 통해서만 학습됩니다. $\mathcal{L}_{CTCVR}$의 기울기(gradient)가 그 노드를 거꾸로 타고 흘러 들어옵니다. 이 구조 덕분에 CVR 타워가 "클릭된 샘플"이 아닌 "전체 노출"에서 학습합니다.

:::deep 더 깊이 — 직접 손실이 없는데 CVR 타워는 어떻게 배우나
...
핵심은 맨 뒤에 곱해진 $p$입니다. CVR 타워가 받는 기울기가 **pCTR에 비례**합니다.

이 한 줄이 두 가지를 동시에 설명합니다. 첫째, 클릭이 없었던 노출도 라벨 0으로 CVR 타워를 밀어 줍니다. 그래서 학습이 전체 노출 공간에서 일어납니다. 둘째, pCTR이 아주 낮은 노출은 CVR 타워를 거의 못 움직입니다. 기울기가 $p$배로 줄어드니까요. 즉 ESMM은 "노출 전체를 균등하게" 배우는 게 아닙니다. **클릭 가능성으로 가중해서** 배웁니다. 이 성질이 있어야 pCVR의 뜻이 "클릭한 다음의 전환 확률"로 유지됩니다.

또 하나, 실무에서 자주 겪는 문제가 있습니다. $pq$는 아주 작은 수입니다. 노출당 전환이 0.06%면 $\log(1-pq)$ 항이 거의 0입니다. float32로 계산하면 유효 자릿수가 날아가기 쉽습니다. 그래서 프로덕션 구현은 logit 공간에서 안정화된 BCE를 씁니다. 두 타워의 출력을 곱할 때도 확률을 직접 곱하지 않고 log를 더합니다.
:::
...
| 관점 | 기존 pCVR 모델 | ESMM |
|------|-------------|------|
| **학습 데이터** | 클릭된 샘플만 (100만 건) | 전체 노출 (1억 건) |
...
| **추가 이점** | 없음 | CTR의 풍부한 시그널이 공유 Embedding을 통해 CVR 타워로 전이 |
...
- 전체 노출 공간에서 학습함으로써 데이터가 드문 문제(Data Sparsity) 문제도 동시에 완화

> ESMM의 강점은 SSB를 구조로 해결한다는 점입니다. 다른 MTL 아키텍처는 태스크 간 지식 공유에 초점을 맞춥니다. 바닥을 같이 쓰는 구조, MMoE, PLE 모두 SSB를 직접 해결하지는 않습니다. 실무에서는 ESMM의 곱셈 구조를 그대로 둡니다. 그 안의 타워만 MMoE나 PLE로 바꿔 끼우는 하이브리드가 일반적입니다.
...
주의할 점도 있습니다. 이 코드는 ESMM의 기울기 학습을 흉내낸 것이 아닙니다. "학습 공간이 넓어지면 같은 정규화가 약하게 작용한다"는 한 가지 메커니즘만 떼어내 보인 것입니다. 실제 ESMM의 이득은 여기에 하나가 더 붙습니다. 공유 Embedding이 CTR 태스크의 풍부한 신호로 함께 학습된다는 점입니다. 그 효과는 다음 파이썬 절에서 따로 확인합니다.

---

## 5. 바닥을 같이 쓰는 구조 & 파라미터를 통째로 나눠 쓰기(Hard Parameter Sharing)

### 가장 기본적인 MTL 구조

바닥을 같이 쓰는 구조는 MTL의 가장 단순하고 직관적인 형태입니다. 하위 레이어를 모든 태스크가 공유하고, 상위에 태스크별 타워를 둡니다.
...
- **구현이 간단**: 기존 태스크 하나짜리 모델에 타워만 추가하면 됩니다
- **데이터 효율적**: 태스크 간 공유 표현을 학습하므로, 데이터가 적은 태스크(예: CVR)가 데이터가 풍부한 태스크(예: CTR)의 시그널을 활용합니다
- **파라미터 효율적**: 하위 레이어의 파라미터를 공유하므로 메모리와 서빙 비용이 절감됩니다
- **Regularization 효과**: 여러 태스크를 동시에 학습하면 각 태스크가 서로에게 regularizer 역할을 하여 overfitting을 방지합니다

### 한계: 서로 방해하는 전이(Negative Transfer)

바닥을 같이 쓰는 구조의 치명적 약점은 **서로 방해하는 전이**입니다. 태스크 간 관계가 상충할 때, 공유 레이어가 두 태스크를 동시에 만족시키지 못하고 양쪽 성능이 모두 하락합니다.

광고 시스템에서 전형적으로 나타나는 서로 방해하는 전이는 세 가지입니다.

- **CTR과 CVR의 상충**: "낚시성 크리에이티브"는 CTR이 높지만 CVR이 낮습니다. 공유 표현이 CTR 최적화 쪽으로 끌리면, CVR 예측이 악화됩니다
- **태스크 간 데이터 규모 차이**: CTR 데이터는 CVR 대비 100배 많습니다. Gradient가 CTR에 지배되어 CVR 타워가 제대로 학습되지 않습니다
- **태스크 간 최적 표현의 차이**: CTR에 유용한 low-level 표현(예: 크리에이티브 시각 피처)이 CVR에는 불필요하거나 해로울 수 있습니다

이 서로 방해하는 전이 문제를 해결하기 위해 등장한 것이 MMoE와 PLE입니다.
...
세계 A를 먼저 보겠습니다. 상수 예측 0.1165에서 시작해 천장이 0.1049입니다. 개선의 여지는 -9.9%뿐입니다. 단독 학습은 그 여지의 69%를 회수했고(-6.8%), 공유는 86%를 회수했습니다(-8.5%). 전환 60건으로 200개를 배우는 것보다, 클릭 2천 건이 채워 준 표현을 빌려 쓰는 게 낫습니다. 이것이 Shared-Bottom이 데이터 효율적이라고 말하는 이유입니다.
...
전문가 망(Expert Network)가 $n$개 있다고 합시다. 각각 $f_1$부터 $f_n$까지입니다. 태스크 $k$의 게이트 망(Gating Network)는 입력 $x$를 받아 softmax 가중치를 냅니다.

$$g^k(x) = \text{softmax}(W_g^k \cdot x) \in \mathbb{R}^n$$

이 가중치로 전문가들의 출력을 섞습니다.

$$h^k(x) = \sum_{i=1}^{n} g_i^k(x) \cdot f_i(x)$$

$g_i^k(x)$는 태스크 $k$가 expert $i$에 주는 가중치입니다. $f_i(x)$는 expert $i$의 출력 벡터입니다. 이렇게 섞인 $h^k(x)$가 태스크 $k$의 타워로 들어갑니다. 가중치가 입력마다 다시 계산된다는 점이 중요합니다. 같은 모델이 요청마다 다른 전문가 조합을 쓴다는 뜻입니다. softmax라서 가중치의 합은 항상 1입니다.

### 바닥을 같이 쓰는 구조 대비 장점

| 관점 | 바닥을 같이 쓰는 구조 | MMoE |
|------|-------------|------|
| **표현 공유 방식** | 모든 태스크가 동일한 표현 사용 | 각 태스크가 필요한 전문가를 선택적으로 활용 |
| **태스크 간 상충 대응** | 불가 (Negative Transfer) | 게이트가 상충하는 전문가를 회피 |
| **유연성** | 없음 | 입력 $x$에 따라 동적으로 전문가 조합 변경 |
| **해석 가능성** | 낮음 | 게이트 가중치로 task-expert 관계 분석 가능 |

실제로 게이트 가중치를 뽑아 보면 태스크별로 쏠림이 보입니다. CTR 태스크는 "유저 행동 패턴" 전문가에 높은 가중치를 줍니다. CVR 태스크는 "광고 품질" 전문가 쪽으로 기웁니다. 두 태스크가 쓰는 전문가가 나뉘면 서로의 기울기가 덜 부딪힙니다. 이것이 서로 방해하는 전이를 완화하는 메커니즘입니다.

### Google 실험 결과

Google은 Census Income 데이터로 태스크 correlation을 인위적으로 조절해 실험했습니다. 상관이 낮아질수록 MMoE와 바닥을 같이 쓰는 구조의 격차가 벌어졌습니다. 상관 0.0에서 바닥을 같이 쓰는 구조는 급격히 무너졌습니다. MMoE는 그 지점에서도 성능을 유지했습니다. 게이트가 태스크 간 상충을 조절한다는 근거입니다. 앞 절 세계 B에서 공유의 이득이 0이 됐던 것과 같은 현상입니다.

---

## 8. PLE (층으로 나눠 뽑는 구조(Progressive Layered Extraction), Tencent 2020)

### MMoE의 한계

MMoE에서 모든 전문가는 모든 태스크에 공유됩니다. 게이트가 전문가 선택을 조절할 뿐입니다. 전문가 자체는 특정 태스크에 특화되지 않습니다. 이 때문에 두 가지 문제가 남습니다.

1. **한쪽이 오르면 한쪽이 내리는 현상(Seesaw) 현상**: 한 태스크의 성능이 올라가면 다른 태스크가 떨어지는 trade-off가 완전히 해결되지 않습니다
2. **태스크 전용(Task-specific) 학습 부족**: 전문가가 모든 태스크의 기울기를 받으므로, 특정 태스크에만 유용한 표현을 깊이 있게 학습하기 어렵습니다

### PLE의 구조

PLE는 **공용 전문가(Shared Expert)**와 **태스크 전용 전문가**를 명시적으로 분리합니다. 그리고 이 묶음을 **여러 층으로 쌓습니다**. 층을 지날수록 태스크별 표현이 조금씩 더 다듬어집니다.
...
PLE의 Extraction Network는 각 층(layer)에서 세 가지 일을 합니다.

1. **공용 전문가**: 모든 태스크에 공통으로 유용한 표현을 학습
2. **태스크 전용 전문가**: 해당 태스크에만 유용한 표현을 학습
3. **게이트**: 공용 전문가와 태스크 전용 전문가의 출력을 태스크별로 가중 결합

층을 거듭할수록 태스크별 표현이 **점진적으로** 나뉩니다. 하위 층에서는 공통 표현을 주로 배웁니다. 상위 층으로 갈수록 태스크 고유의 표현이 분화됩니다. 한 번에 나누지 않고 층층이 나누는 것이 이름에 붙은 "Progressive"의 뜻입니다.

MMoE와의 핵심 차이는 표로 보는 편이 빠릅니다.

| 관점 | MMoE | PLE |
|------|------|-----|
| **전문가 구분** | 모든 전문가가 공유 | 공용 + 태스크 전용 명시 분리 |
| **Layer 수** | 단일 층 | 다층 (Progressive) |
| **태스크 전용 학습** | 게이트 가중치에 의존 | 전용 전문가가 명시적으로 학습 |
| **한쪽이 오르면 한쪽이 내리는 현상 현상** | 부분적 완화 | 대폭 완화 |
| **파라미터 수** | 전문가 수 x 전문가 크기 | (공용 + 태스크별) 전문가 수 x Layer 수 |

### Tencent 실험 결과

Tencent는 자사 대규모 추천 시스템에서 PLE를 검증했습니다. 결과는 세 줄로 요약됩니다.

- MMoE 대비 **모든 태스크에서 동시에** 성능 향상 (한쪽이 오르면 한쪽이 내리는 현상 현상 해소)
- 태스크 correlation이 낮을수록 MMoE 대비 개선 폭이 커짐
- 프로덕션 A/B 테스트에서 VCR(Video Completion Rate) +1.97%, VTR(View-Through Rate) +0.63% 개선

> PLE의 가장 큰 기여는 "모든 태스크의 성능을 동시에 올릴 수 있다"는 실증입니다. 기존 MTL에서는 한 태스크를 올리면 다른 태스크가 내려가기 일쑤였습니다. 이 한쪽이 오르면 한쪽이 내리는 현상 현상이 MTL 도입의 가장 큰 장벽이었습니다.

---

## 9. 실무 선택 가이드

**고를 때 보는 것은 세 가지입니다.** 태스크가 몇 개인가, 서로 얼마나 상관 있는가, 데이터가 얼마나 있는가. 아래 표는 이 세 기준으로 나눈 것입니다.

| 상황 | 추천 구조 | 이유 |
|------|---------|------|
| 태스크가 2개이고 sequential 관계 (CTR → CVR) | **ESMM** | SSB를 구조적으로 해결, 구현 간단 |
| 태스크가 3개 이상 (CTR, CVR, 체류 시간 등) | **MMoE** 또는 **PLE** | 다수 태스크 간 게이트로 유연한 지식 공유 |
| 태스크 간 상관관계가 높음 (CTR & Like) | **바닥을 같이 쓰는 구조** | 간단한 구조로 충분, 서로 방해하는 전이 위험 낮음 |
| 태스크 간 상관관계가 낮음 (CTR & 체류 시간) | **PLE** | 태스크 전용 전문가가 간섭을 방지 |
| 데이터 규모가 작음 (수백만 건 이하) | **바닥을 같이 쓰는 구조** 또는 **ESMM** | 파라미터 공유로 데이터 효율 극대화 |
| 데이터 규모가 큼 (수억 건 이상) | **PLE** | 충분한 데이터로 태스크 전용 전문가 학습 가능 |
...
## 11. 파이썬으로 확인하기 ③ — 가중치를 바꾸면 한쪽이 오르고 한쪽이 내린다

**손실 = w1 × CTR손실 + w2 × CVR손실 에서 w만 바꿔 세 번 학습합니다.** (1,1), (1,5), (5,1) 세 가지입니다. 구조도 데이터도 그대로입니다. 그런데 한 태스크가 좋아지면 다른 태스크가 나빠집니다. 8절에서 말한 Seesaw 현상입니다.
...
읽기 전에 알아 둘 장치가 하나 있습니다. 세 실험 모두 **가중치 합을 2로 맞췄습니다**. 안 맞추면 가중치를 키운 쪽이 학습률까지 같이 커집니다. 그러면 상충 때문에 나빠진 건지 보폭이 커서 나빠진 건지 구분할 수 없습니다. 실무에서도 그대로 겪는 실수입니다.
...
가장 눈에 띄는 줄은 첫 줄입니다. **아무 생각 없이 1:1로 두면 CVR이 +0.1557 나빠집니다.** CTR은 오히려 살짝 좋아졌습니다(-0.0042). 왜 이렇게 기울었을까요. CVR 손실은 클릭된 24%의 행에서만 발생합니다. 기울기 총량이 애초에 CTR 쪽으로 기울어 있습니다. 1:1은 공평한 가중치처럼 보이지만 공평하지 않습니다.

CVR에 5배를 주면(1:5) CVR 손실이 0.3269까지 회복됩니다. 단독 학습과의 차이가 +0.0208로 줄었습니다. 대신 CTR이 +0.0783 나빠졌습니다. 정확히 한쪽이 오르면 한쪽이 내리는 현상 현상입니다. 반대로 CTR에 5배를 주면(5:1) CVR이 +0.1626으로 가장 나빠집니다. 그런데 CTR도 1:1보다 나빠졌습니다(0.3439 대 0.3328). CTR은 1:1에서 이미 거의 최선이라 더 얻을 게 없다는 뜻입니다.
...
그리고 이 표에는 좋은 칸이 없습니다. 어느 가중치를 골라도 한쪽은 단독 학습보다 나쁩니다. 공유 표현이 하나뿐이라 구조적으로 그렇습니다. 가중치 조절로는 어느 쪽이 손해를 볼지만 바꿀 수 있습니다. 상충 자체를 없애려면 표현을 늘려야 합니다. 그게 MMoE의 전문가와 PLE의 task-specific 전문가입니다.
...
2. **바닥을 같이 쓰는 구조는 간단하지만 서로 방해하는 전이에 취약하다** --- 태스크 간 상충 시 공유 레이어가 양쪽 성능을 모두 악화시킵니다. CTR과 CVR처럼 데이터 규모와 최적 표현이 다른 태스크에서 특히 문제가 됩니다.

3. **MMoE는 게이트로 서로 방해하는 전이를 완화한다** --- 각 태스크가 자신만의 게이트 망으로 필요한 전문가를 선택적으로 활용합니다. 태스크 correlation이 낮을수록 바닥을 같이 쓰는 구조 대비 이점이 큽니다.

4. **PLE는 공용 전문가와 태스크 전용 전문가를 분리하여 한쪽이 오르면 한쪽이 내리는 현상 현상을 해소한다** --- 다층 구조로 progressive하게 task-specific 표현을 정제하여, 모든 태스크의 성능을 동시에 올릴 수 있습니다.

5. **단일 모델이 여러 목표를 동시에 추구하면, 각 목표가 서로에게 regularizer 역할을 하여 일반화 성능이 올라간다** --- 이것이 MTL의 근본적인 이점이며, 광고 시스템처럼 여러 예측 태스크가 공통된 유저-광고 표현에 의존하는 도메인에서 특히 강력합니다.
...
> MTL은 모델 아키텍처만의 문제가 아닙니다. 각 태스크의 출력 확률은 [Calibration](post.html?id=calibration)으로 따로 보정해야 합니다. 공유 피처는 [Feature Store](post.html?id=feature-store-serving)가 공급합니다. 추론 비용은 [모델 서빙](post.html?id=model-serving-architecture)에서 관리합니다. 아키텍처 선택은 이 전체 파이프라인의 한 조각입니다.
