...
> **골라 읽는 법** — 긴 글입니다(약 46분). 처음부터 다 읽지 않아도 됩니다.
>
> - 모델 계보만 훑으려면 → §1~3
> - 유저 행동 시퀀스(DIN·DIEN)만 → §4
> - 고르는 기준과 서빙 제약만 → §5~6
> - 두 무대 비교만 → §7~8
...
- **AUC가 올라도 돈은 잃을 수 있다.** AUC는 순위만 봅니다. 예측 확률의 절대값이 틀리면 입찰가가 통째로 틀어집니다. §1에서 못 박은 대로, 이건 보정의 몫입니다 → [Calibration](post.html?id=calibration)
