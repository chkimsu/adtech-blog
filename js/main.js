...
function preprocessMarkdown(text) {
  ...
  // 범위 기호로 쓴 물결표를 엔티티로 바꿔 취소선 오파싱을 막는다.
  // GFM은 물결표 '한 개'도 취소선으로 인정해서, 한 문단에 '1~3개'와 '수백~수천'이
  // 같이 나오면 그 사이 본문이 통째로 그어졌다. 코드·수식은 위에서 스태시돼 안전하고,
  // '~~진짜 취소선~~'은 물결표가 붙어 있어 이 패턴에 걸리지 않는다.
  text = text.replace(/(?<=[0-9A-Za-z가-힣%)\]])~(?=[0-9A-Za-z가-힣])/g, '&#126;');
