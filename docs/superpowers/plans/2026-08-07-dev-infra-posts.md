...
## 본문 태스크 공통 규칙 (Task 4 리뷰에서 나온 것 — 4·5·6·7·8·12·13·14·15·16에 전부 적용)

Task 4의 첫 원고는 문체·문장 길이·표 표기는 다 통과했는데, 아래 아홉 가지에서 걸렸다. **같은 지적이 반복되지 않게 처음부터 지킨다.**

| # | 규칙 | Task 4에서 무슨 일이 있었나 |
|---|---|---|
| 1 | **기술 주장은 실무자의 반박을 견뎌야 한다.** 쓰기 전에 "이걸 아는 사람이 뭐라고 받아칠까"를 한 번 통과시킨다 | "헬스체크로 빼면 배포 손실이 0" 이라고 썼는데, L4 LB는 연결 단위로 분배한다. 12ms 예산이면 매체가 연결을 붙여 두므로 이미 맺힌 연결은 안 옮겨진다. 결론이 자기 서술과 모순됐다 |
| 2 | **주소·대역·프로토콜을 실제로 성립하게 쓴다** | 외부 매체가 사설 대역 `10.0.3.14` 를 부르게 써 놨다. 이 블로그 저자는 **담장 안 DSP** 라 매체와 같은 데이터센터에 있다 — 그 전제를 글에 명시하면 사설 IP·평문·12ms가 전부 성립한다 |
| 3 | **숫자는 표 안뿐 아니라 첫 등장에서 밝힌다** | 표 두 개는 "가상 수치" 라고 밝혔는데 본문의 `12ms`·`초당 3,000건` 은 맨 사실로 나왔다. 하필 그게 글의 뼈대라 독자가 인용하기 가장 쉬운 숫자였다 |
| 4 | **그림이 뒤 문단의 반전을 미리 까발리지 않게 배치한다** | "가장 단순하고 빠르다" 바로 뒤에 "배포하면 요청이 전부 실패" 라고 적힌 그림을 놓아, 다음 문단의 "문제는 배포할 때 드러난다" 가 반전이 아니게 됐다 |
| 5 | **`var(--state-bad)` 는 진짜 실패에만** | 본문이 "무해하다" 고 설명한 상태(헬스체크로 정상 제외)에 실패 색을 칠했다. 그림이 본문의 반대를 말했다 |
| 6 | **다음 부품은 단정이 아니라 사고가 불러온다** | "두 요청을 같은 서버에 둘 이유가 없다" 는 단정이라 독자가 두 갈래로 빠져나갈 수 있었다(같이 둘 이유도 있고, L4에서 포트로 나눠도 된다). 구체적 사고를 보이고 빠져나갈 길까지 막아야 한다 |
| 7 | **그림은 본문이 못 하는 것을 담는다** | 헬스체크가 이 절의 핵심 장치인데 그림에는 `GET /healthz · 5s` 라는 글자만 떠 있었다. 화살표로 동작을 그려야 정보가 된다. figcaption도 본문을 되풀이하지 않는다 |
| 8 | **색만으로 구분하지 않는다** | `--state-bad`(#9c3b26)와 `--accent-primary`(#b0442c)는 이 팔레트에서 같은 벽돌 계열이라 렌더하면 구분이 안 된다. 점선·굵기 같은 모양 신호를 같이 준다 |
| 9 | **산술을 끝까지 센다** | 주기 5초 + 타임아웃 2초 + 연속 2회면 최악 12초인데 10초라고 적었다. 다른 곳이 전부 정확해서 확인하는 독자가 반드시 찾아낸다 |

추가로, **읽고 나면 손에 잡히는 것이 있어야 한다.** Task 4는 헬스체크의 *주기*는 잘 가르쳤는데 *무엇을 검사해야 하나*가 빠졌다 — 그게 엔지니어 독자가 온 이유다. 각 절에서 "이 글을 읽고 내일 뭘 바꿀 수 있나"를 한 가지는 남긴다.

### 절 분량은 KB로 재지 않는다 (Task 5에서 정정)

Task 5에 "3절은 5~6KB" 라는 목표를 줬는데 **틀린 지시였다.** 계획이 요구한 그림·표만 더해도 SVG 4.3 + mermaid 0.65 + yaml 0.44 + 표 2개 1.0 = **6.4KB가 산문 이전에 고정**된다. KB를 맞추려면 계획이 명시한 요소를 빼는 수밖에 없다.

**대신 이렇게 잰다 — 산문(그림·코드·표를 뺀 나머지)이 앞 절보다 가벼운가.**

| 절 | 전체 | 그림·코드·표 | 산문만 |
|---|---|---|---|
| 1절 | 4.44KB | 2.16KB | 2.28KB |
| 2절 | 10.50KB | 4.60KB | 5.90KB |
| 3절 | 9.86KB | 4.31KB | 5.55KB |

2절이 두꺼운 것은 용어(L4/L7)·지형(같은 데이터센터)·예산(12ms)·주소 체계·그림 문법을 **처음 세우느라** 그런 것이다. 3절은 그걸 전부 물려받아 실제로 더 가볍다. 그게 맞는 상태다.

글 1 전체 예상은 47~48KB. `software-architecture-patterns`(50KB)·`ad-log-pipeline`(57KB) 안쪽이라 문제없다. **분량을 줄이려고 그림이나 계산을 빼지 않는다.**
...
````markdown
```yaml
# Ingress 규칙 — 맞는 줄을 전부 찾은 뒤 가장 긴 것을 고른다. 적힌 순서는 상관없다.
rules:
  - host: ads.example.com
    paths:
      - path: /v1/bid      pathType: Prefix  →  bidder-service:8080
      - path: /v1/track    pathType: Prefix  →  log-collector:8080
      - path: /v1/feature  pathType: Prefix  →  feature-store:8080
  - host: admin.example.com
    paths:
      - path: /            pathType: Prefix  →  admin-service:3000
# 맞는 줄이 하나도 없으면 404.
```
````

> **원안의 "순서가 곧 우선순위" 는 사실이 아니었다 (Task 5 리뷰에서 잡힘).** 쿠버네티스 Ingress 명세는 *"precedence will be given first to the longest matching path"* 이고, ingress-nginx 는 템플릿에 쓰기 전에 **경로를 길이 내림차순으로 정렬**한다. nginx 자체도 설정 순서와 무관하게 가장 긴 prefix 를 고른다. 그래서 `/v1` 을 `/v1/bid` 위에 둬도 입찰 요청은 `/v1/bid` 로 간다 — **원안이 극화한 사고는 일어나지 않는다.**

- **진짜 함정은 반대 방향이다.** `/v1` 을 `pathType: Prefix` 로 추가하면 `/v1/bid` 를 빼앗지는 못하지만, 그 아래에서 **404 나던 것들을 조용히 삼킨다** — `/v1/report`, `/v1/status`, 오타 난 `/v1/bidd`. 문법 오류도 없고 배포도 성공하고 헬스체크도 전부 초록이다. 이쪽으로 사고를 만든다
- **손에 잡히는 규칙은 "길이가 이긴다"** 다. 순서를 맞추라고 쓰지 않는다
- `pathType: Prefix` 는 **글자가 아니라 `/` 로 끊은 조각 단위**로 맞춘다 — `/v1/bid` 는 `/v1/bid/test` 를 잡지만 `/v1/bidder` 는 안 잡는다. 이 경계 규칙을 반드시 넣는다
- `pathType` 은 `networking.k8s.io/v1` 에서 **필수 필드**다. yaml 예시에서 빼먹지 않는다
...
> **`classDef` 에 `fill` 과 `color` 를 주지 말 것 (Task 5에서 발견).** 채움과 글자색을 지정하면 **다크 테마에서 글자가 안 보인다.** 사이트 CSS가 mermaid 라벨 맨 안쪽 `<p>` 의 색을 밝은 베이지로 덮어써서, 크림 채움 위에 베이지 글자가 된다(대비 약 1.6:1). 저장소 방침도 같다 — `scripts/recolor-mermaid.js` 주석: *"채움을 아예 없애고 윤곽선만 남긴다. 라이트·다크 양쪽에서 테마 기본 배경을 따라가므로 저절로 맞는다."* **테두리 색과 굵기만 준다.** 굵기 차이(2px vs 1px)가 색맹·흑백 인쇄에서도 통하는 모양 신호 역할을 겸한다.
...
(나) **사이드카가 12ms 입찰 예산을 얼마나 먹나.**

> **아래 숫자는 4절이 확정한 값에 맞춘 것이다 (Task 6 리뷰에서 정정).** 원안은 `bidder 자체 3.0 + pctr 4.5 + feature-store 2.0 = 10.9ms` 였는데, 실제로 쓰인 4절은 Gateway 타임아웃을 10ms로 잡고 `Ingress 0.3 + Gateway 1.1 + bidder 10 = 11.4` 로 못 박았다. **여유가 0.6ms 뿐이라 원안대로면 5절의 논증이 아예 성립하지 않는다** — 사이드카 통과 1회를 0.15ms로 잡아야만 겨우 들어가는데, 실제 Envoy p50 은 0.3~1.0ms다.
>
> 4절에 `bidder 가 실제로 쓰는 건 평소 8ms 안팎이고 10ms는 그 위에 얹은 상한` 이라는 문장을 넣기로 했다. 그래서 5절은 **평소값과 상한값 두 줄로** 계산한다.

고정 입력값:
- 입찰 예산 12ms
- 앞단: Ingress 0.3 + Gateway 1.1 (4절 확정값)
- `bidder` **평소** 8.0ms = 자체 3.0 + pctr 호출 3.0 + feature-store 호출 2.0
- `bidder` **상한** 10.0ms (4절의 `timeout: { ms: 10 }`)
- 사이드카 1회 **통과** 0.3ms (Envoy p50 하단)
- 메시를 넣으면 내부 호출 2개가 각각 양 끝에서 잡히므로 **통과 4회**

출력 두 줄:

| | 메시 전 | 메시 후 | 예산 12ms |
|---|---|---|---|
| 평소 | 9.4ms | 10.6ms | 들어간다 (여유 1.4ms) |
| 상한 | 11.4ms | 12.6ms | **넘는다** |

그리고 사이드카 통과 지연이 0.15 / 0.3 / 0.5 / 1.0ms 일 때 평소·상한이 각각 어디서 예산을 넘는지 표. **이게 5절의 진짜 논점이다** — "평소엔 들어가는데 최악에는 넘친다"는 것이 메시 도입을 망설이게 하는 실제 이유다. 단정으로 "12ms엔 메시를 못 쓴다"고 쓰지 않는다.
