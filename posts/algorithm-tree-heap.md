...
코드로 담는 방법은 간단하다. 값 하나와 왼쪽·오른쪽을 가리킬 자리 둘을 가진 마디(Node)를 만들고, 그 마디끼리 서로를 가리키게 하면 된다.
...
- **이진 탐색 트리가 항상 O(log n)은 아니다.** 정렬된 순서로 넣으면 한 줄이 되어 O(n)이 된다. 같은 값 아홉 개로 4회와 8회로 나뉘었다.
