...
1절의 숫자로 돌아가 보자. 같은 광고인데 1위(5.0%)와 5위(0.75%) 사이에 CTR이 6.7배 벌어졌다. 모델이 이 차이를 그대로 학습하면, "5위에 있던 광고는 원래 나쁘다"고 잘못 결론짓는다. 그리고 다음번에도 그 광고를 5위 밑으로 내린다. 한 번 밀린 광고는 계속 밀린다. 1절에서 본 Rich-Get-Richer 루프가 여기서 실제 피해로 이어지는 것이다.
...
3위였던 C도 2위로 올라와 A·B를 앞지른다. 이건 노이즈다. 광고 하나당 노출이 10,000번뿐이라 클릭 수가 요동친 탓이다. 4절에서 다시 나오는 **IPS의 분산 문제**가 바로 이 흔들림이다. 극단적인 반전(D)은 표본이 작아도 믿을 만하다. 하지만 중간 순위의 미세한 차이는 표본을 늘리기 전엔 장담하기 어렵다.

이 Examination 추정치가 어디서 나왔는지, 보정 계산을 코드로 어떻게 하는지는 4절에서 그대로 이어간다.
...
2절에서 본 다섯 광고 표를 그대로 코드로 만들어 보겠습니다. 실험에서 Examination 확률을 추정하는 부분까지 포함합니다.
...
이 간접 추정은 Propensity 추정 정확도에 그대로 영향을 준다. 4절의 표에서 본 것처럼 IPS는 애초에 Examination 확률 추정이 틀리면 보정 자체가 틀어진다. 그래서 열린 RTB의 Position Bias 보정은 랭킹 공정성보다 pCTR을 대략 맞추는 데 머무는 경우가 많다.

### 담장 안에서는 위치를 정확히 안다 [무대: 닫힌 생태계]

네이버·카카오·구글처럼 자기 지면에 자기 광고를 꽂는 닫힌 생태계는 사정이 다르다. 광고가 화면 몇 번째 자리에 실렸는지를 서빙 로그에 그대로 남긴다. 위치 로그가 100% 정확하니, 1절의 표 같은 Propensity 추정 실험도 어렵지 않다.

더 큰 무기는 **무작위 섞기(randomization)**다. 트래픽의 일부를 떼어 광고 순서를 일부러 뒤섞으면, 같은 광고가 여러 위치에 무작위로 노출된다. 4절의 파이썬 코드에서 "프로브 광고"로 Examination 확률을 추정한 방식이 바로 이거다. 실험이라 매출 손실이 따르지만, 그만큼 편향을 직접 재는 가장 확실한 방법이다. 그래서 닫힌 생태계에서는 Position Bias 보정이 랭킹 모델 학습에 그대로 들어간다. 4·5절에서 다룬 IPS·DLA가 실제로 서빙 랭킹을 바꾸는 지점이 여기다.
