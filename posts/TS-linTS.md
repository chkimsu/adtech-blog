...
| --- | --- | --- | --- |
| **Standard TS** | 분포 형성. → **평균 5% 근처**에서 샘플링됨. | 분포 형성 (완전 랜덤). → 0~100% 사이 아무 값이나 나옴. | 광고 A가 압도적으로 유리함. **광고 B는 운에 맡겨야 함.** |
| **Linear TS** | Feature: `[나이키, 러닝화]` → 이미 학습된 가중치(`나이키` High, `러닝화` High) 적용. → **높은 점수** 예상. | Feature: `[나이키, 러닝화, 신상]` → **A와 동일한 가중치(나이키, 러닝화)를 공유**받음. → **즉시 높은 점수** 획득 가능. | **광고 B도 A만큼 좋은 광고라고 바로 인식**하고 경쟁에 참여함. |
