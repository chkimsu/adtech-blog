...
> **핵심:** 모든 Git 명령은 결국 이 4개 공간 사이의 데이터 이동입니다. 아래 모든 절에서 "이 명령은 어디에서 어디로 데이터를 옮기는가?"를 기억하세요.
...
| 개념 | 정체 |
|------|------|
| **HEAD** | 현재 체크아웃된 위치를 가리키는 포인터 |
| **브랜치** (main, feature/x) | 특정 커밋을 가리키는 포인터. 이름이 붙은 것뿐이라 만들고 지우는 비용이 거의 없습니다 |
| **커밋** | 그 시점 코드 전체의 저장본 + 부모 커밋 링크. 언제든 그 시점으로 되돌릴 수 있습니다 |
...
스테이징은 **"이번 커밋에 넣을 것"을 담아 두는 자리**입니다. 거기에 셋만 담아 커밋하고, 나머지 둘은 다음 커밋으로 넘깁니다. 그래서 실무에서는 `git add .`(전부 담기)보다 `git add <파일>`이나 `git add -p`(변경 조각 단위로 고르기)를 더 자주 씁니다.
...
앞 절의 세 자리로 돌아가 봅시다. 작업 폴더(방금 고친 것), 스테이징(담아 둔 것), 마지막 커밋(저장된 것). 비교할 수 있는 짝이 셋이니 명령도 셋입니다.

- `git diff` — 방금 고친 것 vs 스테이징. **아직 안 담은 변경**을 봅니다.
- `git diff --staged` — 스테이징 vs 마지막 커밋. **커밋하기 직전 최종 점검**입니다.
...
`stash`는 그 사이의 선택지입니다. 변경을 잠시 따로 저장해 두고, 작업 폴더를 깨끗하게 만듭니다.

다만 함정이 있습니다. **stash에 넣은 건 잊힙니다.** stash는 브랜치나 커밋처럼 눈에 보이지 않아서, 며칠 뒤에 "그 작업 어디 갔지?"가 됩니다. `git stash list`를 쳐 보고 다섯 개가 쌓여 있는 걸 발견하는 일이 흔합니다.
...
### "Divergent Branch" 경고 — git pull이 갑자기 멈추는 이유
...
셋을 나누는 기준은 하나입니다. **"배포를 얼마나 자주 하나."**
