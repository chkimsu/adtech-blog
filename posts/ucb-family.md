...
$$\text{Score}_a = \underbrace{\bar{x}_a}_{\text{평균 보상}} + \underbrace{\sqrt{\frac{2 \ln t}{n_a}}}_{\text{탐색 보너스}}$$
...
$$\text{Score}_a = \underbrace{x^T \theta_a}_{\text{개인화 예측}} + \underbrace{\alpha \sqrt{x^T A_a^{-1} x}}_{\text{개인화 탐색 보너스}}$$
...
| 예측 | 전체 평균 $\bar{x}_a$ | 개인화 예측 $x^T \theta_a$ |
| 보너스 | $\sqrt{\frac{2 \ln t}{n_a}}$ (횟수 기반) | $\alpha \sqrt{x^T A_a^{-1} x}$ (Feature 방향 기반) |
| 저장 | 숫자 2개 | 행렬 $A_a$ ($d \times d$) + 벡터 $b_a$ ($d \times 1$) |
...
$$\text{Score}_a = \underbrace{z_a^T \beta}_{\text{공통 지식}} + \underbrace{x^T \theta_a}_{\text{개별 지식}} + \text{탐색 보너스}$$
