...
- **Header Bidding 호환**: 같은 노출이 여러 경매에 동시에 오르는 [헤더비딩](post.html?id=header-bidding) 구조에서는 「2등 값」이 어느 경매의 것인지가 흐려짐
...
이 **맞바꿈**의 균형점이 최적 입찰가 $b^*$입니다. [데모의 Sweep 차트](demo-bid-shading.html)에서 이 균형점을 직접 확인할 수 있습니다.
...
말로는 맞바꿈이지만 숫자로 보면 뚜렷합니다. 진짜 가치 1,000원인 노출 하나를 놓고 깎는 비율만 바꿔 봅니다. 경쟁자 최고가는 **가상 데이터** 20,000건이고, 그 평균은 546원입니다.
...
여기서 실무의 요령이 하나 나옵니다. 꼬리를 알고 싶으면 **사 와야 합니다.** 일부 트래픽에 일부러 높게 입찰해 비싼 경매의 낙찰가를 관측하는 것입니다. 그 입찰은 대체로 손해입니다. 손해를 내고 정보를 사는 셈이니, 탐색 비용입니다([탐색과 활용](post.html?id=exploration-exploitation)에서 다루는 그 맞바꿈과 같은 구조입니다).
...
# Grid Search(500개) 대비 수십 배 빠른 수렴
# 출력:
#   b* = $2.6788  (13회 반복, 구간 < $0.01)
```

**수렴 속도**: 매 반복마다 탐색 구간이 $1/\phi \approx 0.618$배로 줄어듭니다. $V = \$5.00$, $\varepsilon = \$0.01$일 때 **13회 반복**이면 충분합니다. Grid Search($N = 500$개 그리드)보다 수십 배 빠릅니다.
...
- 목표에 따라 **캠페인의 54.5~71.8%**에서 비용 효율이 개선됨
