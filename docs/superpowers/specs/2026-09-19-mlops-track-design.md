...
| 분산 학습 | GPU 1장 6시간 20분(30일 재학습, 배치 4,096, 초당 32,700행). 4장 1시간 50분(효율 86%), 8장 1시간 5분(효율 73%). 임베딩은 Adam 이면 83.2GB 라 GPU 80GB 한 장에 안 들어가 파라미터 서버 2대 | embedding-table-ops(83.2GB, 샤드 2대) |
| 배치 크기 두 벌 | 매일 학습은 배치 1,024 로 초당 7,530행(하루 2,485만 행이 55분), 30일 재학습은 배치 4,096 으로 초당 32,700행. **둘을 섞으면 시간이 안 맞는다** | 새로 |
