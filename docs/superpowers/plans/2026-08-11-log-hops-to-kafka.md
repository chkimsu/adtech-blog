...
|---|---|---|---|
| 앱 SDK 메모리 큐 | 구조체 하나 | 102 B | 클릭 0초 · 노출 최대 5초 |
```
...
- Produces: 1절 102 B, 2절 79 B (실측 확정값). 태스크 6의 9절 전체 지도 표와 태스크 7의 데모 HOPS 상수가 이 두 값을 쓴다.
...
```javascript
const HOPS = [
  { key: 'sdk',       label: '앱 SDK',   bytes: 102,  ms: 0,
