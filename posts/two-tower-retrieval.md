프로덕션 광고 시스템에는 광고 후보가 수백만 개 있습니다. 요청 하나가 들어올 때 그 전부를 정밀하게 점수 낼 수는 없습니다. 먼저 범위를 크게 좁힙니다. 그다음 남은 수백 개만 실제로 정밀하게 봅니다.

그런데 유저에게 실제로 보여줄 수 있는 광고는 1~3개입니다. 수백만 개 모두에 DeepFM이나 DCN 같은 무거운 pCTR 모델을 돌리는 것은 물리적으로 불가능합니다.

> 한 줄 요약: 광고 100만 개를 다 점수 낼 수는 없다. 그래서 유저와 광고를 각각 벡터로 바꿔 두고, 곱셈 몇 번으로 수백 개까지 먼저 좁힌다.

**후보 생성(Retrieval)**은 이 수백만 개를 수백~수천 개로 줄이는 첫 관문입니다. 여기서 놓친 광고는 복구할 수 없습니다. 뒤에 아무리 정교한 랭킹 모델이 있어도 소용없습니다. 이 글은 후보 추리기(Retrieval)의 표준 방법론인 **Two-Tower Model**을 해부합니다. ANN 인덱스로 10ms 이내에 후보를 추리는 실무 아키텍처까지 다룹니다.

> 이 글의 자리: 후보 추리기는 광고 파이프라인의 첫 단계입니다. 앞뒤 단계는 다른 글에서 다룹니다.

- 파이프라인 전체 구조(Multi-Stage Ranking) → [모델 서빙 아키텍처](post.html?id=model-serving-architecture)
- 피처가 모델에 도달하는 과정 → [피처 저장소(Feature Store)](post.html?id=feature-store-serving)
...
후보 추리기 방법론은 여러 가지입니다. 그런데 프로덕션에서 실제로 쓰이는 선택지는 몇 개뿐입니다. 먼저 전체 지형을 봅니다.

| 방법 | 후보 수 처리 | 레이턴시 | 개인화 수준 | 구현 복잡도 |
|------|------------|---------|-----------|-----------|
| **규칙 기반(Rule-based)** | 수백만 → 수천 | < 1ms | 없음 (세그먼트 단위) | 매우 낮음 |
...
> 핵심 관찰: 규칙 기반(사람이 조건을 손으로 적는 방식)와 역색인(Inverted Index)는 빠르고 단순하지만 개인화에 한계가 있습니다. Two-Tower는 개인화와 레이턴시의 **최적 균형점**으로, 대부분의 프로덕션 광고 시스템에서 후보 추리기의 핵심 엔진입니다.

표의 '후보 수 처리' 열을 보면 다섯 방법 모두 결국 수백~수천 개로 줄입니다. 다른 것은 **줄이는 기준**입니다. 규칙 기반은 광고주가 미리 적어 둔 타겟 조건으로 줄입니다. Two-Tower는 유저와 광고를 벡터로 바꿔, 방향이 비슷한 쪽으로 줄입니다. 이 기준의 차이가 개인화 수준의 차이로 그대로 이어집니다.

레이턴시 열의 1~5ms도 눈여겨보세요. 광고 요청 하나에 허용된 시간이 보통 100ms 안쪽입니다. 후보 추리기가 그중 5ms를 쓰면 나머지 95ms를 뒤 단계들이 나눠 씁니다. 그래서 후보 추리기의 예산은 늘 '한 자리 ms'로 잡힙니다.

표의 숫자는 잰 값이 아니라 실무에서 흔히 보는 대략치입니다. 절대값보다 방법 사이의 순서를 보세요.

---

## 2. 왜 후보 추리기가 필요한가: 단계별 랭킹(Multi-Stage Ranking) 복습
...
> 후보 추리기에서 놓친 광고는 아무리 좋은 Ranking 모델이 있어도 복구할 수 없다. 이것이 후보 추리기가 전체 파이프라인 성능의 상한선(upper bound)을 결정하는 이유이다.
...
# 표준 라이브러리만 쓴다. 숫자는 가상이지만 실제 범위에서 크게 벗어나지 않게 잡았다.
...
그런데 후보가 100만 개로 늘면 전수 내적도 128.8ms가 되어 예산을 넘습니다. 여기서 ANN이 등장합니다. 전체의 2%만 골라 훑으면 3.4ms로 떨어집니다. 이 세 줄이 4절의 구조가 왜 그렇게 생겼는지를 전부 설명합니다.

---

## 3. 규칙 기반 후보 추리기(Rule-Based Retrieval) (Baseline)

### 타겟팅 규칙 매칭

가장 전통적인 후보 추리기 방법은 광고주가 설정한 타겟 조건으로 필터링하는 것입니다. 캠페인마다 아래와 같은 타겟 조건이 붙어 있습니다.
...
실무에서는 이 매칭을 빠르게 하려고 **역색인**(역색인)를 만들어 둡니다.
...
**유저 타워(User Tower)**는 유저 피처를 입력받아 유저 임베딩 벡터를 출력합니다.

$$u = f_{\text{user}}(x_{\text{user}}) \in \mathbb{R}^d$$

**광고 타워(Item Tower)**는 광고 피처를 입력받아 광고 임베딩 벡터를 출력합니다.

$$v = g_{\text{item}}(x_{\text{item}}) \in \mathbb{R}^d$$

두 벡터의 유사도가 후보 추리기 점수입니다.
...
여기서 한 가지가 달라집니다. 내적은 벡터의 **길이**에도 영향을 받습니다. 코사인은 길이를 나눠 버리고 **방향**만 봅니다. 같은 후보 풀에서 둘이 다른 상위 5개를 뽑습니다.
...
# 두 방식이 고른 상위 5개 집합이 같은지 확인한다. 다르면 어디서 달라졌는지 들여다본다.
...
#### 음성(Negative) Pair: 성능의 대부분을 결정하는 선택

Two-Tower 학습에서 **음성(negative) sampling 전략이 모델 성능의 대부분을 결정**합니다. 질문은 하나입니다. 어떤 광고를 "유저가 관심 없는 광고"로 규정할 것인가?
...
다만, 같은 배치 안 음성은 인기 광고가 음성으로 더 자주 등장하는 **sampling bias**가 발생합니다. 인기 광고는 많은 유저의 양성에 포함되므로, 다른 유저의 배치에서 음성으로 과대 표집됩니다. 이를 보정하지 않으면 모델이 인기 광고의 점수를 과소평가하게 됩니다.

#### 직접 구현 — in-batch 음성이 인기 광고를 깎는다

"과대 표집"이라는 말이 추상적이니 숫자로 세어 봅니다. 배치 크기가 8이라고 합시다. 한 행의 음성은 나머지 7개입니다. 클릭 로그에서 배치를 뽑으면, 클릭이 많은 광고가 배치에 더 자주 앉습니다. 그러면 그 광고는 남의 음성 자리에도 더 자주 앉습니다.
...
가장 인기 있는 `ad_A`는 음성으로 39,207번 등장했습니다. 롱테일 `ad_H`는 3,430번입니다. 11.4배 차이입니다.
...
이 편향이 얼마나 크게 나타나는지는 진짜 점수들이 얼마나 벌어져 있는지에 달려 있습니다. 점수 차가 촘촘하면 위 예시처럼 순위가 크게 흔들립니다. 노출 편향과 다운샘플링 보정까지 함께 본 전체 그림은 다른 글에 있습니다. 이어서 읽으려면 [음성 표본 뽑기(Negative Sampling)](post.html?id=negative-sampling-bias)으로 가세요.

#### Loss Function

가장 일반적인 loss는 **Softmax Cross-Entropy**입니다. 배치 내 양성 pair $(u_i, v_i^+)$에 대해 아래 식을 최소화합니다.

$$\mathcal{L} = -\sum_{i=1}^{B} \log \frac{\exp(u_i^T v_i^+ / \tau)}{\sum_{j=1}^{B} \exp(u_i^T v_j / \tau)}$$

여기서 $\tau$는 temperature parameter로, 유사도 분포의 sharpness를 조절합니다. $\tau$가 작을수록 hard 음성에 더 집중합니다.

**표본 편향 보정(Sampling Bias Correction)** 은 각 아이템의 뽑힐 확률 $p_j$를 보정합니다(Yi et al., 2019). 위 코드에서 본 $\log q$ 보정이 바로 이 식입니다.
...
### 4-3. 서빙: ANN (가까운 것 근사 검색(Approximate Nearest Neighbor))
...
1. 광고 타워로 모든 광고의 임베딩 $v_i$를 사전 계산
2. 전체 임베딩을 ANN 인덱스에 저장 (수백만 벡터)
3. 인덱스를 주기적으로 갱신 (hourly 또는 daily)

**Online 단계:**

1. 유저 요청이 들어오면 유저 타워로 유저 임베딩 $u$를 실시간 계산
2. ANN 인덱스에서 $u$와 가장 유사한 top-K 광고를 검색
3. 검색 결과를 다음 단계(Pre-Ranking)로 전달

핵심은 **서빙 시점에 광고 타워를 실행하지 않는다**는 것입니다. 수백만 광고에 대해 모델 추론을 하는 대신, 사전 계산된 벡터에서 nearest neighbor 검색만 수행합니다. 이것이 수백만 후보에서 5ms 이내에 top-K를 추출할 수 있는 이유입니다.

### 4-4. ANN 인덱스 비교

ANN(가까운 것 근사 검색)은 정확한 nearest neighbor 대신 근사 결과를 빠르게 반환합니다. 정확도(recall)와 속도를 설정값으로 조절할 수 있습니다.

| 인덱스 | 알고리즘 | 검색 시간 (1M 벡터, d=128) | 재현율(Recall)@100 | 빌드 시간 | 메모리 |
|--------|---------|--------------------------|-----------|----------|-------|
...
위 두 표의 시간과 재현율(recall)도 대략치입니다. 하드웨어와 파라미터 설정에 따라 크게 달라집니다.

**FAISS-HNSW**는 재현율이 가장 높지만 메모리를 많이 사용합니다. **ScaNN**은 Google이 만든 라이브러리입니다. anisotropic vector quantization 을 써서 재현율과 속도 모두 좋습니다. 실무에서는 FAISS-IVF와 HNSW를 조합하거나, ScaNN을 사용하는 경우가 가장 많습니다.

:::deep 더 깊이 — '근사'가 정확한 탐색과 정확히 뭐가 다른가
2절의 계산에서 100만 개 전수 내적은 128.8ms였습니다. 예산을 넘습니다. ANN은 여기서 **일부만 보고 답합니다**. 전체의 2%만 훑으면 3.4ms입니다. 대신 진짜 1위를 놓칠 확률이 생깁니다. 표의 재현율@100 90~99%가 바로 "놓치는 비율"입니다.

훑을 범위를 줄이는 방법은 두 가지입니다. **IVF**(Inverted File)는 벡터를 미리 수천 개 군집으로 나눠 둡니다. 검색할 때 유저 벡터와 가까운 군집 몇 개만 열어 봅니다. 열어 보는 군집 수(`nprobe`)가 설정값입니다. 1로 두면 빠르고 부정확하고, 32로 올리면 느리고 정확합니다. 여기에 PQ(Product Quantization)를 붙이면 벡터를 짧은 코드로 압축해 메모리를 줄입니다. 압축한 만큼 거리 계산이 부정확해집니다.

**HNSW**는 군집 대신 그래프를 만듭니다. 벡터마다 "가까운 이웃 몇 개"로 링크를 걸어 두고, 층을 쌓습니다. 위층은 링크가 드문드문해 멀리 건너뜁니다. 아래층은 촘촘해 세밀하게 좁힙니다. 검색은 위층에서 대충 방향을 잡고 아래층으로 내려오며 다듬습니다. 설정값은 `efSearch`(탐색 중 후보 큐 크기)입니다. 그래프 링크를 전부 저장해야 하니 메모리가 IVF보다 많이 듭니다.

정확한 탐색(brute force)은 후보 수에 정비례합니다. ANN은 대략 로그에 가깝게 늘어납니다. 광고가 10배 늘어도 검색 시간이 10배가 되지 않는다는 뜻입니다. 후보 추리기가 재현율 최우선인 단계라, 여기서 몇 %를 잃는 게 아깝습니다. 그래서 실무에서는 설정값을 재현율 쪽으로 맞추고, 부족하면 규칙 기반 후보를 합집합으로 얹어 메꿉니다.
:::
...
이것이 **"후보 추리기는 재현율, Ranking은 precision"**이라는 역할 분담의 근본적인 이유입니다. 후보 추리기는 정확도를 다소 희생하더라도 빠르게 넓은 후보를 확보하고, Ranking이 그 안에서 정밀하게 순위를 매기는 구조입니다.
...
$K$가 3~5이면 ANN 검색을 3~5회 해야 합니다. 그만큼 레이턴시가 늘어납니다. 대신 여러 관심사를 함께 커버해 재현율이 크게 오릅니다.
...
실무에서의 해법은 **유저 타워를 온라인에서 실시간 추론**하는 것입니다. 두 타워의 갱신 주기를 다르게 잡습니다.

- **유저 타워**: 경량 MLP로 설계하여 서빙 시점에 실시간 추론. 유저의 최근 행동(최근 클릭한 광고, 최근 검색어)을 피처로 포함
- **광고 타워**: 오프라인에서 배치로 임베딩을 계산하고 ANN 인덱스에 저장. 광고 피처는 자주 바뀌지 않으므로 주기적 갱신으로 충분

이 **비대칭 아키텍처**에서는 유저 타워의 추론 레이턴시가 전체 후보 추리기 레이턴시에 직접 영향을 줍니다. 따라서 유저 타워는 가능한 한 경량으로 설계해야 합니다. 일반적으로 2~3 layer MLP, 임베딩 차원 64~128이면 1ms 이내 추론이 가능합니다.

---

## 6. 실무 설계 가이드

Two-Tower 후보 추리기(Two-Tower Retrieval) 시스템을 만들 때 내려야 하는 핵심 결정들입니다. 아래 값은 실무에서 흔히 쓰이는 출발점입니다.

| 결정 사항 | 선택지 | 권장 |
|----------|--------|------|
| **임베딩 차원** | 64 / 128 / 256 | 128이 recall-레이턴시 균형점. 광고 수 1,000만 이하면 64도 충분 |
| **업데이트 주기** | Real-time / Hourly / Daily | 유저 타워: 실시간 추론. Item Index: hourly (새 캠페인 반영) |
| **음성 표본 뽑기** | Random / In-batch / Hard | In-batch + 표본 편향 보정이 표준. Hard 음성을 10~20% 혼합 |
| **ANN 인덱스** | FAISS-IVF / HNSW / ScaNN | 1,000만 이하: HNSW (재현율 최우선). 1억 이상: IVF+PQ (메모리 제약) |
| **후보 추리기 Top-K** | 100 / 500 / 1,000 / 5,000 | 500~1,000이 일반적. 재현율@K와 다음 단계 처리량의 균형 |
| **Temperature $\tau$** | 0.05 / 0.1 / 0.2 | 0.05~0.1. 너무 작으면 학습 불안정, 너무 크면 학습 신호 약화 |
| **Serving Infra** | CPU / GPU | 유저 타워: CPU로 충분 (경량 MLP). ANN 검색: CPU (FAISS) |

### 규칙 기반 + Two-Tower 하이브리드

실무에서는 규칙 기반 후보 추리기와 Two-Tower를 **함께** 씁니다. 순서는 아래와 같습니다.

1. **규칙 기반 필터**: 예산 소진, 타겟 불일치, frequency cap 등 비즈니스 로직으로 명백한 비후보 제거
2. **Two-Tower 후보 추리기**: 필터를 통과한 후보 풀(또는 전체 풀)에서 개인화된 top-K 추출
3. **합집합**: 두 결과의 합집합을 Pre-Ranking에 전달

규칙 기반은 비즈니스 제약 준수를 보장하고, Two-Tower는 개인화된 탐색을 담당합니다. 두 방법은 대체 관계가 아니라 보완 관계입니다.
...
후보 추리기의 핵심 지표는 **재현율@K**입니다. Top-K 후보에 유저가 실제로 클릭한 광고가 들어 있는 비율입니다.

$$\text{Recall@K} = \frac{|\{\text{클릭 광고}\} \cap \{\text{Top-K 후보}\}|}{|\{\text{클릭 광고}\}|}$$

예를 들어 유저가 클릭한 광고가 10개라고 합시다. Top-500 후보에 그중 9개가 들어 있다면 재현율@500은 90%입니다.

### 지표 비교

| 지표 | 정의 | 후보 추리기 적합성 |
|------|------|----------------|
| **재현율@K** | Top-K에 양성이 포함된 비율 | 가장 중요. 후보 추리기의 핵심 목표 |
| **Hit Rate@K** | Top-K에 양성이 1개 이상 포함된 쿼리의 비율 | 재현율@K와 유사하나 binary |
| **NDCG@K** | Top-K 내 순위까지 고려한 지표 | 후보 추리기보다 **Ranking 평가**에 적합 |
| **MRR** | 첫 양성의 역순위 평균 | 후보 추리기보다 **Ranking 평가**에 적합 |

후보 추리기 단계에서는 **순위보다 포함 여부**가 중요합니다. Top-500에서 1위든 500위든, 후보에 포함되기만 하면 Ranking 단계에서 정확한 순위를 매길 수 있습니다.

### 오프라인 평가의 한계: Serving Bias

오프라인 재현율@K 평가에는 근본적인 한계가 있습니다. 학습 데이터의 클릭은 **기존 시스템이 노출한 광고에서만** 발생했습니다. 기존 시스템이 노출하지 않은 광고 중에도 유저가 클릭했을 광고가 존재하지만, 이를 평가할 방법이 없습니다.

따라서 오프라인 재현율@K가 높다고 해서 반드시 좋은 후보 추리기는 아닙니다. 최종적으로는 **온라인 A/B 테스트**에서 CTR, 전환율, 매출 등 비즈니스 지표로 검증해야 합니다.
...
네이버·카카오처럼 담장 안에서 광고를 파는 회사는 유저를 로그인 ID로 알아봅니다. 그 ID는 어제도 오늘도 같습니다. 이 안정성이 5-3절에서 본 비대칭 구조를 바꿉니다. 광고 벡터만 미리 굽는 게 아니라, **유저 벡터도 미리 구워** 둘 수 있습니다.

효과는 2절의 계산으로 바로 읽힙니다. 유저 타워 추론에 잡아 뒀던 0.8ms가 사라집니다. 대신 캐시 조회 한 번이 들어갑니다. 10만 후보를 훑는 13.6ms 중 유저 타워가 0.8ms였으니, 예산이 조금 더 넉넉해집니다. 이 벡터를 어디에 저장하고 어떻게 꺼내오는지는 따로 볼 거리입니다. [피처 저장소](post.html?id=feature-store-serving)에서 다룹니다.
...
광고 쪽도 사정이 좋습니다. 광고 인덱스가 전부 자기 것이니, 새 캠페인이 언제 들어왔는지 정확히 압니다. 인덱스를 hourly로 다시 굽는 일정도 스스로 정합니다. 대신 4-2절에서 본 인기 광고 편향이 여기서 특히 크게 보입니다. 담장 안 트래픽은 소수의 대형 광고주에 쏠려 있어서, `ad_A`의 39,207번 같은 쏠림이 실제로 일어납니다. log-Q 보정을 빼먹으면 그 대형 광고주의 점수가 계통적으로 깎입니다.
...
열린 RTB에서 광고를 사는 쪽(DSP)은 남의 지면에서 남의 쿠키를 봅니다. 그 쿠키는 브라우저마다 다르고 수명도 짧습니다. 어제 구워 둔 유저 벡터를 찾아올 **키 자체가 없는 경우가 많습니다**. 그래서 8절과 달리 유저 타워를 요청마다 실시간으로 돌립니다. 2절에서 잡아 둔 0.8ms가 매번 예산에 실립니다.

입찰 요청 하나는 노출 한 자리에 대한 것입니다. 그래서 뽑아야 하는 후보 수 자체가 작습니다. 담장 안이 한 화면에 여러 광고를 줄 세우려 top-500을 뽑는다면, DSP는 자기 캠페인 수만 개에서 top-수십을 뽑습니다. 후보 풀이 작으니 2절의 전수 내적으로도 충분한 경우가 흔합니다. ANN 인덱스를 얹는 것보다 캠페인 풀을 잘 관리하는 편이 이득일 수 있습니다.
...
학습 데이터도 깨끗하지 않습니다. 경매에서 지면 그 노출의 클릭 여부를 못 봅니다. 그러니 학습 데이터가 자기가 이긴 노출로 편향됩니다. 4-2절의 인기 광고 편향에 이 편향이 겹칩니다. 이 문제는 [Bid Shading & 검열된 데이터](post.html?id=bid-shading-censored)에서 다룹니다. 요청이 들어와 응답이 나가기까지의 전체 흐름은 [광고 서빙 흐름](post.html?id=ad-serving-flow)에 있습니다.
...
(위 표는 가상 데이터가 아니라 두 무대의 설계 차이를 정리한 것입니다.)
...
- **내적과 코사인은 같은 답을 주지 않습니다.** 4-1절 코드에서 상위 5개가 달라졌습니다. 내적은 벡터 길이를 점수에 반영하고, 코사인은 지웁니다. 인기 쏠림을 그대로 둘지 말지의 선택입니다.
...
- **ANN의 '근사'는 버그가 아닙니다.** 재현율@100이 95%면 100개 중 5개를 놓친다는 뜻입니다. 이 손실을 감수하는 대신 100배 빠릅니다. 놓친 만큼은 규칙 기반 후보를 합집합으로 얹어 메꿉니다.
- **오프라인 재현율@K가 오르는 게 항상 좋은 건 아닙니다.** 기존 시스템이 보여준 광고 안에서만 재는 지표입니다. 7절의 serving bias 그대로입니다. 최종 판단은 온라인 A/B입니다.

---

## 마무리

1. **후보 추리기는 전체 파이프라인의 상한선** -- 수백만 후보에서 수백~수천 개를 추리는 첫 관문이며, 여기서 놓친 광고는 복구할 수 없습니다. 재현율 극대화가 최우선 목표입니다.

2. **Two-Tower Model이 현재 표준** -- 유저와 광고를 독립적으로 임베딩하고, ANN 인덱스로 5ms 이내에 검색합니다. 독립 계산이라는 제약이 오히려 서빙 효율성을 가능하게 합니다.

3. **음성 표본 뽑기가 성능을 결정** -- 같은 배치 안 음성 + 표본 편향 보정이 실무 표준입니다. 헷갈리는 음성을 적절히 혼합하면 재현율이 추가로 향상됩니다.

4. **ANN 인덱스 선택은 규모에 따라** -- 1,000만 이하는 HNSW로 재현율 극대화, 1억 이상은 IVF+PQ로 메모리와 속도의 균형을 잡으세요.

5. **규칙 기반과 Two-Tower는 보완 관계** -- 규칙 기반은 비즈니스 제약 준수, Two-Tower는 개인화 탐색을 담당합니다. 실무에서는 항상 함께 사용합니다.

> 후보 추리기는 파이프라인의 첫 단계입니다. 뒤따르는 Ranking 단계에서 DeepFM·DCN·DIN이 정밀한 스코어링을 맡습니다. 후보 추리기가 쓰는 피처는 피처 저장소가 공급합니다.
