...
[DSP·SSP·Ad Exchange](post.html?id=dsp-ssp-exchange)에서 이미 다룬 구조입니다. [광고 기술 생태계 전체 지도](post.html?id=adtech-ecosystem-map)도 함께 참고할 만합니다. 각 역할이 **독립 사업자**로 분리되어 있습니다.
...
|------|----------|---------------|
| 유저 식별 | 3rd Party Cookie (브라우저마다 차단이 늘어남) | **1st Party 로그인 ID** |
...
Walled Garden 이라고 오프라인 모델 평가가 쉬운 것은 아닙니다.

Walled Garden 에서는 **모델이 노출할 광고를 결정**하므로, 노출되지 않은 광고의 잠재 CTR 을 알 수 없습니다. 열린 RTB 의 DSP 가 패찰한 경매의 결과를 못 보는 것과 같은 구조의 문제가, 여기서는 「내 모델이 고르지 않은 광고」에 생깁니다. 이를 **Counterfactual Evaluation** 문제라고 부릅니다. 해결을 위해 다음 방법을 씁니다.
...
Open RTB 에서 DSP 가 하는 Bid Shading 은 보이지 않는 경쟁가를 추정하는 통계 문제입니다. 반면 Walled Garden 의 Bid Shading 은 **모든 입찰가를 보면서 광고주에게 최적가를 제시하는** 최적화 문제에 가깝습니다. 기술적 난이도는 다르지만, 둘 다 "1st Price 환경에서 과다 지불을 방지한다"는 목적은 동일합니다.
...
1. **Walled Garden은 데이터 통합의 힘**으로 Open RTB 대비 정확한 타겟팅과 전환 추적이 가능합니다. 3rd-party 신호가 줄어들수록 이 우위는 더 강해집니다.
