광고 경매에서 "누가 1등인가"는 단순히 돈을 많이 낸 순서가 아닙니다. 시장 유형에 따라 전혀 다른 기준으로 승자가 결정됩니다. 이 글은 eCPM의 정의와 계산법을 정리합니다. 그리고 시장 셋에서 랭킹이 어떻게 달라지는지 비교합니다. 열린 RTB(Open RTB), CPC Exchange, 닫힌 생태계(Walled Garden)입니다.

---

## 1. eCPM이란 무엇인가

eCPM은 1,000회 노출당 기대 수익입니다. effective Cost Per Mille 의 약자입니다. 광고 플랫폼이 서로 다른 과금 모델(CPM, CPC, CPA)의 캠페인을 **하나의 기준으로 비교**하기 위해 사용합니다.
...
예를 들어 pCTR이 2%, pCVR이 5%, CPA가 10,000원이라고 합시다.
...
> [광고 기술 생태계 전체 지도](post.html?id=adtech-ecosystem-map)를 참고하세요. eCPM이 전체 파이프라인에서 어디에 위치하는지 확인할 수 있습니다. DSP의 Ad Ranking 단계에서 참 가치(True Value) 계산의 핵심이 바로 eCPM입니다.
...
eCPM 기준으로는 캠페인 A가 승리했습니다(20,000 > 10,000). 사후에 측정한 실제 CPM에서도 A가 더 높았습니다(17,500 > 12,000). 그러나 두 값이 정확히 일치하지는 않습니다 -- pCTR 예측과 실제 CTR 사이에는 항상 오차가 존재하기 때문입니다. eCPM의 정확도는 **pCTR 모델의 예측 정확도**에 직접적으로 의존합니다.

---

## 3. 서로 다른 과금 모델의 정규화

현실의 광고 거래소(Ad Exchange)에서는 CPM, CPC, CPA 과금 캠페인이 **동일한 지면**을 놓고 동시에 경쟁합니다. 과금 단위가 다른 캠페인을 어떻게 공정하게 비교할 수 있을까요? 답은 **모든 캠페인을 eCPM으로 정규화**하는 것입니다.

### 세 캠페인의 경쟁 예시

| 캠페인 | 과금 모델 | 입찰 정보 | pCTR | pCVR | eCPM 계산 | eCPM |
|--------|----------|----------|------|------|-----------|------|
| A | CPM | CPM 8,000원 | - | - | CPM 그대로 사용 | **8,000원** |
...
승자는 **캠페인 C (CPA)**입니다. CPA 과금은 전환이 실제로 발생해야만 비용이 청구됩니다. 그래서 광고주 입장에서는 "비싸 보이는" 50,000원입니다. 하지만 플랫폼 입장에서 기대 수익으로 환산하면 가장 높은 값입니다.
...
### 4.1 열린 RTB: 최종 CPM 입찰가 기준 [무대: 열린 RTB]

열린 RTB 환경에서 Exchange는 각 DSP의 **최종 CPM 입찰가**만 봅니다. DSP 내부에서 pCTR을 어떻게 계산했는지, 참 가치를 어떻게 산출했는지는 Exchange가 알 수 없습니다.
...
2. **DSP가 제출하는 CPM 입찰가는 내부 eCPM과 다릅니다.** [Bid Shading](post.html?id=bid-shading-censored) 등의 전략으로 참 가치보다 낮게 입찰하기 때문입니다.

**리스크:** DSP의 pCTR 모델이 부정확하면, 가치를 잘못 평가한 입찰가를 제출하게 됩니다. pCTR을 과대 예측하면 과다입찰로 손해를 보고, 과소 예측하면 이길 수 있는 경매를 놓치는 기회손실이 발생합니다.

열린 RTB에서는 이 위험이 한 회사 안에서 끝나지 않습니다. Exchange가 보는 건 각 DSP가 제출한 최종 CPM 입찰가 하나뿐입니다. 그 뒤에는 회사마다 다른 pCTR 모델, 다른 참 가치 계산법, 다른 Bid Shading 전략이 겹겹이 쌓여 있습니다. 같은 지면에 DSP 다섯 곳이 붙으면, 다섯 개의 서로 다른 랭킹 기준이 동시에 작동하는 셈입니다. Exchange도 광고주도 각 DSP 내부의 계산 로직은 들여다볼 수 없습니다. "왜 이 광고가 이겼는가"의 답은 승자가 된 그 DSP만 알고 있습니다. 시장 전체로 보면 최종 결과는 여러 겹의 블랙박스를 통과해 나온 셈입니다.
...
### 4.3 닫힌 생태계: 플랫폼 AI 모델이 모든 것을 평가 [무대: 닫힌 생태계]

닫힌 생태계(네이버, 카카오, 구글, 메타 등)에서는 플랫폼이 **자체 pCTR 모델**을 운영합니다. 그리고 광고주의 입찰가에 pCTR 예측값을 곱한 **품질 반영 점수**로 랭킹을 결정합니다. 플랫폼이 DSP, Exchange, Publisher를 모두 겸하므로, 유저 데이터부터 광고 성과 데이터까지 모든 정보를 활용할 수 있습니다.
...
닫힌 생태계에서는 이 구조가 정반대로 굳어집니다. pCTR 모델도, Ranking Score 공식도, 여기 곱해지는 보정 계수도 전부 플랫폼 한 회사가 정합니다. 그러니 이 랭킹 기준은 단순한 기술적 선택이 아니라 사실상 정책이 됩니다. 플랫폼이 pCTR 가중치를 오늘 조금만 바꿔도, 어떤 업종이 유리해지는지가 통째로 달라집니다. 열린 RTB처럼 여러 회사의 기준이 겹쳐 결과가 흐려지는 게 아닙니다. 단 하나의 기준이 시장 전체에 그대로 적용되는 구조입니다. 광고주 입장에서는 그 기준을 예측하거나 이의를 제기할 방법이 마땅치 않습니다.

> [닫힌 생태계 포스트](post.html?id=walled-garden)를 참고하세요. 폐쇄형 생태계의 구조와 열린 RTB와의 차이를 더 깊이 확인할 수 있습니다.
...
| 비교 항목 | 열린 RTB | CPC Exchange | 닫힌 생태계 |
|----------|----------|-------------|---------------|
...
이 표에서 핵심적인 패턴이 보입니다: **오른쪽으로 갈수록 "품질"의 비중이 커지고, "돈"만으로 이기기 어려워집니다.** 열린 RTB에서는 DSP가 알아서 pCTR을 반영하지만 Exchange는 강제하지 않습니다. CPC Exchange는 Exchange가 CTR을 반영합니다. 닫힌 생태계는 플랫폼이 pCTR 을 직접 곱하여 품질이 낮은 광고에 구조적으로 불이익을 줍니다.
...
정확도가 매출에 실제로 얼마나 영향을 주는지 슬라이더로 직접 흔들어 볼 수 있습니다.

<div class="demo-embed-wrap">
<iframe class="demo-embed" src="demo-pctr-impact.html?embed=1" height="560" loading="lazy" title="pCTR 정확도와 매출 영향 미니 데모"></iframe>
<a class="demo-embed-open" href="demo-pctr-impact.html" target="_blank" rel="noopener">↗ 전체 데모로 열기</a>
</div>
...
| QPS | 수만~수십만 | 초당 처리해야 할 입찰 요청(Bid Request) 수 |
...
이것이 닫힌 생태계 플랫폼들이 "품질 점수"를 강조하는 이유입니다. 광고주가 품질을 개선하면 -> pCTR이 오르면 -> 같은 입찰가로 더 높은 랭킹 -> 더 많은 노출 -> 플랫폼도 더 많은 수익. **양쪽 모두에게 이득**인 구조입니다.
...
:::deep 더 깊이 — 왜 랭킹 점수를 로그 공간에서 계산하는가

위 Final Score 공식은 eCPM에 보정 계수 세 개(α, β, γ)를 곱한 값입니다. eCPM 자체도 CPA 캠페인이라면 이미 pCTR × pCVR × CPA × 1,000처럼 여러 항의 곱입니다. 실무 랭킹 점수는 이렇게 대여섯 개의 확률·계수를 한 번에 곱해 넣는 경우가 흔합니다.

곱셈을 그대로 쌓으면 두 가지가 불편해집니다. 첫째, 1보다 작은 값을 여러 번 곱하면 결과가 0에 빠르게 가까워집니다. 부동소수점은 0 근처에서 정밀도를 잃기 쉽습니다. 그러면 광고 사이의 점수 차이가 반올림 오차에 묻힐 수 있습니다. 둘째, 로그우도(log-odds)로 값을 내놓는 모델의 출력을 곱셈식에 그대로 섞어 쓰기가 어색합니다.

그래서 실무에서는 점수를 곱하는 대신 **로그를 취해 더합니다.**

$$\log(\text{Final Score}) = \log(eCPM) + \log(\alpha_{\text{diversity}}) + \log(\beta_{\text{pacing}}) + \log(\gamma_{\text{policy}})$$

이 변환이 안전한 이유는 로그가 **단조증가 함수**라서입니다. A가 B보다 크면 log A는 항상 log B보다 큽니다. 그러니 로그 공간에서 매긴 순위는 원래 점수의 순위와 정확히 같습니다. 1등이 바뀔 걱정 없이 계산만 곱셈에서 덧셈으로 바뀝니다.

덧셈으로 바꾸면 실무에 두 가지 이득이 생깁니다. 부동소수점이 0 근처에서 잃는 정밀도를 피할 수 있습니다. 그리고 보정 계수를 하나 추가할 때 항 하나만 더하면 되고, 기존 항은 다시 계산할 필요가 없습니다. 그래서 대규모 랭킹 시스템의 점수 계산기는 겉보기 공식과 달리, 실제로는 로그값을 더하는 코드로 구현되곤 합니다.
:::
...
이러한 한계를 보완하기 위해 일부 플랫폼은 클릭 이외의 신호도 함께 봅니다. 체류 시간, 스크롤 깊이, 영상 시청 완료율 같은 신호를 eCPM 계산에 반영하는 방향입니다.

### ⑤ 기준을 바꾸면 승자가 바뀐다

②·③에서 본 한계를 숫자로 확인해 보겠습니다. 같은 광고 후보 4개에 랭킹 기준 3가지를 그대로 적용합니다. 순수 eCPM, 품질 가중, LTV 기준입니다. 품질 가중은 ③의 Final Score 아이디어를 단순화한 것입니다. eCPM에 품질계수 하나만 곱합니다. LTV 기준은 [LTV 포스트](post.html?id=ltv-ad-ranking)의 정의를 그대로 씁니다. eCPM에서 사용자 비용(β)을 뺍니다.

| 후보 | pCTR | 입찰가 | 순수 eCPM | 품질계수 | 품질 가중 점수 | 사용자 비용(β) | LTV 점수 |
|---|---|---|---|---|---|---|---|
| A(반짝세일 쿠폰) | 8.0% | 500원 | 40,000 | 0.60 | 24,000 | 32,000원 | 8,000 |
| B(구독 서비스) | 2.5% | 1,400원 | 35,000 | 0.90 | 31,500 | 500원 | **34,500** |
| C(대출 광고) | 3.0% | 2,400원 | **72,000** | 0.40 | 28,800 | 48,000원 | 24,000 |
| D(지역 맛집) | 5.0% | 750원 | 37,500 | 1.20 | **45,000** | 6,000원 | 31,500 |

결과를 보면 기준마다 다른 광고가 1등입니다. 순수 eCPM 기준으로는 대출 광고(C)가 이깁니다. 입찰가가 가장 높기 때문입니다. 품질 가중 기준으로는 지역 맛집(D)이 이깁니다. eCPM은 중간이지만 품질계수가 가장 높습니다. LTV 기준으로는 구독 서비스(B)가 이깁니다. 순수 eCPM은 네 후보 중 가장 낮지만, 사용자 비용(β)이 가장 작아 장기 순가치가 가장 큽니다.

```python
# 같은 광고 후보 4개에 랭킹 기준 3가지를 적용해 1등이 바뀌는지 확인한다.
# 순수 eCPM(돈) / 품질 가중(eCPM x 품질계수) / LTV 기준(eCPM - 사용자 비용)
ads = {
    "A(반짝세일 쿠폰)": {"pctr": 0.080, "cpc": 500,  "quality": 0.60, "beta": 32_000},
    "B(구독 서비스)":   {"pctr": 0.025, "cpc": 1400, "quality": 0.90, "beta": 500},
    "C(대출 광고)":     {"pctr": 0.030, "cpc": 2400, "quality": 0.40, "beta": 48_000},
    "D(지역 맛집)":     {"pctr": 0.050, "cpc": 750,  "quality": 1.20, "beta": 6_000},
}

def ecpm(a):
    return a["pctr"] * a["cpc"] * 1000   # 순수 eCPM: pCTR x Bid x 1,000 (1절)

criteria = {
    # 세 기준 모두 같은 eCPM에서 출발하지만, 그 위에 더하는 항이 다르다.
    "순수 eCPM": lambda a: ecpm(a),                    # eCPM 그대로 -- 돈이 전부
    "품질 가중":  lambda a: ecpm(a) * a["quality"],    # x 품질계수(③ Final Score 아이디어 단순화)
    "LTV 기준":  lambda a: ecpm(a) - a["beta"],        # - 사용자 비용(beta) -> LTV 포스트 참고
}

for name, score_fn in criteria.items():
    scores = {ad: score_fn(v) for ad, v in ads.items()}
    ranked = sorted(scores, key=scores.get, reverse=True)
    line = "  ".join(f"{ad}={scores[ad]:,.0f}" + (" <- 1위" if ad == ranked[0] else "")
                     for ad in ranked)
    print(f"{name:10s}: {line}")

# 출력:
# 순수 eCPM : C(대출 광고)=72,000 <- 1위  A(반짝세일 쿠폰)=40,000  D(지역 맛집)=37,500  B(구독 서비스)=35,000
# 품질 가중  : D(지역 맛집)=45,000 <- 1위  B(구독 서비스)=31,500  C(대출 광고)=28,800  A(반짝세일 쿠폰)=24,000
# LTV 기준  : B(구독 서비스)=34,500 <- 1위  D(지역 맛집)=31,500  C(대출 광고)=24,000  A(반짝세일 쿠폰)=8,000
```

세 기준 다 "말이 되는" 규칙입니다. 그런데 승자가 매번 바뀝니다. 대출 광고처럼 입찰가만 높은 광고는 품질·LTV를 반영하는 순간 밀려납니다. 반대로 구독 서비스처럼 당장의 돈은 적어도 오래 남는 광고는 LTV 기준에서만 1등을 합니다. 어떤 기준을 쓸지 정하는 것 자체가 플랫폼의 가치판단입니다. LTV 계산의 구체적인 방법은 [LTV 포스트](post.html?id=ltv-ad-ranking)에서 더 다룹니다.
...
3. **시장 유형에 따라 "누가 품질을 평가하는가"가 달라집니다.** 열린 RTB에서는 DSP가 내부적으로 pCTR을 반영하고, CPC Exchange에서는 Exchange가 과거 데이터로 판단하며, 닫힌 생태계에서는 플랫폼의 AI 모델이 절대적 권한을 가집니다.

4. **오른쪽으로 갈수록 품질의 비중이 커집니다.** 열린 RTB에서는 돈만으로 이길 수 있지만, 닫힌 생태계에서는 입찰가가 3배 높아도 pCTR이 낮으면 패배합니다. 이 구조는 광고 품질 개선의 인센티브를 만듭니다.

5. **eCPM의 정확도는 pCTR 모델의 정확도에 직접 의존합니다.** 플랫폼이든 DSP이든, 좋은 pCTR 모델을 가진 쪽이 더 나은 랭킹 결정을 내리고, 궁극적으로 더 높은 수익을 얻습니다.

DSP가 1st Price Auction에서 eCPM을 기반으로 입찰가를 최적화하는 방법이 따로 있습니다. 구체적인 내용은 [Bid Shading 포스트](post.html?id=bid-shading-censored)에서 다룹니다. eCPM으로 "얼마의 가치가 있는가"를 먼저 계산합니다. 그다음 Bid Shading으로 "얼마를 실제로 입찰할 것인가"를 결정합니다. 이것이 DSP 입찰 전략의 전체 그림입니다.

---

## 더 깊이 보기

- pCTR이 입찰가·순위를 결정하는 과정 → [pCTR: 누를 확률이 왜 돈이 되는가](post.html?id=pctr-prediction)
