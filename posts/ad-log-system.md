...
후보 로그(Candidate Log) 는 후보로 뽑힌 광고를 남기는 기록입니다. 앞쪽에서는 **후보 로그**의 역할, **실시간 피처 파이프라인**, 멀티슬롯 **rank=1 추론 문제**를 봅니다. 뒤쪽에서는 로그를 잃지 않고 옮기는 수집 계층과, 필드가 바뀌어도 어제 로그가 안 깨지는 스키마 규칙을 다룹니다.

---

| 로그 | 생성 시점 | 주요 용도 |
|------|----------|----------|
| **요청 로그(Request Log)** | 광고 요청 시 | 트래픽 분석, 빈 자리(No-Fill) 추적 |
| **후보 로그** | 랭킹 시 | 모델 학습, 오프라인 평가 |
| **노출 로그(Impression Log)** | 노출 시 | 과금, CTR 라벨링 |
| **클릭 로그(Click Log)** | 클릭 시 | pCTR 학습 라벨 |
| **전환 로그(Conversion Log)** | 전환 시 | pCVR 학습 라벨, ROAS 측정 |
...
요청 로그(요청이 들어온 것을 남기는 기록)는 단순한 "트래픽 분석, 디버깅" 이상의 역할을 합니다:

- **`request_id` 생성**: 이후 모든 로그를 잇는 **조인 키의 출발점**. 없으면 연결 고리가 끊어집니다.
- **빈 자리(채울 광고가 없어 빈 자리로 끝난 것) 분석의 유일한 소스**: 빈 응답을 준 요청은 노출 로그(광고가 실제로 그려진 것을 남기는 기록)에 안 남습니다. `Fill Rate = Impressions / Requests`의 **분모**가 이 로그입니다.
- **필터링·모니터링 기준점**: 봇 탐지, 빈도 캡(frequency capping), 사전 타겟팅 필터의 결정과 QPS(초당 요청 수)·응답 시간이 남습니다.
- **상황 피처(Context Feature)의 원천**: 시간대·지역·디바이스 같은 Request-level 컨텍스트가 학습 때 **context feature**로 쓰입니다.
...
요청 로그의 고유한 가치 중 하나는 **"왜 광고가 안 나왔는지"**를 추적할 수 있다는 점입니다:
...
| `filtered` | 봇/정책 필터링 | IVT(무효 트래픽(Invalid Traffic)) 비율 모니터링 |

> 채움률(Fill Rate)가 낮으면 **매출 기회 손실**입니다. 원인을 잡으려면 `response_status`와 `filtering_reason` 분석이 필수입니다.

#### 볼륨

요청 로그는 **가장 볼륨이 큽니다**. 전수 기록하고, 나머지 로그는 전부 그 부분 집합입니다. 아래로 갈수록 깔때기처럼 줄어 전환 로그는 요청의 수만 분의 1까지 내려갑니다. 이 차이가 저장소와 보존 기간을 로그마다 다르게 만듭니다. 하루 행 수와 저장 용량 계산은 [형제 글의 깔때기 계산](post.html?id=ad-log-pipeline)에 있습니다.

### 후보 로그

랭킹 단계에서 **스코어링된 모든 후보**를 기록합니다. **탈락한 광고까지 전부** 들어갑니다. 다음 절에서 자세히 봅니다.

### 노출(Impression) · 클릭(Click) · 전환 로그

경매 결과와 유저 행동이 차례로 남는 세 로그입니다. 뒤로 갈수록 건수가 줄고, 대신 한 건의 값이 비싸집니다.

| 로그 | 담기는 것 | ML에서의 쓰임 |
|------|---------|-------------|
| **노출** | 낙찰 광고·광고주 ID, 노출 위치, 낙찰가·과금액, viewability | pCTR 학습의 분모 |
| **클릭** | 클릭된 광고 ID, 클릭 시각, 노출~클릭 간격(dwell time) | pCTR의 positive label |
| **전환(Conversion)** | 전환 종류(purchase·sign_up·install), 금액, 클릭~전환 지연 | pCVR label, ROAS 측정 |

> 전환 로그(구매나 설치가 일어난 것을 남기는 기록)는 클릭 후 **수 시간~수 일** 뒤에 옵니다. 이게 지연 피드백(Delayed Feedback) 문제의 원인입니다. 자세히는 [온라인 학습(Online Learning) 과 지연 피드백](post.html?id=online-learning-delayed-feedback)에서 다룹니다.

---

## 2. 후보 로그 상세 해부

후보 로그는 랭킹 단계에서 **스코어링된 모든 후보 광고의 스냅샷**입니다. 노출 로그가 낙찰된 광고 하나만 기록한다면, 후보 로그는 점수를 받은 후보 전체를 기록합니다.
...
후보 로그의 가장 큰 도전은 **볼륨**입니다. 노출 로그는 요청당 1건(낙찰 광고)이지만, 후보 로그는 요청당 수십~수백 건(후보 전체)입니다. 같은 트래픽에서 저장량이 수십~수백 배로 뜁니다.
...
## 3. 후보 로그가 있을 때 vs 없을 때

후보 로그 도입 여부는 모델 학습 품질에 곧바로 영향을 줍니다.

| 관점 | 후보 로그 없음 | 후보 로그 있음 |
|------|-------------------|-------------------|
| **음성 표본(Negative Sample)** | 노출 후 미클릭만 | 랭킹 탈락 광고도 negative로 |
...
노출 로그만으로 학습하면 모델은 **"이미 경쟁력 있는 광고들 사이의 미세한 차이"**만 배웁니다. Retrieval에서 걸러진 광고, 랭킹에서 탈락한 광고의 패턴은 배우지 못합니다. 이것이 [Negative Sampling](post.html?id=negative-sampling-bias)에서 다룬 구조적 편향의 원인입니다.

후보 로그가 있으면 "탈락한 광고가 왜 탈락했는지"까지 학습할 수 있습니다. 모델의 **판별력(discrimination)**이 크게 올라갑니다.
...
pCTR 학습 데이터는 노출에 클릭을 붙여 `label=1/0`을 만듭니다. pCVR은 클릭에 전환을 붙여 만듭니다. 후보 로그가 있으면 negative sample이 요청당 1건에서 수십~수백 건으로 넓어집니다.
...
로그 종류를 정하는 일보다 어려운 게 **조인 시각을 정하는 일**입니다. 전환은 클릭 후 수 시간~수 일 뒤에 도착합니다. 그래서 너무 일찍 조인하면 "아직 전환 안 한 건"과 "앞으로도 안 할 건"이 구분되지 않고 전부 `label=0`이 됩니다. 이것이 지연 피드백 문제가 실무에서 나타나는 모습입니다.
...
갱신 주기가 다른 이유는 값이 변하는 속도가 다르기 때문입니다. 유저의 최근 10분 클릭 수는 초 단위로 바뀌지만, 과거 30일 구매 이력은 하루에 한 번만 움직입니다. 초 단위 피처를 배치로 만들면 모델이 방금 일어난 행동을 못 봅니다. 반대로 배치 피처를 요청마다 실시간으로 계산하면 같은 값을 매번 다시 세게 됩니다.
...
| 유저 과거 30일 구매 이력 | Batch 집계 | 1일 1회 | 피처 저장소(Feature Store) |
| 광고 임베딩 벡터 | 모델 학습 후 추출 | 배포 시 | 피처 저장소 |

### 세 갈래 피처의 결합

추론 시 **Batch + Streaming + Real-Time** 세 갈래 피처가 하나의 피처 벡터(Feature Vector) 로 합쳐집니다. 피처 벡터는 모델에 넣는 숫자 묶음입니다.
...
> 피처 파이프라인의 전체 아키텍처는 [피처 저장소](post.html?id=feature-store-serving)에서 상세히 다룹니다.
...
**학습 시**: 모델은 실제 자리(position)을 피처로 씁니다. 1번 자리 광고는 CTR이 높고 3번은 낮습니다. 모델은 이 차이를 배웁니다.

**추론 시**: 아직 자리가 없습니다. 누구를 1번에 놓을지 정하려면 스코어링을 해야 하는데, 스코어링하려면 자리가 필요합니다. **서로가 서로를 먼저 필요로 하는 순환 문제**입니다.

### 왜 rank=1로 고정하는가

실무에서 가장 흔한 해법은 **모든 후보를 자리=1로 고정**하여 스코어링하는 것입니다:
...
| 문제 | 설명 |
|------|------|
| **Calibration 깨짐** | pCTR이 전부 과대추정됨. 자리=3에 놓일 광고도 1 기준으로 예측 → eCPM 왜곡 |
| **비용 vs 정확도** | slot 1 낙찰 후 나머지를 자리=2로 재스코어링해야 맞지만, 추론 비용이 슬롯 수만큼 증가 |
| **부익부빈익빈** | 보정 없이 학습하면 "좋은 위치 → CTR 높음 → 다시 좋은 위치" 순환이 강화됨 |

### 실무 해법

#### 1. 자리 피처(Feature) 분리 (Examination Hypothesis)

모델 구조에서 **자리의 영향을 분리**하여, 추론 시 position-free 스코어를 사용합니다:
...
학습 때는 자리 정보를 쓰고, 추론 때는 자리에 기대지 않는 공정한 스코어를 씁니다.

:::deep 더 깊이 — IPS(Inverse Propensity Scoring)로 학습 데이터를 되돌리기
자리별 "examination probability"의 역수를 가중치로 써서 학습 데이터의 자리 bias를 보정합니다.

$$w_i = \frac{1}{P(\text{examine} | \text{position}_i)}$$

자리=1의 가중치는 낮게 잡습니다. 어차피 눈에 잘 띄는 자리니까요.
자리=3의 가중치는 높게 잡습니다. 잘 안 보이는데도 클릭했다면 정말 좋은 광고입니다.
:::

#### 2. 사후 보정 계수

rank=1로 추론한 스코어에 자리별 보정 계수를 곱해 실제 pCTR을 추정합니다. 계수는 사전에 통계로 뽑아 둡니다. 예를 들어 1번 자리 `1.0`, 2번 `0.65`, 3번 `0.40`을 쓴다고 하겠습니다. 3번에 놓일 광고의 pCTR은 rank=1 스코어의 40%로 내려 잡습니다.

> 자리 편향(Position Bias)의 이론과 보정 기법은 [자리 편향](post.html?id=position-bias-ultr)에서 다룹니다.
...
난이도는 무대에 따라 다릅니다. 담장 안은 노출·클릭·전환이 모두 자사 서버에 남습니다. 스키마를 한 팀이 통제하고 조인 키도 내가 만든 것이라 확실해서, 필드 변경은 사내 합의 한 번으로 끝납니다.

열린 RTB는 로그가 SSP·자사 서버·광고주 포스트백으로 흩어져 있습니다. 스키마 합의 자체가 회사 간 협상이 되고, 남이 보내주는 필드는 예고 없이 바뀝니다. 그래서 **내가 만드는 필드와 남이 주는 필드를 나눠 두고**, 남이 주는 쪽은 전부 nullable로 잡습니다.
...
- **Opportunity 로그(Log)**: 후보 로그와 비슷. 노출 기회마다 **후보군 전체의 스코어를 기록**
- **실시간 피처**: 자체 피처 저장소에서 최근 N분 인게이지먼트를 실시간 집계
- **이중 전환 수집**: 전환을 **서버사이드(CAPI)** + **클라이언트 픽셀** 양쪽에서 받아 누락 방지
...
- 요청 로그에 해당하는 **Ad Serving 로그**에 타임라인 위치, 트윗 컨텍스트, **팔로우 그래프 피처**를 함께 담음
- **Earlybird(검색 인덱스)** 단계의 리트리벌 로그도 따로 수집해 후보 생성 과정까지 추적

### LinkedIn

- **Unified Logging Framework**: 광고 이벤트를 **Kafka → Brooklin → HDFS**로 통합 수집
- 요청(Request) 단계에서 **member features snapshot**을 따로 저장 — 프로필이 바뀌어도 학습 데이터가 안 흔들림
- B2B 특성상 **company-level aggregated features**(회사 규모, 업종, 직급 분포)가 요청 로그에 포함

### Criteo (리타게팅 DSP)

- 공개 논문·데이터셋으로 로그 구조를 가장 상세히 밝힌 회사
- **Display 로그**: 요청 + 후보(Candidate) + 노출을 한 로그에 통합. **과거 상품 조회 시퀀스**가 요청 피처로
...
| **피처 Snapshot** | 추론 시점 피처 값을 로그에 남겨 학습 데이터를 일관되게 | Meta, LinkedIn, Criteo |
| **후보 분리 저장** | 후보군 전체 로그는 **샘플링해 별도 저장** | Meta, Google |
| **서버 + 클라이언트 이중 수집** | 노출/클릭을 양쪽에서 받아 누락 방지 | Google, Meta |
...
| 설계 결정 | 영향 |
|----------|------|
| **후보 로그 도입 여부** | 음성 표본 품질 → 모델 판별력 |
| **실시간 피처 파이프라인** | 피처 신선도 → 모델 예측 정확도 |
| **자리 처리 방식** | Calibration 품질 → eCPM/입찰 정확도 |
| **전환 로그 조인 타이밍** | 지연 피드백 처리 → pCVR 정확도 |
...
[Ad Tech 개발 레이어](post.html?id=adtech-dev-layers)에서 "측정 · 어트리뷰션 → 예측 모델"로 향하는 피드백 루프 — 그것이 바로 이 로그 파이프라인입니다.
