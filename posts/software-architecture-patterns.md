...
|---|---|---|---|
| 직접 호출 → 이벤트 | 주문 가용성 99.2% → 99.85% (연 69.9→13.1시간) | "아직 안 된 일"이 남는다 (알림 30분 장애 = 6천 건 밀림) | §1 |
| 모놀리식 → 마이크로서비스 | 배포 1회 영향 100만 → 12.5만 명 (8배) | 사고 건수 0.6 → 0.8건, 관리할 "사이" 190개 | §4 |
| 단일 DB → 복사본 | 읽기 95%일 때 복사본 6대로 12,000 QPS 소화 | 복제 지연 (내가 쓴 글이 안 보임) | §6 |
