네이버 검색광고, 카카오 비즈보드, 쿠팡 리테일 미디어. 한국 디지털 광고 시장에서 가장 큰 매출을 만드는 플랫폼들입니다. 이 플랫폼들은 하나같이 **DSP, SSP, 광고 거래소(Ad Exchange), Publisher, DMP를 모두 자사 안에** 가지고 있습니다. 이런 구조를 업계에서는 **닫힌 생태계(Walled Garden)**이라 부릅니다.

이 글에서는 열린 RTB(Open RTB) 생태계와 닫힌 생태계의 구조적 차이를 분석합니다. 그리고 엔지니어 관점에서 각각이 pCTR 모델링, 입찰 전략, 데이터 활용에 어떤 영향을 미치는지 해부합니다.

---

## 1. 열린 RTB vs 닫힌 생태계: 구조 비교

**같은 배너 광고 하나가 뜹니다. 한쪽은 회사 네 곳의 문턱을 넘고, 한쪽은 한 회사 안에서 끝납니다.** 이 구조 차이가 이 글 전체를 관통합니다.

### 열린 RTB 생태계 (분산형)

[DSP·SSP·광고 거래소](post.html?id=dsp-ssp-exchange)에서 이미 다룬 구조입니다. [광고 기술 생태계 전체 지도](post.html?id=adtech-ecosystem-map)도 함께 참고할 만합니다. 각 역할이 **독립 사업자**로 분리되어 있습니다.

예를 들어 조선일보 지면에 배너 하나가 뜨려면 여러 회사를 거칩니다. 광고주 → DSP(The Trade Desk) → 광고 거래소(Google AdX)로 이어집니다. 그다음 → SSP(Magnite) → Publisher(조선일보) 순서로 도착합니다. 이 과정에서 **네 번의 회사 경계**를 넘습니다. 경계를 넘을 때마다 계약이 따로 있고, 수수료도 따로 붙습니다. 이 수수료가 실제로 얼마나 쌓이는지는 이미 계산해 봤습니다. [DSP·SSP·광고 거래소 글](post.html?id=dsp-ssp-exchange)에서 단계별로 확인할 수 있습니다.
...
네이버, 카카오, 구글, 메타 등은 이 모든 역할을 **하나의 회사**가 수행합니다. 네이버 안에서는 지면도, 경매도, 결제 데이터도, 회원 데이터도 한 회사 것입니다. 다른 회사의 지면을 빌리거나 다른 회사의 데이터를 사 올 필요가 없습니다.

경계가 없으니 회사 간 계약도, 그 사이에서 새는 수수료도 없습니다. 대신 부서 간 데이터 공유는 계약이 아니라 **내부 API 호출 한 번**으로 끝납니다. pCTR 모델을 만드는 엔지니어 입장에서는 이런 뜻입니다. DSP·SSP·Exchange가 따로 쌓던 로그를, 하나의 파이프라인에서 그대로 이어받습니다. 이 데이터 통합이 실제로 돈으로 어떻게 이어지는지는 8절에서 숫자로 계산해 봅니다.
...
| 구분 | 열린 RTB | 닫힌 생태계 |
|------|----------|---------------|
| 유저 식별 | 3rd 자사 쿠키(Party Cookie) (브라우저마다 차단이 늘어남) | **1st Party 로그인 ID** |
...
이것이 pCTR 모델에 주는 의미:
- **Feature 풍부도**: 열린 RTB의 DSP가 사용하는 피처가 수십 개라면, 닫힌 생태계는 수백~수천 개의 1st party 피처를 활용 가능
- **라벨 정확도**: 전환 추적이 자사 시스템 안에서 완결되므로 지연 피드백(Delayed Feedback), Attribution 문제가 크게 완화됨
- **Privacy 내성**: 3rd 자사 쿠키 차단(Safari, Firefox)과 ATT 의 영향을 거의 받지 않음
- **실험 속도**: 홀드아웃(A/B 테스트)을 자사 트래픽에 바로 걸 수 있어, 효과를 실험으로 직접 확인하는 주기가 짧음 ([어트리뷰션 입문](post.html?id=attribution-basics)의 담장 안 측정 참고)

### ② 경매 구조: 내부 경매의 특수성

열린 RTB에서는 여러 DSP가 Exchange를 통해 경쟁합니다. 닫힌 생태계에서는 **같은 플랫폼 내의 광고주끼리만** 경쟁합니다.
...
- **Bid Shading 불필요**: 대부분 GSP(Generalized Second-Price) 또는 VCG 방식을 사용. 네이버 검색광고는 바로 아래 순위의 광고주를 이기는 데 필요한 최소 금액에 조금만 더해 과금하는 구조
- **pCTR의 역할 변화**: 열린 RTB에서 pCTR은 참 가치(True Value) 계산의 입력이지만, 닫힌 생태계에서는 **랭킹 점수 자체의 핵심 요소**. pCTR이 높으면 낮은 CPC로도 상위 노출 가능
- **경쟁 범위 제한**: 같은 키워드/타겟팅을 설정한 광고주끼리만 경쟁. 열린 RTB처럼 수백 개 DSP가 동시 입찰하는 상황이 아님

### ③ 정보 비대칭의 해소

열린 RTB에서 DSP 엔지니어가 겪는 핵심 난관들은 다음과 같습니다. 닫힌 생태계에서는 이 난관들이 구조적으로 해소됩니다.

| 열린 RTB의 난관 | 원인 | 닫힌 생태계에서의 해결 |
|----------------|------|------------------------|
| **가려진 데이터(Censored Data)** | 패찰 시 시장가 미관측 | 내부 경매이므로 **모든 입찰가 관측 가능** |
| **Selection Bias** | 낙찰한 광고만 피드백 수집 | 노출 순위별 클릭 데이터 수집 → **자리 편향(Position Bias) 보정**으로 전환 |
| **지연 피드백** | 3rd party 전환 추적 지연 | 자사 결제 시스템과 직접 연동 → **실시간 전환 확인** |
| **Cross-device 추적** | Cookie 기반 확률 매칭 | 로그인 ID 기반 → **로그인한 기기끼리는 확정 매칭** |
| **Feature 제한** | 입찰 요청의 제한된 정보 | **검색 쿼리 + 행동 이력 + 구매 데이터 통합** |

---

## 3. 닫힌 생태계 내부의 pCTR 모델링 차이

### ① 피처 설계: 검색 의도가 핵심

열린 RTB의 디스플레이 광고에서는 유저 프로필(나이, 성별, 관심사)과 지면 정보가 주요 피처입니다. 반면 네이버/카카오 같은 검색 광고 기반 닫힌 생태계에서는 **검색 쿼리의 의도(intent)**가 가장 강력한 피처입니다.

| 피처 카테고리 | 열린 RTB (디스플레이) | 닫힌 생태계 (검색 광고) |
|-------------|---------------------|-------------------------|
...
닫힌 생태계의 검색 광고에서는 **노출 위치(Position)**가 CTR에 막대한 영향을 미칩니다. 1위 광고는 위치 자체의 이점으로 높은 CTR을 받고, 5위 광고는 같은 품질이라도 낮은 CTR을 보입니다.

이를 분리하기 위해 닫힌 생태계의 pCTR 모델은 보통 다음과 같이 설계됩니다.
...
이 분리가 중요한 이유는 이렇습니다. 랭킹 시에는 자리 편향을 제거한 **순수 광고 품질 점수**로 순위를 매겨야 합니다. 위치 효과를 제거하지 않으면 강화 루프(rich-get-richer)가 생깁니다. "1위라서 클릭이 많았고, 클릭이 많으니까 1위를 유지하는" 악순환입니다.

### ③ 학습 데이터: 노출되지 않은 광고는 평가할 수 없다

닫힌 생태계 이라고 오프라인 모델 평가가 쉬운 것은 아닙니다.

닫힌 생태계 에서는 **모델이 노출할 광고를 결정**하므로, 노출되지 않은 광고의 잠재 CTR 을 알 수 없습니다. 열린 RTB 의 DSP 가 패찰한 경매의 결과를 못 보는 것과 같은 구조의 문제가, 여기서는 「내 모델이 고르지 않은 광고」에 생깁니다. 이를 **반사실 채점(Counterfactual Evaluation)** 문제라고 부릅니다. 해결을 위해 다음 방법을 씁니다.
...
**닫힌 생태계이라는 이름표는 같아도, 안을 들여다보면 저마다 다른 데이터를 무기로 삼습니다.** 한국 3사와 글로벌 4사를 나란히 놓고 비교해 봅니다.
...
이 중 **쿠팡이 pCTR·pCVR 모델링에 가장 유리한 위치**에 있다는 평가가 많습니다. 검색부터 클릭, 구매까지 전 과정이 하나의 앱 안에서 끊기지 않기 때문입니다. 네이버·카카오도 통합 데이터를 갖고 있습니다. 다만 구매 데이터의 완전성은 네이버페이·카카오페이 연동 여부에 따라 달라집니다.
...
네 회사의 **무기가 뚜렷하게 다릅니다.** Google은 검색 의도, 즉 무엇을 찾는가를 가장 강하게 쥐고 있습니다. Meta는 관계망, 즉 누구와 연결되어 있는가를 쥐고 있습니다. Amazon은 구매 이력, 즉 무엇을 실제로 사는가를 쥐고 있습니다.

흥미로운 쪽은 Amazon입니다. 검색 광고의 강자 Google보다 매출 규모는 작습니다. 하지만 **구매 직전 데이터**라는 면에서는 오히려 더 유리하다는 평가를 받습니다. 지갑을 여는 순간과 가장 가까운 신호이기 때문입니다.

Apple의 ~$10B은 공식 공시 항목이 아니라 업계 추정치입니다. Apple은 광고 매출을 별도로 공개하지 않습니다. 다만 ATT 이후 **경쟁사의 데이터는 제한하면서 자사 광고 사업은 키우는** 행보가 뚜렷합니다. 이 배경은 8절의 심화 박스에서 더 다룹니다.

---

## 5. 닫힌 생태계의 한계와 트레이드오프

닫힌 생태계가 모든 면에서 우월한 것은 아닙니다.

### ① 수요 경쟁의 부재

열린 RTB에서는 수백 개 DSP가 동시 입찰하여 매체 수익을 극대화합니다. 닫힌 생태계는 자사 광고주만 참여하므로, **매체 관점에서 경쟁 입찰가가 낮을 수 있습니다**. 이것이 네이버가 외부 광고 네트워크(GFA 등)를 별도로 운영하는 이유 중 하나입니다.

### ② 광고주 Lock-in

광고주 입장에서 닫힌 생태계는 **데이터 이동이 불가능**합니다. 네이버에서 쌓은 캠페인 데이터와 최적화 결과를 카카오로 가져갈 수 없습니다. 이로 인해:
- 플랫폼 간 성과 비교가 어려움 (각자 다른 기준으로 리포팅)
- **Attribution 전쟁**: 각 플랫폼이 자사에 유리하게 전환을 집계하려는 동기

### ③ 투명성 문제

열린 RTB에서는 DSP가 입찰 요청/Response를 직접 제어하고 경매 결과를 검증할 수 있습니다. 닫힌 생태계에서는 플랫폼이 **경매 알고리즘, 품질 점수, 과금 로직**을 모두 통제하며 외부에 공개하지 않습니다.

광고주는 "왜 내 광고가 3위인지", "품질 점수가 어떻게 계산되는지"를 정확히 알 수 없습니다. 이 블랙박스 구조는 플랫폼에 대한 신뢰 문제로 이어질 수 있습니다.

### ④ 지면 선택의 폭, 그리고 담장의 잠식 [무대: 열린 RTB]

열린 RTB의 근본적인 강점은 **선택의 폭**입니다. DSP 하나만 붙이면 수만 개 매체의 지면에 접근할 수 있습니다. 닫힌 생태계 광고주는 그 회사의 자사 매체 안에서만 광고를 집행할 수 있습니다.
...
실제로는 순수한 열린 RTB나 순수한 닫힌 생태계보다 **하이브리드 구조**가 주류입니다.
...
- **구글**: Google Ads 검색(Walled Garden) + Google Display Network + AdX(열린 RTB Exchange)

이 구조에서 플랫폼은 **자사 매체의 프리미엄 인벤토리**는 내부 경매로 수익을 극대화합니다. 동시에 **외부 매체 네트워크**는 열린 RTB로 규모를 확장합니다.

---

## 7. 닫힌 생태계의 Bid Shading: 정보를 다 가진 플랫폼이 입찰가를 깎는 이유

앞서 닫힌 생태계는 시장 가격 정보를 내부적으로 보유한다고 설명했습니다. 그래서 [Bid Shading](post.html?id=bid-shading-censored)이 필요 없습니다. 그런데 카카오 모먼트의 "스마트 입찰", 네이버의 "자동 입찰" 같은 기능이 있습니다. 이 기능은 본질적으로 **플랫폼이 광고주 대신 입찰가를 깎아주는 Bid Shading**입니다. 왜 정보를 다 가진 플랫폼이 Bid Shading을 도입했을까요?

### 이유 1: 통합 경매(Unified Auction) -- 외부 DSP와의 공정한 경쟁
...
해결책은 **모두 1st Price로 통일**하는 것입니다. 하지만 1st Price로 전환하면 내부 광고주가 300원을 그대로 지불하게 됩니다. 이전에는 201원만 냈는데 갑자기 300원을 내라고 하면? 이때 플랫폼은 **"경쟁 상황을 보니 201원이면 이깁니다"**라고 판단해 자동으로 입찰가를 깎아줍니다. 이것이 통합 경매 환경의 Bid Shading입니다.
...
### 열린 RTB의 Bid Shading과의 차이

| 비교 항목 | 열린 RTB Bid Shading | 닫힌 생태계 Bid Shading |
|-----------|---------------------|--------------------------|
| 수행 주체 | DSP (외부) | 플랫폼 (내부) |
...
열린 RTB 에서 DSP 가 하는 Bid Shading 은 보이지 않는 경쟁가를 추정하는 통계 문제입니다. 반면 닫힌 생태계 의 Bid Shading 은 **모든 입찰가를 보면서 광고주에게 최적가를 제시하는** 최적화 문제에 가깝습니다. 기술적 난이도는 다르지만, 둘 다 "1st Price 환경에서 과다 지불을 방지한다"는 목적은 동일합니다.

> 자동 입찰 알고리즘은 [Auto-Bidding 포스트](post.html?id=auto-bidding-pacing)에서 다룹니다. PID Controller, Lagrangian Dual 등이 그 예입니다. 닫힌 생태계의 "스마트 입찰"은 이 알고리즘들의 단순화된 버전으로 이해할 수 있습니다.

---

## 8. 같은 광고비, 다른 실수령액: 담장 안이 돈이 되는 구조

**같은 광고비 ₩10,000,000이 들어와도, 열린 RTB와 담장 안에서 남는 돈이 다릅니다.** 중간에 몇 회사를 거치는지, 그리고 그 돈이 얼마나 정확한 곳에 쓰이는지가 다릅니다.

월 예산 ₩10,000,000을 쓰는 광고주가 있다고 하겠습니다. 열린 RTB에서는 이 돈이 DSP → 광고 거래소 → SSP를 거쳐야 매체에 도착합니다. 담장 안에서는 광고주가 곧바로 플랫폼 한 곳에만 돈을 냅니다.

| 단계 | 열린 RTB 잔액 | 담장 안 잔액 |
|---|---|---|
| 광고주 월 예산 | ₩10,000,000 | ₩10,000,000 |
| DSP 수수료 -15% | ₩8,500,000 | 해당 없음(회사가 하나) |
| 광고 거래소 수수료 -10% | ₩7,650,000 | 해당 없음 |
...
그런데 격차는 여기서 끝나지 않습니다. 담장 안 플랫폼은 같은 유저에 대해 훨씬 많은 1st-party 데이터를 갖고 있습니다. 2절에서 본 것처럼, 이 데이터는 pCTR 예측을 더 정확하게 만듭니다. 예측이 정확할수록 광고와 유저를 더 잘 맞춥니다. 같은 예산이라도 실제 클릭·전환이 더 많이 나온다는 뜻입니다.
...
그 결과 담장 안에서 낙찰된 광고들의 평균 진짜 CTR(4.21%)이 열린 RTB(3.95%)보다 높습니다. 격차 자체는 1.06배로 크지 않아 보입니다. 하지만 이 차이는 노출되는 모든 광고에 반복해서 곱해집니다. 2절의 랭킹 공식은 Score = pCTR × Bid였습니다. pCTR이 정확할수록 실현 eCPM도 최적점에 더 가까워집니다.
...
Safari의 ITP, Firefox의 ETP는 오래전부터 서드파티 쿠키를 기본 차단합니다. Chrome 은 3rd-party 쿠키를 없애겠다고 했다가 2025년에 유지하기로 했습니다. Apple은 2021년 4월 iOS 14.5부터 ATT(App Tracking Transparency)를 도입했습니다. 앱이 다른 앱·웹사이트에서 유저를 추적하려면 사용자 동의를 먼저 받아야 합니다.
...
1. **닫힌 생태계는 데이터 통합의 힘**으로 열린 RTB 대비 정확한 타겟팅과 전환 추적이 가능합니다. 3rd-party 신호가 줄어들수록 이 우위는 더 강해집니다.

2. **pCTR 모델링의 관점이 다릅니다** — 열린 RTB에서는 "제한된 정보로 참 가치를 추정"하는 게 핵심이지만, 닫힌 생태계에서는 "풍부한 데이터로 자리 편향을 분리하고 순수 품질을 평가"하는 게 핵심입니다.

3. **경매 구조가 다르지만, Bid Shading은 공통 과제** — 가려진 데이터 문제는 닫힌 생태계에서는 발생하지 않지만, 통합 경매와 광고주 보호를 위해 Bid Shading 자체는 닫힌 생태계에서도 필수 기술이 되었습니다. 다만 "모르는 가격을 추정하는 문제"가 아니라 "아는 가격에서 최적 깎기를 결정하는 문제"로 성격이 달라집니다.

4. **현실은 하이브리드** — 순수한 닫힌 생태계는 없습니다. 모든 주요 플랫폼이 자사 매체(내부 경매)와 외부 네트워크(Open RTB)를 동시에 운영하며, 이 두 세계를 잇는 통합 데이터 플랫폼이 경쟁력의 핵심입니다.

5. **AdTech 엔지니어에게 시사점** — 열린 RTB 기술(Bid Shading, MAB, Censored Regression)과 닫힌 생태계 기술(자리 편향 보정, IPS, 반사실 채점) 모두를 이해해야 현대 광고 시스템의 전체 그림이 그려집니다. Censored Regression 은 보이지 않는 절반을 감안해 맞추는 회귀입니다.
...
- 담장 안/열린 RTB에서 측정(어트리뷰션)이 어떻게 다른지 → [어트리뷰션 입문](post.html?id=attribution-basics)
